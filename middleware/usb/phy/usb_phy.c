...
void *USB_EhciPhyGetBase(uint8_t controllerId)
{
	...
	} else if ((controllerId == (uint8_t)kUSB_ControllerIp3516Hs0) || (controllerId ==
									   (uint8_t)
	...
	for (instance = 0; instance < (sizeof(usbphy_base_temp) / sizeof(usbphy_base_temp[0]));
	     instance++) {
...
uint32_t USB_EhciPhyInit(uint8_t controllerId, uint32_t freq, usb_phy_config_struct_t *phyConfig)
{
	...
	ANATOP->HW_ANADIG_REG_3P0.RW =
		(ANATOP->HW_ANADIG_REG_3P0.RW &
		 (~(ANATOP_HW_ANADIG_REG_3P0_OUTPUT_TRG(0x1F) |
		    ANATOP_HW_ANADIG_REG_3P0_ENABLE_ILIMIT_MASK))) |
		ANATOP_HW_ANADIG_REG_3P0_OUTPUT_TRG(0x17) |
		ANATOP_HW_ANADIG_REG_3P0_ENABLE_LINREG_MASK;
	ANATOP->HW_ANADIG_USB2_CHRG_DETECT.SET =
		ANATOP_HW_ANADIG_USB2_CHRG_DETECT_CHK_CHRG_B_MASK |
		ANATOP_HW_ANADIG_USB2_CHRG_DETECT_EN_B_MASK;
#endif
	...
	usbPhyBase->PWD = 0U;
	if (((uint8_t)kUSB_ControllerIp3516Hs0 == controllerId) ||
	    ((uint8_t)kUSB_ControllerIp3516Hs1 == controllerId) ||
	...
	if (NULL != phyConfig) {
		...
		usbPhyBase->TX =
			((usbPhyBase->TX & (~(USBPHY_TX_D_CAL_MASK | USBPHY_TX_TXCAL45DM_MASK |
					      USBPHY_TX_TXCAL45DP_MASK))) |
			 (USBPHY_TX_D_CAL(phyConfig->D_CAL) | USBPHY_TX_TXCAL45DP(
				  phyConfig->TXCAL45DP) |
...
void USB_PhyDeviceForceEnterFSMode(uint8_t controllerId, uint8_t enable)
{
	...
	if (0U != enable) {
		...
		while ((0U != (usbPhyBase->USB1_VBUS_DET_STAT &
			       USBPHY_USB1_VBUS_DET_STAT_VBUS_VALID_3V_MASK)) && (0U != delay)) {
			delay--;
		}
