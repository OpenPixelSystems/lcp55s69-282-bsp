...
usb_status_t USB_DeviceCdcRndisKeepaliveCommand(usb_device_cdc_rndis_struct_t *handle,
						uint8_t **message, uint32_t *len);
usb_status_t USB_DeviceCdcRndisQueryCommand(usb_device_cdc_rndis_struct_t *handle,
					    uint8_t **message, uint32_t *len);
usb_status_t USB_DeviceCdcRndisSetCommand(usb_device_cdc_rndis_struct_t *handle, uint8_t **message,
					  uint32_t *len);
...
s_rndisCommand[RNDIS_MAX_EXPECTED_COMMAND_SIZE];
...
static usb_status_t USB_DeviceCdcRndisResponseAvailable(usb_device_cdc_rndis_struct_t *handle)
{
	...
	status = USB_DeviceCdcAcmSend((void *)cdcAcmHandle, cdcAcmHandle->interruptIn.ep,
				      s_responseAvailableData,
				      NOTIF_PACKET_SIZE);
	return status;
}
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
usb_status_t USB_DeviceCdcRndisMessageSet(usb_device_cdc_rndis_struct_t *handle, uint8_t **message,
					  uint32_t *len)
{
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
usb_status_t USB_DeviceCdcRndisMessageGet(usb_device_cdc_rndis_struct_t *handle, uint8_t **message,
					  uint32_t *len)
{
	uint32_t messageType;
	...
	*len = 0U;
	...
	case RNDIS_SET_MSG:
		...
		((rndis_set_msg_struct_t *)((void *)handle->rndisCommand))->informationBufferLength
			=
				USB_LONG_TO_LITTLE_ENDIAN(
					((rndis_set_msg_struct_t *)((void *)handle->rndisCommand))->
						 ...
						 uint32_t *			len)
{
	...
	rndisInitCmplt->messageLength = USB_LONG_TO_LITTLE_ENDIAN(
		RNDIS_RESPONSE_INITIALIZE_MSG_SIZE);
	...
	rndisInitCmplt->maxPacketsPerTransfer = USB_LONG_TO_LITTLE_ENDIAN(
		RNDIS_SINGLE_PACKET_TRANSFER);
	...
	rndisInitCmplt->packetAlignmentFactor = USB_LONG_TO_LITTLE_ENDIAN(
		RNDIS_PACKET_ALIGNMENT_FACTOR);
...

{
	...
	/* preparing for Byte 4-7 : MessageLength*/
	rndisKeepaliveCmplt->messageLength = USB_LONG_TO_LITTLE_ENDIAN(
		RNDIS_RESPONSE_KEEPALIVE_MSG_SIZE);
...

{
	...
	case NDIS_OID_GEN_MAXIMUM_FRAME_SIZE:
		...
		if (NULL != handle->rndisCallback) {
			...
			(void)handle->rndisCallback(handle->cdcAcmHandle,
	...
	case NDIS_OID_GEN_LINK_SPEED:
		...
		if (NULL != handle->rndisCallback) {
			 ...
			 * USB_DeviceCdcRndisInit */
			(void)handle->rndisCallback(handle->cdcAcmHandle,
						    kUSB_DeviceCdcEventAppGetLinkSpeed, &reqParam);
		}
	...
	case NDIS_OID_GEN_VENDOR_ID:
		...
		*((uint32_t *)infoBuf) = USB_LONG_TO_LITTLE_ENDIAN(
			(uint32_t)(((uint32_t)RNDIS_VENDOR_ID << 8U) |
				   (uint32_t)RNDIS_NIC_IDENTIFIER_VENDOR));
		break;
	...
	case NDIS_OID_802_3_PERMANENT_ADDRESS:
		...
		if (NULL != handle->rndisCallback) {
			 ...
			 * USB_DeviceCdcRndisInit */
			(void)handle->rndisCallback(handle->cdcAcmHandle,
						    kUSB_DeviceCdcEventAppGetMacAddress, &reqParam);
		}
	...
	case NDIS_OID_802_3_RCV_ERROR_ALIGNMENT:
		...
		infoBufLen = sizeof(uint32_t);
		*((uint32_t *)infoBuf) = USB_LONG_TO_LITTLE_ENDIAN(
			handle->numRecvFramesAlignmentError);
		break;
	...
	*len = RNDIS_RESPONSE_SET_MSG_SIZE;
	...
	case NDIS_OID_GEN_CURRENT_PACKET_FILTER:
		...
		if (rndisSetMsg->informationBufferLength != 4U) {
			...
			temp = (void *)((uint8_t *)(&rndisSetMsg->requestID) +
					rndisSetMsg->informationBufferOffset);
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
usb_status_t USB_DeviceCdcRndisResetCommand(usb_device_cdc_rndis_struct_t *handle,
					    uint8_t **message, uint32_t *len)
