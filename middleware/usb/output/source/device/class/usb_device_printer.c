 ...
 */
static usb_status_t USB_DevicePrinterBulkInCallback(usb_device_handle
						    handle,
						    ...
						    callbackParam)
{
	...
	if ((NULL != printerHandle->classConfig) && (NULL !=
						     printerHandle->classConfig->classCallback)) {
 ...
 * @retval kStatus_USB_InvalidHandle    The device handle not be found.
 */
static usb_status_t USB_DevicePrinterBulkOutCallback(usb_device_handle
						     handle,
						     usb_device_endpoint_callback_message_struct_t *
						     message,
						     void *
						     callbackParam)
{
	...
	if ((NULL != printerHandle->classConfig) && (NULL !=
		 ...
		 * it is from the second parameter of classInit */
		status = printerHandle->classConfig->classCallback((class_handle_t)printerHandle,
								   kUSB_DevicePrinterEventRecvResponse,
								   message);
	}
...
static usb_status_t USB_DevicePrinterEndpointsInit(usb_device_printer_struct_t *printerHandle)
{
	...
	if ((printerHandle->configuration == 0U) ||
	    (printerHandle->configuration >
	     printerHandle->classConfig->classInfomation->configurations)) {
		return status;
	}

	configInterfaceList =
		&(printerHandle->classConfig->classInfomation->interfaceList[printerHandle->
									     configuration - 1U]);
	for (interfaceIndex = 0; interfaceIndex < configInterfaceList->count; ++interfaceIndex) {
		if (USB_DEVICE_CONFIG_PRINTER_CLASS_CODE ==
		    configInterfaceList->interfaces[interfaceIndex].classCode) {
			/* index means the alternate interface's index here */
			for (index = 0; index <
			     configInterfaceList->interfaces[interfaceIndex].count; ++index) {
				if (configInterfaceList->interfaces[interfaceIndex].interface[index]
				    .alternateSetting ==
				    printerHandle->alternate) {
					interface =
						&(configInterfaceList->interfaces[interfaceIndex].
						  interface[
							  index]);
					break;
				}
			}
			printerHandle->interfaceNumber =
				configInterfaceList->interfaces[interfaceIndex].interfaceNumber;
			break;
		}
	}
	if (interface == NULL) {
	...
	for (index = 0; index < interface->endpointList.count; ++index) {
		...
		epInitStruct.endpointAddress =
			interface->endpointList.endpoint[index].endpointAddress;
		...
		status = USB_DeviceInitEndpoint(printerHandle->deviceHandle, &epInitStruct,
						&epCallback);
	}
...
static usb_status_t USB_DevicePrinterEndpointsDeinit(usb_device_printer_struct_t *printerHandle)
{
	...
	for (index = 0; index < printerHandle->interfaceHandle->endpointList.count; ++index) {
		status = USB_DeviceDeinitEndpoint(printerHandle->deviceHandle,
						  printerHandle->interfaceHandle->endpointList.
						  endpoint[index].endpointAddress);
	}
...
usb_status_t USB_DevicePrinterEvent(void *handle, uint32_t event, void *param)
{
	...
	case kUSB_DeviceClassEventSetEndpointHalt:
		if ((printerHandle->classConfig == NULL) || (printerHandle->interfaceHandle ==
							     NULL)) {
			break;
		}
		/* Get the endpoint address */
		temp8 = *((uint8_t *)param);
		for (temp16 = 0; temp16 < printerHandle->interfaceHandle->endpointList.count;
		     ++temp16) {
			if (temp8 ==
			    printerHandle->interfaceHandle->endpointList.endpoint[temp16].
			    endpointAddress) {
				...
				status = USB_DeviceStallEndpoint(printerHandle->deviceHandle,
								 temp8);
				break;
			}
		}
	...
	case kUSB_DeviceClassEventClearEndpointHalt:
		...
		for (temp16 = 0; temp16 < printerHandle->interfaceHandle->endpointList.count;
			...
			if (temp8 ==
				...
				status = USB_DeviceUnstallEndpoint(printerHandle->deviceHandle,
								   temp8);
				if (USB_IN == (((temp8) &
						USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) >>
					       USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT)) {
					if (0U != printerHandle->bulkInPipeStall) {
						...
						if ((uint8_t *)USB_UNINITIALIZED_VAL_32 !=
						    printerHandle->bulkInPipeDataBuffer) {
							status = USB_DeviceSendRequest(
								printerHandle->deviceHandle,
								(printerHandle->interfaceHandle->
								 endpointList.endpoint[temp16].
								 endpointAddress &
								 USB_DESCRIPTOR_ENDPOINT_ADDRESS_NUMBER_MASK),
								printerHandle->bulkInPipeDataBuffer,
								printerHandle->bulkInPipeDataLen);
							if (kStatus_USB_Success != status) {
								...
								endpointCallbackMessage.length =
									printerHandle->
									bulkInPipeDataLen;
								endpointCallbackMessage.isSetup =
									0U;
								(void)
								USB_DevicePrinterBulkInCallback(
									printerHandle->deviceHandle,
									(
										void *)&
									endpointCallbackMessage,
					...
					if (printerHandle->bulkOutPipeStall == 1U) {
						printerHandle->bulkOutPipeStall = 0U;
						if ((uint8_t *)USB_UNINITIALIZED_VAL_32 !=
						    printerHandle->bulkOutPipeDataBuffer) {
							status = USB_DeviceRecvRequest(
								...
								(printerHandle->interfaceHandle->
								 ...
								 USB_DESCRIPTOR_ENDPOINT_ADDRESS_NUMBER_MASK),
								printerHandle->bulkOutPipeDataBuffer,
								printerHandle->bulkOutPipeDataLen);
							if (kStatus_USB_Success != status) {
								usb_device_endpoint_callback_message_struct_t
									endpointCallbackMessage;
								endpointCallbackMessage.buffer =
									printerHandle->
									bulkOutPipeDataBuffer;
								endpointCallbackMessage.length =
									printerHandle->
									bulkOutPipeDataLen;
								endpointCallbackMessage.isSetup =
	...
	case kUSB_DeviceClassEventClassRequest:
	{
		...
		case USB_DEVICE_PRINTER_GET_DEVICE_ID:
			...
			status = printerHandle->classConfig->classCallback(
				(class_handle_t)printerHandle, kUSB_DevicePrinterEventGetDeviceId,
				&classRequest);
		...
		case USB_DEVICE_PRINTER_GET_PORT_STATUS:
			...
			if (classRequest.interface != printerHandle->interfaceNumber) {
				 ...
				 * it is from the second parameter of classInit */
				status = printerHandle->classConfig->classCallback(
					(class_handle_t)printerHandle,
					kUSB_DevicePrinterEventGetPortStatus, &classRequest);
				controlRequest->buffer = classRequest.buffer;
				controlRequest->length = classRequest.length;
			}
			break;
		...
		case USB_DEVICE_PRINTER_SOFT_RESET:
			...
			if (classRequest.interface != printerHandle->interfaceNumber) {
				...
				status = printerHandle->classConfig->classCallback(
					(class_handle_t)printerHandle,
					kUSB_DevicePrinterEventSoftReset, &classRequest);
			}
			break;
