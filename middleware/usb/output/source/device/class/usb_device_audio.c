...
usb_status_t USB_DeviceAudioInterruptIn(usb_device_handle				handle,
					...
					void *
					callbackParam);
usb_status_t USB_DeviceAudioIsochronousIn(usb_device_handle
					  handle, usb_device_endpoint_callback_message_struct_t *
					  message, void *
					  callbackParam);
usb_status_t USB_DeviceAudioIsochronousOut(usb_device_handle
					   handle, usb_device_endpoint_callback_message_struct_t *
					   message, void *
					   callbackParam);
					...
					callbackParam)
{
	...
	if ((NULL != audioHandle->configStruct) && (NULL !=
					  ...
					  callbackParam)
{
	...
	if ((NULL != audioHandle->configStruct) && (NULL !=
		...
		status = audioHandle->configStruct->classCallback((class_handle_t)audioHandle,
								  kUSB_DeviceAudioEventStreamSendResponse,
								  message);
	}
...
usb_status_t USB_DeviceAudioStreamEndpointsInit(usb_device_audio_struct_t *audioHandle)
{
	...
	if (audioHandle->configuration >
	    audioHandle->configStruct->classInfomation->configurations) {
		return status;
	}
		...
		if ((USB_DEVICE_CONFIG_AUDIO_CLASS_CODE ==
			...
			for (index = 0; index < interfaceList->interfaces[count].count; index++) {
				if (interfaceList->interfaces[count].interface[index].
				    alternateSetting == audioHandle->streamAlternate) {
					interface =
						&interfaceList->interfaces[count].interface[index];
					break;
				}
			}
			audioHandle->streamInterfaceNumber =
	...
	for (count = 0U; count < interface->endpointList.count; count++) {
		...
		if ((USB_ENDPOINT_ISOCHRONOUS == (epInitStruct.transferType &
...
usb_status_t USB_DeviceAudioStreamEndpointsDeinit(usb_device_audio_struct_t *audioHandle)
{
	...
	for (count = 0U; count < audioHandle->streamInterfaceHandle->endpointList.count; count++) {
		status = USB_DeviceDeinitEndpoint(
			audioHandle->handle,
			audioHandle->streamInterfaceHandle->endpointList.endpoint[count].
			endpointAddress);
	}

	for (count = 0U; count < audioHandle->streamInterfaceHandle->endpointList.count; count++) {
		if ((audioHandle->streamInterfaceHandle->endpointList.endpoint[count].
		     endpointAddress &
		     USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) >>
		    USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT ==
		    USB_IN) {
			if (0U != audioHandle->streamInPipeBusy) {
				...
				(void)USB_DeviceAudioIsochronousIn(audioHandle->handle, &message,
								   audioHandle);
			}
		} else {
			if (0U != audioHandle->streamOutPipeBusy) {
				message.length = USB_UNINITIALIZED_VAL_32;
				(void)USB_DeviceAudioIsochronousOut(audioHandle->handle, &message,
								    audioHandle);
			}
		}
	}
	...
	for (count = 0U; count < interfaceList->count; count++) {
		if ((USB_DEVICE_CONFIG_AUDIO_CLASS_CODE ==
			...
			for (index = 0U; index < interfaceList->interfaces[count].count; index++) {
				if (interfaceList->interfaces[count].interface[index].
				    alternateSetting == audioHandle->controlAlternate) {
					interface =
						&interfaceList->interfaces[count].interface[index];
					break;
				}
			}
			audioHandle->controlInterfaceNumber =
				interfaceList->interfaces[count].interfaceNumber;
			break;
		}
	}
...
usb_status_t USB_DeviceAudioControlEndpointsDeinit(usb_device_audio_struct_t *audioHandle)
{
	...
	for (count = 0U; count < audioHandle->controlInterfaceHandle->endpointList.count; count++) {
		status = USB_DeviceDeinitEndpoint(
			audioHandle->handle,
			audioHandle->controlInterfaceHandle->endpointList.endpoint[count].
			endpointAddress);
	}
	...
	for (i = 0; i < entity_list->count; i++) {
		if (entityId == entity_list->entity[i].entityId) {
			...
			case USB_DESCRIPTOR_SUBTYPE_AUDIO_CONTROL_OUTPUT_TERMINAL:
				...
				status = USB_DeviceAudioSetControlTerminal(audioHandle,
									   controlRequest);
				break;
	...
	for (i = 0; i < entity_list->count; i++) {
		if (entityId == entity_list->entity[i].entityId) {
			...
			case USB_DESCRIPTOR_SUBTYPE_AUDIO_CONTROL_INPUT_TERMINAL:
				status = USB_DeviceAudioGetControlTerminal(audioHandle,
									   controlRequest);
				break;
			case USB_DESCRIPTOR_SUBTYPE_AUDIO_CONTROL_FEATURE_UNIT:
...
usb_status_t USB_DeviceAudioEvent(void *handle, uint32_t event, void *param)
{
	...
	case kUSB_DeviceClassEventSetInterface:
		...
		if (audioHandle->controlInterfaceNumber == ((uint8_t)(interfaceAlternate >> 8U))) {
			...
			error = USB_DeviceAudioControlEndpointsInit(audioHandle);
		} else if (audioHandle->streamInterfaceNumber == ((uint8_t)(interfaceAlternate >>
									    8U))) {
	...
	case kUSB_DeviceClassEventSetEndpointHalt:
		...
		if (NULL != audioHandle->controlInterfaceHandle) {
			for (count = 0U; count <
			     audioHandle->controlInterfaceHandle->endpointList.count; count++) {
				if (*temp8 ==
				    audioHandle->controlInterfaceHandle->endpointList.endpoint[count
				    ].
				    endpointAddress) {
					/* Only stall the endpoint belongs to control interface of the class */
					error = USB_DeviceStallEndpoint(audioHandle->handle,
									*temp8);
				}
			}
		}
		if (NULL != audioHandle->streamInterfaceHandle) {
			for (count = 0U; count <
			     audioHandle->streamInterfaceHandle->endpointList.count; count++) {
				if (*temp8 ==
				    audioHandle->streamInterfaceHandle->endpointList.endpoint[count]
				    .
				    endpointAddress) {
		...
		if (NULL != audioHandle->controlInterfaceHandle) {
			for (count = 0U; count <
				...
				if (*temp8 ==
					...
					error = USB_DeviceUnstallEndpoint(audioHandle->handle,
									  *temp8);
				}
			}
		}
...
#if (USB_DEVICE_CONFIG_AUDIO_CLASS_2_0)
	{
		...
		if (audioHandle->controlInterfaceNumber == interface_index) {
			entity_list =
				(usb_device_audio_entities_struct_t *)audioHandle->
				controlInterfaceHandle
				->classSpecific;
				...
				if (entityId == entity_list->entity[i].entityId) {
					...
					case USB_DESCRIPTOR_SUBTYPE_AUDIO_CONTROL_FEATURE_UNIT:
						if (((controlRequest->setup->bmRequestType &
							...
							switch (controlRequest->setup->wValue >>
								8) {
							case USB_DEVICE_AUDIO_FU_MUTE_CONTROL:
								audioCommand =
									USB_DEVICE_AUDIO_GET_CUR_MUTE_CONTROL_AUDIO20;
								break;
							case USB_DEVICE_AUDIO_FU_VOLUME_CONTROL:
								if (controlRequest->setup->bRequest
								    ==
								    USB_DEVICE_AUDIO_REQUEST_CUR) {
									audioCommand =
										USB_DEVICE_AUDIO_GET_CUR_VOLUME_CONTROL_AUDIO20;
								} else if (controlRequest->setup->
									   bRequest ==
									   USB_DEVICE_AUDIO_REQUEST_RANGE)
								{
									audioCommand =
										USB_DEVICE_AUDIO_GET_RANGE_VOLUME_CONTROL_AUDIO20;
								} else {
									/*no action*/
								}
								break;
						...
						} else if (((controlRequest->setup->bmRequestType &
							     USB_REQUEST_TYPE_DIR_MASK) ==
							...
							case USB_DEVICE_AUDIO_FU_MUTE_CONTROL:
								audioCommand =
									USB_DEVICE_AUDIO_SET_CUR_MUTE_CONTROL_AUDIO20;
								break;
							case USB_DEVICE_AUDIO_FU_VOLUME_CONTROL:
								if (controlRequest->setup->bRequest
								    ==
								    USB_DEVICE_AUDIO_REQUEST_CUR) {
									audioCommand =
										USB_DEVICE_AUDIO_SET_CUR_VOLUME_CONTROL_AUDIO20;
								}
								break;
							default:
								/*no action*/
						...
						if (((controlRequest->setup->bmRequestType &
							...
							case USB_DEVICE_AUDIO_CS_SAM_FREQ_CONTROL:
								if (controlRequest->setup->bRequest
								    ...
								    USB_DEVICE_AUDIO_REQUEST_CUR) {
									audioCommand =
										USB_DEVICE_AUDIO_GET_CUR_SAM_FREQ_CONTROL;
								} else if (controlRequest->setup->
									   bRequest ==
									   USB_DEVICE_AUDIO_REQUEST_RANGE)
								{
									audioCommand =
										USB_DEVICE_AUDIO_GET_RANGE_SAM_FREQ_CONTROL;
								} else {
									/*no action*/
								}
								break;
							case USB_DEVICE_AUDIO_CS_CLOCK_VALID_CONTROL
								:
								audioCommand =
									USB_DEVICE_AUDIO_GET_CUR_CLOCK_VALID_CONTROL;
								break;
							default:
								/*no action*/
								break;
							...
							case USB_DEVICE_AUDIO_CS_SAM_FREQ_CONTROL:
								audioCommand =
									USB_DEVICE_AUDIO_SET_CUR_SAM_FREQ_CONTROL;
								break;
							case USB_DEVICE_AUDIO_CS_CLOCK_VALID_CONTROL
								:
								audioCommand =
									USB_DEVICE_AUDIO_SET_CUR_CLOCK_VALID_CONTROL;
								break;
							default:
								/*no action*/
								break;
			...
			if ((0U != audioCommand) && (NULL != audioHandle->configStruct) &&
				...
				/* classCallback is initialized in classInit of s_UsbDeviceClassInterfaceMap,
				 *       it is from the second parameter of classInit*/
				error = audioHandle->configStruct->classCallback(
					(class_handle_t)audioHandle, audioCommand,
					controlRequest);
			}
		}
	}
			...
			if ((controlRequest->setup->bmRequestType &
				...
				if (USB_REQUEST_TYPE_TYPE_CLASS ==
					...
					case USB_DEVICE_AUDIO_SET_REQUEST_ENDPOINT:
						error = USB_DeviceAudioSetRequestEndpoint(
							audioHandle, controlRequest);
						break;
					case USB_DEVICE_AUDIO_GET_REQUEST_ENDPOINT:
						error = USB_DeviceAudioGetRequestEndpoint(
							audioHandle, controlRequest);
						break;
					default:
						/*no action*/
				...
				if (audioHandle->controlInterfaceNumber == interface_index) {
					if (USB_REQUEST_TYPE_TYPE_CLASS ==
						...
						case USB_DEVICE_AUDIO_SET_REQUEST_INTERFACE:
							error = USB_DeviceAudioSetRequestInterface(
								audioHandle, controlRequest);
							break;
						case USB_DEVICE_AUDIO_GET_REQUEST_INTERFACE:
							error = USB_DeviceAudioGetRequestInterface(
								audioHandle, controlRequest);
							break;
						default:
							/*no action*/
