...
static usb_status_t USB_DeviceVideoControlIn(usb_device_handle
					     handle, usb_device_endpoint_callback_message_struct_t *
					     message, void *
					     callbackParam);
static usb_status_t USB_DeviceVideoStreamIn(usb_device_handle
					    handle, usb_device_endpoint_callback_message_struct_t *
					    message, void *
					    callbackParam);
static usb_status_t USB_DeviceVideoStreamOut(usb_device_handle
					     handle, usb_device_endpoint_callback_message_struct_t *
					     message, void *
					     callbackParam);
...
static usb_status_t USB_DeviceVideoVsStillCommitRequest(usb_device_video_struct_t *videoHandle,
							usb_device_control_request_struct_t *
							controlRequest);
static usb_status_t USB_DeviceVideoVsStillImageTriggerRequest(
					     ...
					     callbackParam)
{
	...
	if ((NULL != videoHandle->configStruct) && (NULL !=
					    ...
					    callbackParam)
{
	...
	if ((NULL != videoHandle->configStruct) && (NULL !=
		 ...
		 * it is from the second parameter of classInit */
		error = videoHandle->configStruct->classCallback((class_handle_t)videoHandle,
								 kUSB_DeviceVideoEventStreamSendResponse,
								 message);
	}
...
static usb_status_t USB_DeviceVideoStreamEndpointsInit(usb_device_video_struct_t *videoHandle)
{
	...
	if (videoHandle->configuration >
	    videoHandle->configStruct->classInfomation->configurations) {
		return status;
	}
	...
	interfaceList =
		&videoHandle->configStruct->classInfomation->interfaceList[videoHandle->
									   configuration - 1U];
	...
	for (uint32_t count = 0U; count < interfaceList->count; count++) {
		if ((USB_DEVICE_VIDEO_CC_VIDEO == interfaceList->interfaces[count].classCode) &&
		    (USB_DEVICE_VIDEO_SC_VIDEOSTREAMING ==
			...
			for (uint32_t index = 0U; index < interfaceList->interfaces[count].count;
				...
				if (interfaceList->interfaces[count].interface[index].
				    alternateSetting == videoHandle->streamAlternate) {
					interface =
						&interfaceList->interfaces[count].interface[index];
					break;
				}
			}
			videoHandle->streamInterfaceNumber =
...
static usb_status_t USB_DeviceVideoStreamEndpointsDeinit(usb_device_video_struct_t *videoHandle)
{
	...
	for (uint32_t count = 0U; count < videoHandle->streamInterfaceHandle->endpointList.count;
	     count++) {
		status = USB_DeviceDeinitEndpoint(
			videoHandle->handle,
			videoHandle->streamInterfaceHandle->endpointList.endpoint[count].
			endpointAddress);
	}
...
static usb_status_t USB_DeviceVideoControlEndpointsInit(usb_device_video_struct_t *videoHandle)
{
	...
	for (uint32_t count = 0U; count < interfaceList->count; count++) {
		if ((USB_DEVICE_VIDEO_CC_VIDEO == interfaceList->interfaces[count].classCode) &&
		    (USB_DEVICE_VIDEO_SC_VIDEOCONTROL ==
		     interfaceList->interfaces[count].subclassCode)) {
			for (uint32_t index = 0U; index < interfaceList->interfaces[count].count;
			     index++) {
				if (interfaceList->interfaces[count].interface[index].
				    alternateSetting == videoHandle->controlAlternate) {
					interface =
						&interfaceList->interfaces[count].interface[index];
					break;
				}
			}
			videoHandle->controlInterfaceNumber =
				interfaceList->interfaces[count].interfaceNumber;
			break;
		}
	}
...
static usb_status_t USB_DeviceVideoControlEndpointsDeinit(usb_device_video_struct_t *videoHandle)
{
	...
	for (uint32_t count = 0U; count < videoHandle->controlInterfaceHandle->endpointList.count;
	     count++) {
		status = USB_DeviceDeinitEndpoint(
			videoHandle->handle,
			videoHandle->controlInterfaceHandle->endpointList.endpoint[count].
			endpointAddress);
	}
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
static usb_status_t USB_DeviceVideoVcPowerModeControl(
	usb_device_video_struct_t *videoHandle,
	usb_device_control_request_struct_t *
	controlRequest)
{
	...
	if ((0U != command) && (NULL != videoHandle->configStruct) && (NULL !=
								       videoHandle->configStruct->
								       classCallback)) {
		/* ClassCallback is initialized in classInit of s_UsbDeviceClassInterfaceMap,
		 * it is from the second parameter of classInit */
		status = videoHandle->configStruct->classCallback((class_handle_t)videoHandle,
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
static usb_status_t USB_DeviceVideoVcInterfaceRequest(
	usb_device_video_struct_t *videoHandle,
	usb_device_control_request_struct_t *
	controlRequest)
{
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
static usb_status_t USB_DeviceVideoVcCameraTerminalRequest(
	usb_device_video_struct_t *videoHandle,
	usb_device_control_request_struct_t *
	controlRequest)
{
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
static usb_status_t USB_DeviceVideoVcProcessingUnitRequest(
	usb_device_video_struct_t *videoHandle,
	usb_device_control_request_struct_t *
	controlRequest)
{
					     ...
					     controlRequest)
{
	...
	entity_list =
		(usb_device_video_entities_struct_t *)videoHandle->controlInterfaceHandle->
		classSpecific;
		...
		for (uint32_t i = 0U; i < entity_list->count; i++) {
			if (entityId == entity_list->entity[i].entityId) {
				...
				case USB_DESCRIPTOR_SUBTYPE_VIDEO_VC_INPUT_TERMINAL:
					if (USB_DEVICE_VIDEO_ITT_CAMERA ==
					    entity_list->entity[i].terminalType) {
						/* Handle the video camera terminal control request. */
						status = USB_DeviceVideoVcCameraTerminalRequest(
							videoHandle, controlRequest);
					}
					break;
				...
				case USB_DESCRIPTOR_SUBTYPE_VIDEO_VC_PROCESSING_UNIT:
					...
					status = USB_DeviceVideoVcProcessingUnitRequest(videoHandle,
											controlRequest);
					break;
...
usb_status_t USB_DeviceVideoEvent(void *handle, uint32_t event, void *param)
{
...
#ifdef DEBUG
			usb_echo(
				"kUSB_DeviceClassEventSetConfiguration, USB_DeviceInitEndpoint fail\r\n");
#endif
		}
	...
	case kUSB_DeviceClassEventSetInterface:
		...
		if (videoHandle->controlInterfaceNumber == ((uint8_t)(interfaceAlternate >> 8U))) {
			...
			error = USB_DeviceVideoControlEndpointsInit(videoHandle);
		} else if (videoHandle->streamInterfaceNumber == ((uint8_t)(interfaceAlternate >>
									    8U))) {
	...
	case kUSB_DeviceClassEventSetEndpointHalt:
		...
		temp8 = ((uint8_t *)param);
		if (NULL != videoHandle->controlInterfaceHandle) {
			for (uint32_t count = 0U; count <
			     videoHandle->controlInterfaceHandle->endpointList.count; count++) {
				if (*temp8 ==
				    videoHandle->controlInterfaceHandle->endpointList.endpoint[count
				    ].
				    endpointAddress) {
					/* Only stall the endpoint belongs to control interface of the class */
					error = USB_DeviceStallEndpoint(videoHandle->handle,
									*temp8);
				}
			}
		}
		if (NULL != videoHandle->streamInterfaceHandle) {
			for (uint32_t count = 0U; count <
			     videoHandle->streamInterfaceHandle->endpointList.count; count++) {
				if (*temp8 ==
				    videoHandle->streamInterfaceHandle->endpointList.endpoint[count]
				    .
				    endpointAddress) {
		...
		if (NULL != videoHandle->controlInterfaceHandle) {
			for (uint32_t count = 0U; count <
				...
				if (*temp8 ==
					...
					error = USB_DeviceUnstallEndpoint(videoHandle->handle,
									  *temp8);
				}
			}
		}
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
usb_status_t USB_DeviceVideoInit(uint8_t controllerId, usb_device_class_config_struct_t *config,
