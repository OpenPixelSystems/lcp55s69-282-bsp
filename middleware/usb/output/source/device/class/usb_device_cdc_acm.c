 ...
 * @return A USB error code or kStatus_USB_Success.
 */
static usb_status_t USB_DeviceCdcAcmInterruptIn(usb_device_handle
						handle,
						...
						callbackParam)
{
	...
	if ((NULL != cdcAcmHandle->configStruct) && (NULL !=
						     cdcAcmHandle->configStruct->classCallback)) {
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
static usb_status_t USB_DeviceCdcAcmBulkIn(usb_device_handle
					   handle,
					   usb_device_endpoint_callback_message_struct_t *
					   message,
					   void *
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
static usb_status_t USB_DeviceCdcAcmBulkOut(usb_device_handle
					    handle,
					    usb_device_endpoint_callback_message_struct_t *
					    message,
					    void *
...
static usb_status_t USB_DeviceCdcAcmEndpointsInit(usb_device_cdc_acm_struct_t *cdcAcmHandle)
{
	...
	if ((cdcAcmHandle->configuration == 0U) ||
	    (cdcAcmHandle->configuration >
	     cdcAcmHandle->configStruct->classInfomation->configurations)) {
		return error;
	}

	interfaceList =
		&cdcAcmHandle->configStruct->classInfomation->interfaceList[cdcAcmHandle->
									    configuration -
									    1U];
		...
		if (USB_DEVICE_CONFIG_CDC_COMM_CLASS_CODE ==
			...
			for (index = 0; index < interfaceList->interfaces[count].count; index++) {
				if (interfaceList->interfaces[count].interface[index].
				    alternateSetting == cdcAcmHandle->alternate) {
					interface =
	...
	for (count = 0; count < interface->endpointList.count; count++) {
		...
		if ((USB_IN == ((epInitStruct.endpointAddress &
			...
			cdcAcmHandle->interruptIn.ep = (epInitStruct.endpointAddress &
	...
	for (count = 0; count < interface->endpointList.count; count++) {
		...
		if ((USB_IN == ((epInitStruct.endpointAddress &
			...
			cdcAcmHandle->bulkIn.ep = (epInitStruct.endpointAddress &
						   USB_DESCRIPTOR_ENDPOINT_ADDRESS_NUMBER_MASK);
			cdcAcmHandle->bulkIn.isBusy = 0;
		...
		} else if ((USB_OUT == ((epInitStruct.endpointAddress &
					 USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) >>
					USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT)) &&
			   (USB_ENDPOINT_BULK == epInitStruct.transferType)) {
			cdcAcmHandle->bulkOut.ep = (epInitStruct.endpointAddress &
						    USB_DESCRIPTOR_ENDPOINT_ADDRESS_NUMBER_MASK);
...
static usb_status_t USB_DeviceCdcAcmEndpointsDeinit(usb_device_cdc_acm_struct_t *cdcAcmHandle)
{
	...
	if ((NULL == cdcAcmHandle->commInterfaceHandle) || (NULL ==
	...
	for (count = 0; count < cdcAcmHandle->commInterfaceHandle->endpointList.count; count++) {
		status = USB_DeviceDeinitEndpoint(
			cdcAcmHandle->handle,
			cdcAcmHandle->commInterfaceHandle->endpointList.endpoint[count].
			endpointAddress);
	}
	for (count = 0; count < cdcAcmHandle->dataInterfaceHandle->endpointList.count; count++) {
		status = USB_DeviceDeinitEndpoint(
			cdcAcmHandle->handle,
			cdcAcmHandle->dataInterfaceHandle->endpointList.endpoint[count].
			endpointAddress);
	}
...
usb_status_t USB_DeviceCdcAcmEvent(void *handle, uint32_t event, void *param)
{
...
#ifdef DEBUG
			usb_echo(
				"kUSB_DeviceClassEventSetInterface, USB_DeviceInitEndpoint fail\r\n");
#endif
		}
		break;
	case kUSB_DeviceClassEventSetEndpointHalt:
		...
		temp8 = ((uint8_t *)param);
		for (count = 0; count < cdcAcmHandle->commInterfaceHandle->endpointList.count;
		     count++) {
			if (*temp8 ==
			    cdcAcmHandle->commInterfaceHandle->endpointList.endpoint[count].
			    endpointAddress) {
		...
		for (count = 0; count < cdcAcmHandle->dataInterfaceHandle->endpointList.count;
		     count++) {
			if (*temp8 ==
			    cdcAcmHandle->dataInterfaceHandle->endpointList.endpoint[count].
			    endpointAddress) {
...

	{
		...
		if (USB_REQUEST_TYPE_TYPE_CLASS == (controlRequest->setup->bmRequestType &
				  ...
				  class_handle_t *			handle)
{
	...
	cdcAcmHandle->interruptIn.mutex =
		(osa_mutex_handle_t)&cdcAcmHandle->interruptIn.mutexBuffer[0];
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
usb_status_t USB_DeviceCdcAcmSend(class_handle_t handle, uint8_t ep, uint8_t *buffer, uint32_t
				  length)
{
	usb_device_cdc_acm_struct_t *cdcAcmHandle;
	usb_status_t status = kStatus_USB_Error;
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
usb_status_t USB_DeviceCdcAcmRecv(class_handle_t handle, uint8_t ep, uint8_t *buffer, uint32_t
				  length)
{
	usb_device_cdc_acm_struct_t *cdcAcmHandle;
	usb_status_t status;
