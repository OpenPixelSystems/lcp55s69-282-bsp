...
static usb_status_t USB_DeviceHidInterruptIn(usb_device_handle
					     handle, usb_device_endpoint_callback_message_struct_t *
					     message, void *
					     callbackParam);
static usb_status_t USB_DeviceHidInterruptOut(usb_device_handle
					      handle,
					     ...
					     callbackParam)
{
	...
	if ((NULL != hidHandle->configStruct) && (NULL != hidHandle->configStruct->classCallback)) {
		...
		status =
			hidHandle->configStruct->classCallback((class_handle_t)hidHandle,
...
static usb_status_t USB_DeviceHidEndpointsInit(usb_device_hid_struct_t *hidHandle)
{
	...
	interfaceList =
		&hidHandle->configStruct->classInfomation->interfaceList[hidHandle->configuration -
									 1U];

	/* Find interface by using the alternate setting of the interface. */
	for (count = 0U; count < interfaceList->count; count++) {
		if (USB_DEVICE_CONFIG_HID_CLASS_CODE ==
			...
			for (index = 0U; index < interfaceList->interfaces[count].count; index++) {
				if (interfaceList->interfaces[count].interface[index].
				    alternateSetting == hidHandle->alternate) {
					interface =
						&interfaceList->interfaces[count].interface[index];
					break;
				}
			}
			hidHandle->interfaceNumber =
...
static usb_status_t USB_DeviceHidEndpointsDeinit(usb_device_hid_struct_t *hidHandle)
{
	...
	for (count = 0U; count < hidHandle->interfaceHandle->endpointList.count; count++) {
		status = USB_DeviceDeinitEndpoint(hidHandle->handle,
						  hidHandle->interfaceHandle->endpointList.endpoint[
...
usb_status_t USB_DeviceHidEvent(void *handle, uint32_t event, void *param)
{
	...
	case kUSB_DeviceClassEventSetEndpointHalt:
		...
		for (count = 0U; count < hidHandle->interfaceHandle->endpointList.count; count++) {
			    ...
			    hidHandle->interfaceHandle->endpointList.endpoint[count].endpointAddress)
			{
				...
				if (USB_IN ==
				    ((hidHandle->interfaceHandle->endpointList.endpoint[count].
				      endpointAddress &
				      USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) >>
				     USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT)) {
					...
					if (0U != hidHandle->interruptInPipeStall) {
						...
						if ((uint8_t *)USB_UNINITIALIZED_VAL_32 !=
							...
							error = USB_DeviceSendRequest(
								hidHandle->handle,
								(hidHandle->interfaceHandle->
								 endpointList.endpoint[count].
								 endpointAddress
								 &
					...
					if (0U != hidHandle->interruptOutPipeStall) {
						...
						if ((uint8_t *)USB_UNINITIALIZED_VAL_32 !=
							...
							if (kStatus_USB_Success != error) {
								...
								endpointCallbackMessage.length =
									hidHandle->
									interruptOutPipeDataLen;
								endpointCallbackMessage.isSetup =
									0U;
								(void)USB_DeviceHidInterruptOut(
									hidHandle->handle,
									(void *)&
									endpointCallbackMessage,
									handle);
	...
	case kUSB_DeviceClassEventClassRequest:
	{
		...
		case USB_DEVICE_HID_REQUEST_GET_REPORT:
			/* Get report request */
			report.reportType = (uint8_t)((controlRequest->setup->wValue & 0xFF00U) >>
						      0x08U);
		...
		case USB_DEVICE_HID_REQUEST_SET_IDLE:
			/* Set idle request */
		{
			hidHandle->idleRate = (uint8_t)((controlRequest->setup->wValue & 0xFF00U) >>
							0x08U);
			/* classCallback is initialized in classInit of s_UsbDeviceClassInterfaceMap,
			 * it is from the second parameter of classInit */
			error = hidHandle->configStruct->classCallback(
				(class_handle_t)hidHandle, kUSB_DeviceHidEventSetIdle,
				&hidHandle->idleRate);
		}
		break;
			...
			/* Set protocol request */
		{
			 ...
			 * it is from the second parameter of classInit */
			error = hidHandle->configStruct->classCallback(
				(class_handle_t)hidHandle, kUSB_DeviceHidEventSetProtocol,
				&hidHandle->protocol);
		}
		break;
		default:
