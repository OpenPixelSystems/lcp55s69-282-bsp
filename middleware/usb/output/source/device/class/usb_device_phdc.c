 ...
 */
static usb_status_t USB_DevicePhdcBulkInCallback(usb_device_handle
						 handle,
						 usb_device_endpoint_callback_message_struct_t *
						 message,
						 void *
						 callbackParam)
{
	...
	if ((NULL != phdcHandle->configStruct) && (NULL !=
 ...
 * @retval kStatus_USB_InvalidHandle    The device handle not be found.
 */
static usb_status_t USB_DevicePhdcBulkOutCallback(usb_device_handle
						  handle,
						  usb_device_endpoint_callback_message_struct_t *
						  message,
						  void *
 ...
 * @retval kStatus_USB_InvalidHandle    The device handle not be found.
 */
static usb_status_t USB_DevicePhdcInterruptInCallback(usb_device_handle
						      handle,
						      usb_device_endpoint_callback_message_struct_t
						      *message,
						      void *
						      callbackParam)
{
	...
	if ((NULL != phdcHandle->configStruct) && (NULL !=
		 ...
		 * it is from the second parameter of classInit */
		status = phdcHandle->configStruct->classCallback((class_handle_t)phdcHandle,
								 kUSB_DevicePhdcEventInterruptInSendComplete,
								 message);
	}
	return status;
}
...
static usb_status_t USB_DevicePhdcEndpointsInit(usb_device_phdc_struct_t *phdcHandle)
{
	...
	interfaceList =
		&phdcHandle->configStruct->classInfomation->interfaceList[phdcHandle->configuration
									  - 1U];
	for (uint32_t count = 0U; count < interfaceList->count; count++) {
		if (USB_DEVICE_CONFIG_PHDC_CLASS_CODE ==
			...
			for (uint32_t index = 0U; index < interfaceList->interfaces[count].count;
				...
				if (interfaceList->interfaces[count].interface[index].
				    alternateSetting == phdcHandle->alternate) {
					interface =
						&interfaceList->interfaces[count].interface[index];
					break;
				}
			}
			phdcHandle->interfaceNumber =
	...
	for (uint32_t count = 0U; count < interface->endpointList.count; count++) {
		...
		if (USB_ENDPOINT_INTERRUPT == epInitStruct.transferType) {
			...
			phdcHandle->interruptIn.ep = (epInitStruct.endpointAddress &
		...
		} else if (USB_IN == ((epInitStruct.endpointAddress &
				       USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) >>
				      USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT)) {
			epCallback.callbackFn = USB_DevicePhdcBulkInCallback;
			phdcHandle->bulkIn.ep = (epInitStruct.endpointAddress &
						 USB_DESCRIPTOR_ENDPOINT_ADDRESS_NUMBER_MASK);
			...
			phdcHandle->bulkOut.ep = (epInitStruct.endpointAddress &
						  USB_DESCRIPTOR_ENDPOINT_ADDRESS_NUMBER_MASK);
...
static usb_status_t USB_DevicePhdcEndpointsDeinit(usb_device_phdc_struct_t *phdcHandle)
{
	...
	for (uint32_t count = 0; count < phdcHandle->interfaceHandle->endpointList.count; count++) {
		status = USB_DeviceDeinitEndpoint(phdcHandle->handle,
						  phdcHandle->interfaceHandle->endpointList.endpoint
...
usb_status_t USB_DevicePhdcEvent(void *handle, uint32_t event, void *param)
{
	...
	case kUSB_DeviceClassEventSetEndpointHalt:
		...
		temp8 = ((uint8_t *)param);
		for (uint32_t count = 0; count < phdcHandle->interfaceHandle->endpointList.count;
		     count++) {
			if (*temp8 ==
			    phdcHandle->interfaceHandle->endpointList.endpoint[count].
			    endpointAddress) {
	...
	case kUSB_DeviceClassEventClearEndpointHalt:
		...
		temp8 = ((uint8_t *)param);
		for (uint32_t count = 0U; count < phdcHandle->interfaceHandle->endpointList.count;
		     count++) {
			if (*temp8 ==
			    phdcHandle->interfaceHandle->endpointList.endpoint[count].
			    endpointAddress) {
						...
						if ((uint8_t *)USB_UNINITIALIZED_VAL_32 !=
							...
							if (kStatus_USB_Success != error) {
								...
								endpointCallbackMessage.isSetup =
									0U;
								(void)
								USB_DevicePhdcInterruptInCallback(
									phdcHandle->handle,
									(
										void *)&
									endpointCallbackMessage,
					...
					if (0U != phdcHandle->bulkIn.pipeStall) {
						...
						if ((uint8_t *)USB_UNINITIALIZED_VAL_32 !=
							...
							if (kStatus_USB_Success != error) {
								...
								endpointCallbackMessage.isSetup =
									0U;
								(void)USB_DevicePhdcBulkInCallback(
									phdcHandle->handle,
									(
										void *)&
									endpointCallbackMessage,
					...
					if (0U != phdcHandle->bulkOut.pipeStall) {
						...
						if ((uint8_t *)USB_UNINITIALIZED_VAL_32 !=
							...
							if (kStatus_USB_Success != error) {
								...
								endpointCallbackMessage.isSetup =
									0U;
								(void)USB_DevicePhdcBulkOutCallback(
									phdcHandle->handle,
									(
										void *)&
									endpointCallbackMessage,
	...
	case kUSB_DeviceClassEventClassRequest:
	{
		...
		case USB_DEVICE_PHDC_REQUEST_SET_FEATURE:
		{
			...
			error = phdcHandle->configStruct->classCallback(
				(class_handle_t)phdcHandle, kUSB_DevicePhdcEventSetFeature,
		...
		case USB_DEVICE_PHDC_REQUEST_CLEAR_FEATURE:
		{
			 ...
			 * it is from the second parameter of classInit */
			error = phdcHandle->configStruct->classCallback(
				(class_handle_t)phdcHandle, kUSB_DevicePhdcEventClearFeature,
				&controlRequest->setup->wValue);
		}
		break;
