...
USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) static
usb_device_request_sense_data_struct_t requestSense1;
USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) static usb_device_read_capacity_struct_t
readCapacity1;
USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) static
...
formatCapacityData1[sizeof(usb_device_capacity_list_header_struct_t) +
		    sizeof(usb_device_current_max_capacity_descriptor_struct_t) +
		    sizeof(usb_device_formattable_capacity_descriptor_struct_t) * 3];
USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) usb_device_request_sense_data_struct_t
requestSense2;
USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) usb_device_read_capacity_struct_t
readCapacity2;
USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) usb_device_read_capacity16_data_struct_t
readCapacity162;
...
formatCapacityData2[sizeof(usb_device_capacity_list_header_struct_t) +
		    ...
		    sizeof(usb_device_formattable_capacity_descriptor_struct_t) * 3];
USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) static
usb_device_request_sense_data_struct_t requestSense3;
USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) static usb_device_read_capacity_struct_t
readCapacity3;
USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) static
usb_device_read_capacity16_data_struct_t
...
static usb_status_t USB_DeviceMscProcessUfiCommand(usb_device_msc_struct_t *mscHandle)
{
	...
	ufi->thirteenCase.hostExpectedDirection = (uint8_t)(mscHandle->mscCbw->flags >>
							    USB_DEVICE_MSC_CBW_DIRECTION_SHIFT);
					...
					callbackParam)
{
	...
	if (message->length == USB_UNINITIALIZED_VAL_32) {
		...
		if ((0U != mscHandle->dataInFlag) &&
		    (mscHandle->configurationStruct->classCallback != NULL) &&
			...
			(void)mscHandle->configurationStruct->classCallback(
		...
		if ((mscHandle->configurationStruct->classCallback != NULL)) {
			...
			if (0U != mscHandle->transferRemaining) {
				mscHandle->currentOffset +=
					(message->length /
					 mscHandle->luInformations[mscHandle->mscCbw->
								   logicalUnitNumber].
					 lengthOfEachLba);
			...
			if (0U == mscHandle->transferRemaining) {
				...
				mscHandle->mscCsw->dataResidue = USB_LONG_TO_LITTLE_ENDIAN(
					mscHandle->mscCsw->dataResidue);
				(void)USB_DeviceSendRequest(mscHandle->handle,
							    mscHandle->bulkInEndpoint,
							    (uint8_t *)mscHandle->mscCsw,
							    USB_DEVICE_MSC_CSW_LENGTH);
			}
		}
	} else if ((message->length == USB_DEVICE_MSC_CSW_LENGTH) &&
		   (USB_DEVICE_MSC_DCSWSIGNATURE == csw->signature)) {
					 ...
					 callbackParam)
{
	...
	if (0U != mscHandle->dataOutFlag) {
		...
		if (0U == mscHandle->transferRemaining) {
	...
	} else if ((0U != mscHandle->cbwValidFlag) && (message->length ==
						       USB_DEVICE_MSC_CBW_LENGTH) &&
		   ...
		   ((mscHandle->mscCbw->cbLength >= 0x01U) && (mscHandle->mscCbw->cbLength <=
							       0x10U))) {
		mscHandle->cbwPrimeFlag = 0U;
		...
		mscHandle->mscCbw->dataTransferLength = USB_LONG_FROM_LITTLE_ENDIAN(
			mscHandle->mscCbw->dataTransferLength);

		mscHandle->dataOutFlag = (uint8_t)(((0U == (mscHandle->mscCbw->flags &
							    USB_DEVICE_MSC_CBW_DIRECTION_BIT)) &&
						    (0U != mscHandle->mscCbw->dataTransferLength)) ?
						   1U :
						   0U);

		mscHandle->dataInFlag = (uint8_t)(((0U != (mscHandle->mscCbw->flags &
							   USB_DEVICE_MSC_CBW_DIRECTION_BIT)) &&
						   (0U != mscHandle->mscCbw->dataTransferLength)) ?
						  1U :
						  0U);
		...
		if (!((0U != mscHandle->dataOutFlag) || ((0U != mscHandle->dataInFlag) || (0U !=
											   mscHandle
											   ->
											   needInStallFlag))))
		{
...
usb_status_t USB_DeviceMscEndpointsInit(usb_device_msc_struct_t *mscHandle)
{
	...
	if ((0U == mscHandle->configuration) ||
	    (mscHandle->configuration >
	     mscHandle->configurationStruct->classInfomation->configurations)) {
		return error;
	}
		...
		if (USB_DEVICE_CONFIG_MSC_CLASS_CODE ==
			...
			for (index = 0; index < interfaceList->interfaces[count].count; index++) {
				if (interfaceList->interfaces[count].interface[index].
				    alternateSetting == mscHandle->alternate) {
					interface =
						&interfaceList->interfaces[count].interface[index];
					break;
				}
			}
			mscHandle->interfaceNumber =
	...
	for (count = 0; count < interface->endpointList.count; count++) {
		...
		if (USB_IN == ((epInitStruct.endpointAddress &
...
usb_status_t USB_DeviceMscEndpointsDeinit(usb_device_msc_struct_t *mscHandle)
{
	...
	for (count = 0; count < mscHandle->interfaceHandle->endpointList.count; count++) {
		status = USB_DeviceDeinitEndpoint(mscHandle->handle,
						  mscHandle->interfaceHandle->endpointList.endpoint[
			       ...
			       class_handle_t *handle)
{
	...
	ufi->requestSense->additionalSenseLength =
		USB_DEVICE_MSC_UFI_REQ_SENSE_ADDITIONAL_SENSE_LEN;
	ufi->requestSense->senseKey = USB_DEVICE_MSC_UFI_NO_SENSE;
	ufi->requestSense->additionalSenseCode = USB_DEVICE_MSC_UFI_NO_SENSE;
	ufi->requestSense->additionalSenseQualifer = USB_DEVICE_MSC_UFI_NO_SENSE;
...
usb_status_t USB_DeviceMscEvent(void *handle, uint32_t event, void *param)
{
	...
	case kUSB_DeviceClassEventDeviceReset:
		...
		error = mscHandle->configurationStruct->classCallback(
			(class_handle_t)mscHandle, kUSB_DeviceMscEventGetLbaInformation,
			(void *)&diskInformation);
		if (0U >= diskInformation.logicalUnitNumberSupported) {
			diskInformation.logicalUnitNumberSupported = 0U;
		} else {
			mscHandle->logicalUnitNumber =
				(uint8_t)diskInformation.logicalUnitNumberSupported - 1U;
		}
		/*add for misra2.2*/
		if (0U == diskInformation.logicalUnitNumberSupported) {
		...
		for (index = 0U; (index <= mscHandle->logicalUnitNumber) && (index <
									     USB_DEVICE_MSC_MAX_LUN);
		     ++index) {
			if (((0U ==
			      diskInformation.logicalUnitInformations[index].lengthOfEachLba) ||
			     (0U ==
			      diskInformation.logicalUnitInformations[index].totalLbaNumberSupports)))
			{
				error = kStatus_USB_Error;
			} else {
				mscHandle->luInformations[index] =
					diskInformation.logicalUnitInformations[index];
			}
		}
		ufi = &mscHandle->mscUfi;
		ufi->requestSense->validErrorCode = USB_DEVICE_MSC_UFI_REQ_SENSE_VALID_ERROR_CODE;
	...
	case kUSB_DeviceClassEventSetEndpointHalt:
		if ((NULL == mscHandle->configurationStruct) || (NULL ==
								 mscHandle->interfaceHandle)) {
			break;
		}
		/* Get the endpoint address */
		temp8 = ((uint8_t *)param);

		if ((mscHandle->inEndpointStallFlag == 0U) && (*temp8 ==
							       mscHandle->bulkInEndpoint)) {
		...
		if ((mscHandle->outEndpointStallFlag == 0U) && (*temp8 ==
	...
	case kUSB_DeviceClassEventClearEndpointHalt:
		...
		if (((mscHandle->stallStatus == (uint8_t)USB_DEVICE_MSC_STALL_IN_CSW) ||
			...
			if (mscHandle->cswPrimeFlag == 1U) {
				...
				(void)USB_DeviceCancel(mscHandle->handle,
						       mscHandle->bulkInEndpoint);
			} else {
				/*send csw*/
				mscHandle->mscCsw->dataResidue = USB_LONG_TO_LITTLE_ENDIAN(
		...
		if ((mscHandle->performResetDoneFlag == 1U) && (mscHandle->inEndpointStallFlag ==
								0U) &&
		    (mscHandle->outEndpointStallFlag == 0U)) {
			mscHandle->performResetDoneFlag = 0U;
			if (mscHandle->cbwPrimeFlag == 1U) {
				...
				(void)USB_DeviceRecvRequest(mscHandle->handle,
							    mscHandle->bulkOutEndpoint,
							    (uint8_t *)mscHandle->mscCbw,
							    USB_DEVICE_MSC_CBW_LENGTH);
				mscHandle->cbwPrimeFlag = 1U;
	...
	case kUSB_DeviceClassEventClassRequest:
	{
		...
		case USB_DEVICE_MSC_GET_MAX_LUN:
			...
			if ((control_request->setup->wIndex == mscHandle->interfaceNumber) &&
			    ...
			    (control_request->setup->wLength == 0x0001U) &&
		...
		case USB_DEVICE_MSC_BULK_ONLY_MASS_STORAGE_RESET:
			...
			if ((control_request->setup->wIndex == mscHandle->interfaceNumber) &&
			    (0U == control_request->setup->wValue) && (0U ==
								       control_request->setup->
								       wLength) &&
			    ((control_request->setup->bmRequestType & USB_REQUEST_TYPE_DIR_MASK) ==
...
usb_status_t USB_DeviceMscSend(usb_device_msc_struct_t *mscHandle)
{
	...
	{
		status = USB_DeviceSendRequest(mscHandle->handle, mscHandle->bulkInEndpoint,
					       lba.buffer, lba.size);
	} else {
		mscHandle->needInStallFlag = 0U;
		mscHandle->inEndpointStallFlag = 1U;
...
usb_status_t USB_DeviceMscRecv(usb_device_msc_struct_t *mscHandle)
{
	...
	if (mscHandle->currentOffset <
	    mscHandle->luInformations[mscHandle->mscCbw->logicalUnitNumber].totalLbaNumberSupports)
	{
		status = USB_DeviceRecvRequest(mscHandle->handle, mscHandle->bulkOutEndpoint,
					       lba.buffer, lba.size);
	} else {
		mscHandle->needOutStallFlag = 0U;
		mscHandle->outEndpointStallFlag = 1U;
