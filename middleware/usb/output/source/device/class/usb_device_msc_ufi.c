...
usb_status_t USB_DeviceMscUfiThirteenCasesCheck(struct _usb_device_msc_struct *mscHandle)
{
	...
	if (mscCheckEvent->hostExpectedDataLength == 0U) {
		...
		if (mscCheckEvent->deviceExpectedDataLength == 0U) {    /*case 1, Device intends to transfer no data*/
			mscHandle->mscCsw->cswStatus = USB_DEVICE_MSC_COMMAND_PASSED;
		} else {                                                /*case 2 ,Device intends to send data to the host; */
		...
		if (mscCheckEvent->deviceExpectedDataLength == 0U) {
			...
			if (kStatus_USB_Success == status) {
				...
				ufi->requestSense->additionalSenseCode =
					USB_DEVICE_MSC_UFI_UNRECOVERED_READ_ERROR;
			}
			status = kStatus_USB_InvalidRequest;
		} else if (0U != mscCheckEvent->deviceExpectedDirection) {
			if (mscCheckEvent->hostExpectedDataLength >
			    mscCheckEvent->deviceExpectedDataLength) {
				...
				if (ufi->thirteenCase.lbaSendRecvSelect == 1U) {
					status = USB_DeviceMscLbaTransfer(mscHandle, USB_IN,
									  &mscCheckEvent->
									  lbaInformation);
				} else {
					status = USB_DeviceSendRequest(mscHandle->handle,
								       mscHandle->bulkInEndpoint,
								       mscCheckEvent->buffer,
								       mscCheckEvent->
								       deviceExpectedDataLength);
				}

				if (kStatus_USB_Success == status) {
					mscHandle->mscCsw->cswStatus =
						USB_DEVICE_MSC_COMMAND_PASSED;
				} else {
					mscHandle->mscCsw->cswStatus =
						USB_DEVICE_MSC_COMMAND_FAILED;
					ufi->requestSense->senseKey =
						USB_DEVICE_MSC_UFI_MEDIUM_ERROR;
					ufi->requestSense->additionalSenseCode =
						USB_DEVICE_MSC_UFI_UNRECOVERED_READ_ERROR;
				}
				status = kStatus_USB_InvalidRequest;
			} else if (mscCheckEvent->hostExpectedDataLength ==
				   mscCheckEvent->deviceExpectedDataLength) { /*case 6*,device intends to send dCBWDataTransferLength excepted by the host*/
				mscHandle->mscCsw->dataResidue = 0;
				if (ufi->thirteenCase.lbaSendRecvSelect == 1U) {
					status = USB_DeviceMscLbaTransfer(mscHandle, USB_IN,
									  &mscCheckEvent->
			...
			} else { /*case 7, device intends to send more data than the host indicated*/
				...
				if (ufi->thirteenCase.lbaSendRecvSelect == 1U) {
					...
					status = USB_DeviceSendRequest(mscHandle->handle,
								       ...
								       mscCheckEvent->buffer,
								       mscCheckEvent->
								       hostExpectedDataLength);
				}

				if (kStatus_USB_Success == status) {
					if (ufi->thirteenCase.lbaSendRecvSelect == 1U) {
						mscHandle->mscCsw->cswStatus =
							USB_DEVICE_MSC_PHASE_ERROR;
					} else {
						mscHandle->mscCsw->cswStatus =
							USB_DEVICE_MSC_COMMAND_PASSED;
					}
			...
			(void)USB_DeviceSendRequest(mscHandle->handle, mscHandle->bulkInEndpoint,
						    mscCheckEvent->buffer, 0);
			mscHandle->mscCsw->cswStatus = USB_DEVICE_MSC_PHASE_ERROR;
			status = kStatus_USB_InvalidRequest;
		}
	} else {
		/*Host expects to send data to the device*/
		if (0U == mscCheckEvent->deviceExpectedDataLength) { /*case 9,Device intends to transfer no data*/
			(void)USB_DeviceStallEndpoint(mscHandle->handle,
						      mscHandle->bulkOutEndpoint);
			mscHandle->mscCsw->dataResidue = mscCheckEvent->hostExpectedDataLength;
			mscHandle->mscCsw->cswStatus = USB_DEVICE_MSC_COMMAND_FAILED;
			mscHandle->outEndpointStallFlag = 1;
			status = kStatus_USB_InvalidRequest;
		} else if (0U != mscCheckEvent->deviceExpectedDirection) { /*case 10,Device intends to send data to the host*/
			(void)USB_DeviceStallEndpoint(mscHandle->handle,
						      mscHandle->bulkOutEndpoint);
			mscHandle->mscCsw->dataResidue = mscCheckEvent->hostExpectedDataLength;
			mscHandle->mscCsw->cswStatus = USB_DEVICE_MSC_PHASE_ERROR;
			...
			if (mscCheckEvent->hostExpectedDataLength >
			    mscCheckEvent->deviceExpectedDataLength) { /*case 11, device intends to process less than the amount of data that the host indicated*/
				mscHandle->mscCsw->dataResidue =
					mscCheckEvent->hostExpectedDataLength -
					mscCheckEvent->deviceExpectedDataLength;

				if (ufi->thirteenCase.lbaSendRecvSelect == 1U) {
					status = USB_DeviceMscLbaTransfer(mscHandle, USB_OUT,
									  &mscCheckEvent->
									  lbaInformation);
				} else {
					status = USB_DeviceRecvRequest(mscHandle->handle,
								       mscHandle->bulkOutEndpoint,
								       mscCheckEvent->buffer,
								       mscCheckEvent->
								       deviceExpectedDataLength);
				...
				if (kStatus_USB_Success == status) {
					...
					ufi->requestSense->senseKey =
						USB_DEVICE_MSC_UFI_MEDIUM_ERROR;
					ufi->requestSense->additionalSenseCode =
						USB_DEVICE_MSC_UFI_WRITE_FAULT;
				}
				status = kStatus_USB_InvalidRequest;
			} else if (mscCheckEvent->hostExpectedDataLength ==
				   mscCheckEvent->deviceExpectedDataLength) { /*case 12,device intends to process equal to the amount of data that the host indicated*/
				mscHandle->mscCsw->dataResidue = 0;
				if (ufi->thirteenCase.lbaSendRecvSelect == 1U) {
					status = USB_DeviceMscLbaTransfer(mscHandle, USB_OUT,
									  &mscCheckEvent->
			...
			} else { /*case 13,device  intends to process more data than the host indicated*/
				mscHandle->mscCsw->dataResidue = 0;
				if (ufi->thirteenCase.lbaSendRecvSelect == 1U) {
					mscCheckEvent->lbaInformation.transferNumber =
						mscCheckEvent->hostExpectedDataLength /
...
usb_status_t USB_DeviceMscUfiRequestSenseCommand(struct _usb_device_msc_struct *mscHandle)
{
	...
	if (mscHandle->configurationStruct->classCallback != NULL) {
		...
		(void)mscHandle->configurationStruct->classCallback((class_handle_t)mscHandle,
								    kUSB_DeviceMscEventRequestSense,
								    (void *)&temp);
	}
...
usb_status_t USB_DeviceMscUfiModeSelectCommand(struct _usb_device_msc_struct *mscHandle)
{
	...
	if (0U != (mscHandle->mscCbw->cbwcb[1] & 0x01U)) {
		...
		ufi->requestSense->additionalSenseCode =
			USB_DEVICE_MSC_UFI_INVALID_FIELD_IN_COMMAND_PKT;
	}
	return status;
}
...
usb_status_t USB_DeviceMscUfiReadCapacityCommand(struct _usb_device_msc_struct *mscHandle)
{
	...
	if (mscHandle->configurationStruct->classCallback != NULL) {
		...
		status = mscHandle->configurationStruct->classCallback(
			(class_handle_t)mscHandle, kUSB_DeviceMscEventReadCapacity,
	...
	if ((0U != diskInformation.lengthOfEachLba) && (0U !=
							diskInformation.totalLbaNumberSupports)) {
		mscHandle->luInformations[logicalUnitNumber].totalLbaNumberSupports =
			diskInformation.totalLbaNumberSupports;
		mscHandle->luInformations[logicalUnitNumber].lengthOfEachLba =
			diskInformation.lengthOfEachLba;
	...
	if (mscHandle->mscCbw->cbwcb[0] == USB_DEVICE_MSC_READ_CAPACITY_10_COMMAND) {
		...
		ufi->readCapacity->lastLogicalBlockAddress = USB_LONG_TO_BIG_ENDIAN(
			mscHandle->luInformations[mscHandle->mscCbw->logicalUnitNumber].
			totalLbaNumberSupports - 1U);
		ufi->readCapacity->blockSize = USB_LONG_TO_BIG_ENDIAN(
			(uint32_t)mscHandle->luInformations[mscHandle->mscCbw->logicalUnitNumber].
			lengthOfEachLba);
...
usb_status_t USB_DeviceMscUfiReadFormatCapacityCommand(struct _usb_device_msc_struct *mscHandle)
{
	...
	if (mscHandle->configurationStruct->classCallback != NULL) {
		 ...
		 * it is from the second parameter of classInit */
		status = mscHandle->configurationStruct->classCallback(
			(class_handle_t)mscHandle, kUSB_DeviceMscEventReadFormatCapacity,
			(void *)&diskInformation);
	}
	...
	allocation_length = (uint16_t)((((uint16_t)mscHandle->mscCbw->cbwcb[7]) << 8) |
				       mscHandle->mscCbw->cbwcb[8]);
	/*reference ufi command spec table-33 Descriptor Code definition*/
	num_formattable_cap_desc =
		(uint8_t)((0U != ufi->formattedDisk) ? ((0U != mscHandle->implementingDiskDrive) ?
							0x02U : 0x03U) : 0x00U);

	formattable_capacity_head.blockNumber =
		USB_LONG_TO_BIG_ENDIAN(
			mscHandle->luInformations[mscHandle->mscCbw->logicalUnitNumber].
			totalLbaNumberSupports);
	formattable_capacity_head.blockLength =
		USB_LONG_TO_BIG_ENDIAN(
			mscHandle->luInformations[mscHandle->mscCbw->logicalUnitNumber].
			lengthOfEachLba);

	descriptor_code = (uint8_t)((0U != ufi->formattedDisk) ?
				    USB_DEVICE_MSC_UFI_FORMATTED_MEDIA :
	...
	current_max_head.descriptorCodeBlockLength =
		USB_LONG_TO_BIG_ENDIAN(((((uint32_t)descriptor_code) << 24) |
					mscHandle->luInformations[mscHandle->mscCbw->
								  logicalUnitNumber].lengthOfEachLba));
	...
	response_size = (uint32_t)sizeof(usb_device_capacity_list_header_struct_t) +
			...
			sizeof(usb_device_formattable_capacity_descriptor_struct_t) *
			((uint32_t)num_formattable_cap_desc);
