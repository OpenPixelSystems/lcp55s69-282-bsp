...
#define USB_HOST_OHCI_CONTROL_HCFS(n) (((n) << USB_HOST_OHCI_CONTROL_HCFS_SHIFT) & \
				       USB_HOST_OHCI_CONTROL_HCFS_MASK)
...
#define USB_HOST_OHCI_RHDESCRIPTORA_POTPGT(n) \
	(((n) << USB_HOST_OHCI_RHDESCRIPTORA_POTPGT_SHIFT) & \
	 USB_HOST_OHCI_RHDESCRIPTORA_POTPGT_MASK)
...
#endif
	usb_host_ohci_endpoint_descritpor_struct_t		ed[USB_HOST_CONFIG_OHCI_MAX_ED +
								   3U];
...
typedef struct _usb_host_ohci_state_struct {
	...
	uint32_t							taskEventHandleBuffer[(
												      OSA_EVENT_HANDLE_SIZE
												      +
												      3)
											      / 4];     /*!< task event handle buffer*/
	osa_mutex_handle_t						mutex;                          /*!< OHCI layer mutex*/
	uint32_t							mutexBuffer[(
											    OSA_MUTEX_HANDLE_SIZE
											    + 3) /
										    4];
	usb_host_ohci_pipe_struct_t					pipePool[
 ...
 */
extern usb_status_t USB_HostOhciCreate(uint8_t controllerId, usb_host_handle hostHandle,
 ...
 * @retval kStatus_USB_Success              The host is initialized successfully.
 */
extern usb_status_t USB_HostOhciClosePipe(usb_host_controller_handle	controllerHandle,
					  usb_host_pipe_handle		pipeHandle);
 ...
 */
extern usb_status_t USB_HostOhciIoctl(usb_host_controller_handle controllerHandle, uint32_t
