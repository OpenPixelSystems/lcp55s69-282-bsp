 ...
 */
static void USB_HostEnumerationTransferCallback(void *param, usb_host_transfer_t *transfer,
						usb_status_t status);
 ...
 * @return kStatus_USB_Success or error codes
 */
static usb_status_t USB_HostProcessCallback(usb_host_device_instance_t *deviceInstance, uint32_t
					    dataLength);
...
static usb_status_t USB_HostNotifyDevice(usb_host_handle hostHandle,
					 usb_host_device_instance_t *deviceInstance, uint32_t
					 eventCode, uint32_t eventParameter);
 ...
 * @return kStatus_USB_Success or error codes.
 */
static usb_status_t USB_HostRemoveDeviceInstance(usb_host_handle hostHandle, usb_device_handle
						 deviceHandle);
						...
						usb_status_t status)
{
	...
	if (nextStep == 1U) {
		...
		if (s_EnumEntries[deviceInstance->state - 1U].process == NULL) {
			deviceInstance->state = (uint8_t)s_EnumEntries[deviceInstance->state -
								       1U].successState;             /* next state */
		} else {
			...
			if (status == kStatus_USB_Success) {    /* process success */
				deviceInstance->state =
					(uint8_t)s_EnumEntries[deviceInstance->state -
							       1U].successState;
	...
	if (nextStep == 2U) {
		if (deviceInstance->enumRetries > 0U) { /* next whole retry */
			...
			deviceInstance->stallRetries =
				USB_HOST_CONFIG_ENUMERATION_MAX_STALL_RETRIES;
	...
	if (nextStep == 3U) {
		(void)USB_HostNotifyDevice(deviceInstance->hostHandle, deviceInstance,
					   (uint32_t)kUSB_HostEventEnumerationFail,
					   (uint32_t)failReason);
	}
}
...
static usb_status_t USB_HostProcessState(usb_host_device_instance_t *deviceInstance)
{
	...
	if (state != kStatus_DEV_EnumDone) {
		if (USB_HostMallocTransfer(deviceInstance->hostHandle, &transfer) !=
		    kStatus_USB_Success) {
#ifdef HOST_ECHO
	...
	case kStatus_DEV_GetDes: /* get descriptor state */
		...
		status = USB_HostStandardSetGetDescriptor(deviceInstance, transfer,
							  &getDescriptorParam);
		break;
	case kStatus_DEV_SetAddress: /* set address state */
		transfer->setupPacket->bRequest = USB_REQUEST_STANDARD_SET_ADDRESS;
		status = USB_HostStandardSetAddress(deviceInstance, transfer,
	...
	case kStatus_DEV_SetCfg: /* set configuration state */
		transfer->setupPacket->wValue =
			USB_SHORT_TO_LITTLE_ENDIAN(
				deviceInstance->configuration.configurationDesc->bConfigurationValue);
	...
	case kStatus_DEV_EnumDone: /* enumeration done state */
		...
		status = USB_HostNotifyDevice(deviceInstance->hostHandle, deviceInstance,
					      (uint32_t)kUSB_HostEventEnumerationDone,
					    ...
					    dataLength)
{
	...
	case kStatus_DEV_GetDes8: /* process get 8 bytes descriptor result */
		...
		(void)hostInstance->controllerTable->controllerIoctl(
			hostInstance->controllerHandle, kUSB_HostUpdateControlPacketSize,
			deviceInstance->controlPipe);
		break;
	...
	case kStatus_DEV_SetAddress: /* process set address result */
		...
		/* the callbackFn is initialized in USB_HostGetControllerInterface */
		(void)hostInstance->controllerTable->controllerIoctl(
			hostInstance->controllerHandle, kUSB_HostUpdateControlEndpointAddress,
			deviceInstance->controlPipe);
		break;
	...
	case kStatus_DEV_GetCfg9: /* process get 9 bytes configuration result */
		...
		deviceInstance->configurationLen = USB_SHORT_FROM_LITTLE_ENDIAN_ADDRESS(
			configureDesc->wTotalLength);
		if (deviceInstance->configurationDesc != NULL) {
#if ((defined(USB_HOST_CONFIG_BUFFER_PROPERTY_CACHEABLE)) && \
			(USB_HOST_CONFIG_BUFFER_PROPERTY_CACHEABLE > 0U))
		...
		/* fix misra 4.14 */
		if (deviceInstance->configurationLen >
		    USB_HOST_CONFIG_MAX_CONFIGURATION_DESCRIPTOR_LENGTH) {
			return kStatus_USB_Error;
		}
		...
		if ((deviceInstance->configurationLen & 0x03U) != 0U) {
			...
			deviceInstance->configurationDesc =
				(uint8_t *)SDK_Malloc((deviceInstance->configurationLen & 0xFFFCu) +
						      4, USB_CACHE_LINESIZE);
#else
			deviceInstance->configurationDesc =
				(uint8_t *)OSA_MemoryAllocate(
					(((uint32_t)deviceInstance->configurationLen) & 0xFFFCU) +
					4UL);
#endif
		} else {
#if ((defined(USB_HOST_CONFIG_BUFFER_PROPERTY_CACHEABLE)) && \
			(USB_HOST_CONFIG_BUFFER_PROPERTY_CACHEABLE > 0U))
			deviceInstance->configurationDesc =
				(uint8_t *)SDK_Malloc(deviceInstance->configurationLen,
						      USB_CACHE_LINESIZE);
#else
			deviceInstance->configurationDesc = (uint8_t *)OSA_MemoryAllocate(
				deviceInstance->configurationLen);
#endif
		}
		if (deviceInstance->configurationDesc == NULL) {
	...
	case kStatus_DEV_GetCfg: /* process get configuration result */
		...
		if (((usb_descriptor_configuration_t *)temp)->bMaxPower >
		...
		status = USB_HostNotifyDevice(deviceInstance->hostHandle, deviceInstance,
					      (uint32_t)kUSB_HostEventAttach,
					      (uint32_t)kStatus_USB_Success);

		if (status != kStatus_USB_Success) {
			/* next configuration */
			if (deviceInstance->configurationValue <
			    deviceInstance->deviceDescriptor->bNumConfigurations) {
				return kStatus_USB_Retry;
			} else {
				/* notify application device is not supported */
				(void)USB_HostNotifyDevice(deviceInstance->hostHandle,
							   deviceInstance,
							   (uint32_t)kUSB_HostEventNotSupported,
							   (uint32_t)kStatus_USB_Success);
					 ...
					 uint32_t			eventParameter)
{
	...
	for (interfaceIndex = 0U; interfaceIndex < deviceInstance->configuration.interfaceCount;
	     ++interfaceIndex) {
		if (((usb_descriptor_interface_t *)deviceInstance->configuration.interfaceList[
			     interfaceIndex].interfaceDesc)
	...
	if ((hostInstance->deviceCallback != NULL) &&
		...
		status1 = hostInstance->deviceCallback(deviceInstance,
						       &deviceInstance->configuration, eventCode);
	}
	if (0U != haveHub) {
		status2 = USB_HostHubDeviceEvent(hostInstance, deviceInstance,
						 &deviceInstance->configuration,
...
static uint8_t USB_HostAllocateDeviceAddress(usb_host_instance_t *hostInstance)
{
	...
	if (addressIndex < 8U) {
		for (addressBitIndex = 0U; addressBitIndex < 8U; ++addressBitIndex) { /* find the idle address position bit */
			if (0U == (hostInstance->addressBitMap[addressIndex] & (0x01U <<
										addressBitIndex))) {
				hostInstance->addressBitMap[addressIndex] |= (0x01U <<
									      addressBitIndex); /* set the allocated bit */
					  ...
					  usb_host_device_instance_t *	deviceInstance)
{
	...
	if (deviceInstance->setAddress != 0U) {
		...
		if (deviceInstance->allocatedAddress != 0U) {
			USB_HostReleaseDeviceAddress(hostInstance,
						     deviceInstance->allocatedAddress);
		}
	}
	...
	if (deviceInstance->controlPipe != NULL) {
		...
		if (USB_HostClosePipe(hostInstance, deviceInstance->controlPipe) !=
		    kStatus_USB_Success) {
#ifdef HOST_ECHO
...
static usb_status_t USB_HostParseDeviceConfigurationDescriptor(usb_device_handle deviceHandle)
{
	...
	if ((unionDes->common.bLength == USB_DESCRIPTOR_LENGTH_CONFIGURE) &&
		...
		temp = (void *)unionDes;
		deviceInstance->configuration.configurationDesc =
			(usb_descriptor_configuration_t *)temp;
		...
		unionDes = (usb_descriptor_union_t *)((uint32_t)unionDes +
						      unionDes->common.bLength);
		while ((uint32_t)unionDes < endPos) {
			if (unionDes->common.bDescriptorType != USB_DESCRIPTOR_TYPE_INTERFACE) {
				if (deviceInstance->configuration.configurationExtension == NULL) {
					deviceInstance->configuration.configurationExtension =
						(uint8_t *)unionDes;
				}
				...
				deviceInstance->configuration.configurationExtensionLength +=
					unionDes->common.bLength;
				unionDes = (usb_descriptor_union_t *)((uint32_t)unionDes +
								      unionDes->common.bLength);
			} else {
		...
		while ((uint32_t)unionDes < endPos) {
			if (unionDes->common.bDescriptorType == USB_DESCRIPTOR_TYPE_INTERFACE) {
				if (unionDes->interface.bAlternateSetting == 0x00U) {
					if (deviceInstance->configuration.interfaceCount >=
					...
					interfaceParse =
						&deviceInstance->configuration.interfaceList[
					...
					interfaceParse->interfaceIndex =
						unionDes->interface.bInterfaceNumber;
					...
					unionDes = (usb_descriptor_union_t *)((uint32_t)unionDes +
									      ...
									      bLength);
					while ((uint32_t)unionDes < endPos) {
						if ((unionDes->common.bDescriptorType !=
						     USB_DESCRIPTOR_TYPE_INTERFACE) &&
						    (unionDes->common.bDescriptorType !=
						     USB_DESCRIPTOR_TYPE_ENDPOINT)) {
							if (interfaceParse->interfaceExtension ==
							    NULL) {
								interfaceParse->interfaceExtension =
									(uint8_t *)unionDes;
							}
							if ((unionDes->common.bDescriptorType ==
							     0x00U) ||
							...
							interfaceParse->interfaceExtensionLength +=
								unionDes->common.bLength;
							unionDes =
								(usb_descriptor_union_t *)((uint32_t)
											   unionDes
											   +
											   unionDes
											   ->common.
					...
					if (interfaceParse->interfaceDesc->bNumEndpoints != 0U) {
						if ((unionDes->common.bDescriptorType !=
						     USB_DESCRIPTOR_TYPE_ENDPOINT) ||
						    (interfaceParse->interfaceDesc->bNumEndpoints >
						     USB_HOST_CONFIG_INTERFACE_MAX_EP)) {
#ifdef HOST_ECHO
							usb_echo("interface descriptor error\n");
#endif
							return kStatus_USB_Error;
						}
						for (; interfaceParse->epCount <
						     interfaceParse->interfaceDesc->bNumEndpoints;
...
#ifdef HOST_ECHO
								usb_echo(
									"endpoint descriptor error\n");
#endif
								return kStatus_USB_Error;
							}
							temp =
								(void *)&interfaceParse->epList[
									interfaceParse
									->
									epCount];
							...
							temp = (void *)unionDes;
							epParse->epDesc =
								(usb_descriptor_endpoint_t *)temp;
							...
							while ((uint32_t)unionDes < endPos) {
								     ...
								     USB_DESCRIPTOR_TYPE_INTERFACE))
								{
									...
									if ((unionDes->common.
									     ...
									     0x00U) ||
									    (unionDes->common.
									     bLength == 0x00U)) { /* the descriptor data is wrong */
										return
											kStatus_USB_Error;
									}
									epParse->epExtensionLength
										+= unionDes->common.
										   bLength;
									unionDes =
										(
...
/* check whether is the device attached? */
	...
	while (currentInstance != NULL) {
		if ((currentInstance->hubNumber == hubNumber) && (currentInstance->portNumber ==
								  portNumber)) {
			*deviceHandle = NULL;
#ifdef HOST_ECHO
	...
	newInstance = (usb_host_device_instance_t *)OSA_MemoryAllocate(
		sizeof(usb_host_device_instance_t));
	if (newInstance == NULL) {
#ifdef HOST_ECHO
		usb_echo("allocate dev instance fail\r\n");
#endif
		(void)USB_HostNotifyDevice(hostInstance, NULL,
					   (uint32_t)kUSB_HostEventEnumerationFail,
	...
	newInstance->deviceDescriptor =
		(usb_descriptor_device_t *)SDK_Malloc(sizeof(usb_descriptor_device_t) + 9,
						      USB_CACHE_LINESIZE);
#else
	newInstance->deviceDescriptor = (usb_descriptor_device_t *)OSA_MemoryAllocate(
		sizeof(usb_descriptor_device_t) + 9U);
#endif
	...
	newInstance->enumBuffer = (uint8_t *)((uint8_t *)newInstance->deviceDescriptor +
	...
	if ((speed != USB_SPEED_HIGH) && (level > 1U)) {
		newInstance->hsHubNumber = (uint8_t)USB_HostHubGetHsHubNumber(hostHandle,
									      hubNumber);
		newInstance->hsHubPort = (uint8_t)USB_HostHubGetHsHubPort(hostHandle, hubNumber,
									  portNumber);
	} else {
	...
	if (USB_HostOpenPipe(hostHandle, &newInstance->controlPipe, &pipeInit) !=
	    kStatus_USB_Success) {
	...
	if (USB_HostProcessState(newInstance) != kStatus_USB_Success) {
		(void)USB_HostNotifyDevice(hostInstance, newInstance,
					   (uint32_t)kUSB_HostEventEnumerationFail,
					   (uint32_t)kStatus_USB_Error);
	}
...
/* search for device instance handle */
	...
	while (deviceInstance != NULL) {
		if ((deviceInstance->hubNumber == hubNumber) && (deviceInstance->portNumber ==
								 portNumber)) {
			break;
		}
...
usb_status_t USB_HostDetachDeviceInternal(usb_host_handle hostHandle, usb_device_handle
					  deviceHandle)
{
	usb_host_device_instance_t *deviceInstance = (usb_host_device_instance_t *)deviceHandle;
	usb_host_instance_t *hostInstance = (usb_host_instance_t *)hostHandle;
	...
	if (deviceInstance->state >= (uint8_t)kStatus_DEV_Initial) {            /* device instance is valid */
		...
		if (deviceInstance->state < (uint8_t)kStatus_DEV_AppUsed) {     /* enumeration is not done */
			if (deviceInstance->controlPipe != NULL) {
				(void)USB_HostCancelTransfer(hostInstance,
							     deviceInstance->controlPipe, NULL);
			}
			...
			(void)USB_HostNotifyDevice(hostInstance, deviceInstance,
						   (uint32_t)kUSB_HostEventDetach,
						   (uint32_t)kStatus_USB_Success);
		}
	}
...
{
	return (NULL != deviceHandle) ?
	       ((usb_host_device_instance_t *)deviceHandle)->deviceAttachState : 0x0U;
}
...
usb_status_t USB_HostOpenDeviceInterface(usb_device_handle deviceHandle, usb_host_interface_handle
					 interfaceHandle)
{
	usb_host_device_instance_t *deviceInstance = (usb_host_device_instance_t *)deviceHandle;
	...
	for (interfaceIndex = 0; interfaceIndex < deviceInstance->configuration.interfaceCount;
	     ++interfaceIndex) {
		if (&deviceInstance->configuration.interfaceList[interfaceIndex] ==
		    interfaceHandle) {
			deviceInstance->interfaceStatus[interfaceIndex] =
				(uint8_t)kStatus_interface_Opened;
			break;
		}
	}
	(void)USB_HostUnlock();
...
usb_status_t USB_HostCloseDeviceInterface(usb_device_handle deviceHandle, usb_host_interface_handle
					  interfaceHandle)
{
	usb_host_device_instance_t *deviceInstance = (usb_host_device_instance_t *)deviceHandle;
	usb_host_instance_t *hostInstance = NULL;
	...
	if (interfaceHandle != NULL) {
		...
		for (interfaceIndex = 0; interfaceIndex <
		     deviceInstance->configuration.interfaceCount; ++interfaceIndex) {
			if (&deviceInstance->configuration.interfaceList[interfaceIndex] ==
			    interfaceHandle) {
				deviceInstance->interfaceStatus[interfaceIndex] =
					(uint8_t)kStatus_interface_Detached;
				break;
			}
		}
	}
	...
	if (deviceInstance->deviceAttachState == (uint8_t)kStatus_device_Detached) { /* device is removed from host */
		...
		for (interfaceIndex = 0; interfaceIndex <
		     deviceInstance->configuration.interfaceCount; ++interfaceIndex) {
			if (deviceInstance->interfaceStatus[interfaceIndex] ==
			    (uint8_t)kStatus_interface_Opened) {
				removeLabel = 0U;
				break;
			}
	...
	if (USB_HostValidateDevice(hostInstance, deviceInstance) == kStatus_USB_Success) { /* device is valid */
		...
		if (deviceInstance->state >= (uint8_t)kStatus_DEV_Initial) {    /* device is valid */
			if (deviceInstance->state <
				...
				for (interfaceIndex = 0U; interfaceIndex <
				     deviceInstance->configuration.interfaceCount;
				     ++interfaceIndex) {
					if (deviceInstance->interfaceStatus[interfaceIndex] ==
					    (uint8_t)kStatus_interface_Opened) {
#ifdef HOST_ECHO
						usb_echo(
							"error: there is class instance that is not deinited\r\n");
#endif
						break;
					}
				}
