...
static usb_audio_request_t s_usbAudioFuRequests[NUMBER_OF_FEATURE_COMMANDS] = {
	...
	{ AUDIO_FU_MUTE_MASK,	ITF_REQUEST,   CUR_REQUEST,   AUDIO_FU_MUTE,
	  1U },
	/* Volume command request  */
	{ AUDIO_FU_VOLUME_MASK, ITF_REQUEST,   CUR_REQUEST,   AUDIO_FU_VOLUME,
	  2U },
	{ AUDIO_FU_VOLUME_MASK, ITF_REQUEST,   MIN_REQUEST,   AUDIO_FU_VOLUME,
	  2U },
	{ AUDIO_FU_VOLUME_MASK, ITF_REQUEST,   MAX_REQUEST,   AUDIO_FU_VOLUME,
	  2U },
	{ AUDIO_FU_VOLUME_MASK, ITF_REQUEST,   RES_REQUEST,   AUDIO_FU_VOLUME,
	  2U },
};
...
static usb_audio_request_t s_usbAudioEpRequests[NUMBER_OF_ENDPOINT_COMMANDS] = {
	...
	{ AUDIO_PITCH_MASK,	    EP_REQUEST,		CUR_REQUEST,	     AUDIO_PITCH_CONTROL,
	  1U },

	/* USB audio Sampling frequency command request  */
	{ AUDIO_SAMPLING_FREQ_MASK, EP_REQUEST,		CUR_REQUEST,
	  AUDIO_SAMPLING_FREQ_CONTROL, 3U },
	{ AUDIO_SAMPLING_FREQ_MASK, EP_REQUEST,		MIN_REQUEST,
	  AUDIO_SAMPLING_FREQ_CONTROL, 3U },
	{ AUDIO_SAMPLING_FREQ_MASK, EP_REQUEST,		MAX_REQUEST,
	  AUDIO_SAMPLING_FREQ_CONTROL, 3U },
	{ AUDIO_SAMPLING_FREQ_MASK, EP_REQUEST,		RES_REQUEST,
	  AUDIO_SAMPLING_FREQ_CONTROL, 3U },
};
 ...
 */
usb_status_t USB_HostAudioInit(usb_device_handle	deviceHandle,
			       usb_host_class_handle *	classHandlePtr)
{
	audio_instance_t *audioPtr = (audio_instance_t *)OSA_MemoryAllocate(
		sizeof(audio_instance_t));
	uint32_t info_value;
	uint32_t *temp;
...
usb_status_t USB_HostAudioDeinit(usb_device_handle deviceHandle, usb_host_class_handle classHandle)
{
	...
	if (classHandle != NULL) {
		if (audioPtr->isoInPipe != NULL) {
			status = USB_HostCancelTransfer(audioPtr->hostHandle, audioPtr->isoInPipe,
							NULL);
		...
		if (audioPtr->isoOutPipe != NULL) {
			status = USB_HostCancelTransfer(audioPtr->hostHandle, audioPtr->isoOutPipe,
							NULL);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("error when cancel pipe\r\n");
		...
		if ((audioPtr->controlPipe != NULL) && (audioPtr->controlTransfer != NULL)) {
			status = USB_HostCancelTransfer(audioPtr->hostHandle, audioPtr->controlPipe,
							audioPtr->controlTransfer);
		}
 ...
 */
static void _USB_HostAudioStreamIsoInPipeCallback(void *param, usb_host_transfer_t *transfer,
						  usb_status_t status)
{
	...
	if (audioPtr->inCallbackFn != NULL) {
		...
		audioPtr->inCallbackFn(audioPtr->inCallbackParam, transfer->transferBuffer,
 ...
 * @param status        transfer status.
 */
static void _USB_HostAudioStreamIsoOutPipeCallback(void *param, usb_host_transfer_t *transfer,
						   usb_status_t status)
{
	audio_instance_t *audioPtr = (audio_instance_t *)param;
	...
	if (audioPtr->outCallbackFn != NULL) {
		...
		audioPtr->outCallbackFn(audioPtr->outCallbackParam, transfer->transferBuffer,
					transfer->transferSofar, status);
	}
	(void)USB_HostFreeTransfer(audioPtr->hostHandle, transfer);
}
 ...
 * @param status        transfer status.
 */
static void _USB_HostAudioControlCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					  status)
{
	audio_instance_t *audioPtr = (audio_instance_t *)param;
	...
	if (audioPtr->controlCallbackFn != NULL) {
		...
		audioPtr->controlCallbackFn(audioPtr->controlCallbackParam,
					  ...
					  void *		callbackParam)
{
	...
	if (USB_HostSendSetup(audioPtr->hostHandle, audioPtr->controlPipe, transfer) !=
	    kStatus_USB_Success) {
#ifdef HOST_ECHO
...
static usb_status_t _USB_HostAudioInitEndpoint(audio_instance_t *		audioPtr,
					       usb_descriptor_endpoint_t *	ep_desc)
{
	...
	pipe_init.direction = ((ep_desc->bEndpointAddress &
				USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) >>
			       USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT);
	pipe_init.endpointAddress = (ep_desc->bEndpointAddress &
				     USB_DESCRIPTOR_ENDPOINT_ADDRESS_NUMBER_MASK);
	pipe_init.interval = ep_desc->bInterval;
	pipe_init.maxPacketSize = (uint16_t)((USB_SHORT_FROM_LITTLE_ENDIAN_ADDRESS(
...
#ifdef HOST_ECHO
				usb_echo(
					"usb_host_audio_stream_set_interface fail to open pipe\r\n");
#endif
				return kStatus_USB_Error;
			}
 ...
 * @param status      transfer status.
 */
static void _USB_HostAudioSetInterfaceCallback(void *param, usb_host_transfer_t *transfer,
					       usb_status_t status)
{
	audio_instance_t *audioPtr = (audio_instance_t *)param;
					     ...
					     void *			callbackParam)
{
	...
	while (length < interface_ptr->interfaceExtensionLength) {
		if ((ptr1.common->bDescriptorType == 0x04U) && (ptr1.interface->bAlternateSetting ==
								alternateSetting)) {
	...
	while (ep < interface_ptr->epCount) {
		...
		if (ptr1.common->bDescriptorType == 0x05U) {
			...
			if (0x00U == ((endpointDesc->bmAttributes >> 4U) & 0x3U)) {
				interface_ptr->epList[ep].epDesc =
	...
	if (alternateSetting == 0U) {
		...
		if (USB_HostMallocTransfer(audioPtr->hostHandle, &transfer) !=
		    kStatus_USB_Success) {
#ifdef HOST_ECHO
			usb_echo("error to get transfer\r\n");
#endif
		...
		transfer->setupPacket->wIndex = USB_SHORT_TO_LITTLE_ENDIAN(
			((usb_host_interface_t *)audioPtr->streamIntfHandle)->interfaceDesc->
			bInterfaceNumber);
					      ...
					      void *			callbackParam)
{
	...
	while (length < interface_ptr->interfaceExtensionLength) {
		if (((interface_ptr->interfaceDesc->bDescriptorType == 0x04U) &&
		    ...
		    ((ptr1.common->bDescriptorType == 0x04U) &&
		     (ptr1.interface->bAlternateSetting == alternateSetting))) {
			break;
		}
	...
	while (length < interface_ptr->interfaceExtensionLength) {
		if (ptr1.common->bDescriptorType == 0x24U) {
			...
			if (ptr1.common->bData[0] == USB_AUDIO_DESC_SUBTYPE_CS_HEADER) {
				...
				audioPtr->deviceAudioVersion = USB_SHORT_FROM_LITTLE_ENDIAN_ADDRESS(
					((uint8_t *)header));
	...
	if (alternateSetting == 0U) {
		...
		transfer->setupPacket->bmRequestType = USB_REQUEST_TYPE_RECIPIENT_INTERFACE;
		transfer->setupPacket->wIndex = USB_SHORT_TO_LITTLE_ENDIAN(
			((usb_host_interface_t *)audioPtr->controlIntfHandle)->interfaceDesc->
			bInterfaceNumber);
		transfer->setupPacket->wValue = USB_SHORT_TO_LITTLE_ENDIAN(alternateSetting);
		transfer->setupPacket->wLength = 0;
		transfer->transferBuffer = NULL;
 ...
 */
uint16_t USB_HostAudioPacketSize(usb_host_class_handle classHandle, uint8_t pipeType, uint8_t
				 direction)
{
	audio_instance_t *audioPtr = (audio_instance_t *)classHandle;
	if (classHandle == NULL) {
				     ...
				     void *			callbackParam)
{
	...
	if (USB_HostRecv(audioPtr->hostHandle, audioPtr->isoInPipe, transfer) !=
	    kStatus_USB_Success) {
#ifdef HOST_ECHO
				     ...
				     void *			callbackParam)
{
	...
	if (USB_HostSend(audioPtr->hostHandle, audioPtr->isoOutPipe, transfer) !=
	    kStatus_USB_Success) {
#ifdef HOST_ECHO
...
usb_status_t USB_HostAudioControlGetCurrentAltsettingSpecificDescriptors(
	...
	usb_host_class_handle classHandle, uint32_t DescriptorType, uint32_t DescriptorSubType,
	void **Descriptor)
{
	audio_instance_t *audioPtr = (audio_instance_t *)classHandle;
						   ...
						   void *			callbackParam)
{
	...
	if (AUDIO_DEVICE_VERSION_01 == if_ptr->deviceAudioVersion) {
		windex = (uint16_t)((uint16_t)((uint16_t)(fuDesc->fuDesc10.bunitid) << 8U) |
				    (if_ptr->streamIfnum));
		atribute_index = (uint8_t)(fuDesc->fuDesc10.bcontrolsize * ((uint16_t)csAndCn &
									    0xFFU));
		...
		windex = (uint16_t)((uint16_t)((uint16_t)(fuDesc->fuDesc20.bunitid) << 8U) |
				    (if_ptr->streamIfnum));
	...
	if (AUDIO_DEVICE_VERSION_01 == if_ptr->deviceAudioVersion) {
		...
		if ((request != USB_AUDIO_CS_REQUEST_CODE_RANGE_20) && (request !=
									USB_AUDIO_CS_REQUEST_CODE_CUR_20))
		{
			return kStatus_USB_Error;
		}
		switch (controlSelector) {
	...
	status = _USB_HostAudioControl(classHandle, typeRequest, request, (uint16_t)csAndCn, windex,
				       length, (uint8_t *)buf,
						   ...
						   void *			callbackParam)
{
	...
	if (AUDIO_DEVICE_VERSION_02 == if_ptr->deviceAudioVersion) {
		...
		windex = (uint16_t)((uint16_t)((uint16_t)(clockDesc->bClockID) << 8U) |
				    (if_ptr->streamIfnum));
	} else {
		return kStatus_USB_Error;
	}
...
{ /* Body */
	...
	/* get request value */
	request_value = (uint16_t)((uint16_t)((uint16_t)p_feature_request->requestValue << 8U) |
				   channelNo);
	...
	status = _USB_HostAudioControl(classHandle, (p_feature_request->typeRequest |
						     ((uint8_t)cmdCode & 0x80U)),
				       (p_feature_request->codeRequest | ((uint8_t)cmdCode &
									  0x80U)), request_value,
				       windex,
				       p_feature_request->length, (uint8_t *)buf, callbackFn,
				       callbackParam);
...
usb_status_t USB_HostAudioEndpointRequest(
	usb_host_class_handle classHandle, void *buf, uint32_t cmdCode, transfer_callback_t
	callbackFn, void *callbackParam)
{
	...
	if ((0U != audioPtr->isoEndpDesc->bmAttributes) && (0U !=
							    p_endpoint_request->controlMask)) {
		status = kStatus_USB_Success;
	} else {
	...
	if (kStatus_USB_Success == status) {
		...
		status = _USB_HostAudioControl(classHandle, (p_endpoint_request->typeRequest |
							     ((uint8_t)cmdCode & 0x80U)),
					       (p_endpoint_request->codeRequest |
						((uint8_t)cmdCode & 0x80U)), request_value,
					       endp_num, p_endpoint_request->length, (uint8_t *)buf,
					       callbackFn, callbackParam);
	}
...
usb_status_t USB_HostAudioSetStreamOutDataInterval(usb_host_class_handle classHandle, uint8_t
						   intervalValue)
{
	audio_instance_t *audioPtr = (audio_instance_t *)classHandle;
	usb_status_t status = kStatus_USB_Error;
