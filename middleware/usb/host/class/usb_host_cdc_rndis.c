				  ...
				  void *		callbackParam)
{
	...
	return USB_HostCdcSendEncapsulatedCommand(classHandle, messageBuffer,
						  (uint16_t)message->messageLength, callbackFn,
						  callbackParam);
}
				   ...
				   void *			callbackParam)
{
	...
	if (0U != informationLength) {
		if (informationLength > (messageBufferLength - RNDIS_QUERY_MSG_SIZE)) {
			...
			message->informationBufferLength = (messageBufferLength -
							    RNDIS_QUERY_MSG_SIZE);
		}
		/*fixed misra 18.1*/
				      ...
				      void *			callbackParam)
{
	...
	return USB_HostCdcDataSend(classHandle, messageBuffer, message->messageLength, callbackFn,
				   callbackParam);
}
