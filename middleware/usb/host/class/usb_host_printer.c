 ...
 */
static void USB_HostPrinterInPipeCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					  status);
 ...
 * @param status      transfer status.
 */
static void USB_HostPrinterOutPipeCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					   status);
 ...
 * @param status      transfer status.
 */
static void USB_HostPrinterControlPipeCallback(void *param, usb_host_transfer_t *transfer,
					       usb_status_t status);
 ...
 * @param status      transfer status.
 */
static void USB_HostPrinterSetInterfaceCallback(void *param, usb_host_transfer_t *transfer,
						usb_status_t status);
 ...
 */
static usb_status_t USB_HostPrinterControl(usb_host_printer_instance_t *printerInstance, uint8_t
					   requestType, uint8_t request, uint16_t wvalue, uint16_t
					   windex, uint16_t wlength, uint8_t *data,
...
static void USB_HostPrinterClearInHaltCallback(void *param, usb_host_transfer_t *transfer,
					       usb_status_t status)
{
	...
	if (printerInstance->inCallbackFn != NULL) {
		...
		printerInstance->inCallbackFn(printerInstance->outCallbackParam,
					      printerInstance->stallDataBuffer,
					      printerInstance->stallDataLength,
					      kStatus_USB_TransferStall);
	}
	(void)USB_HostFreeTransfer(printerInstance->hostHandle, transfer);
}

static void USB_HostPrinterClearOutHaltCallback(void *param, usb_host_transfer_t *transfer,
						usb_status_t status)
{
	usb_host_printer_instance_t *printerInstance = (usb_host_printer_instance_t *)param;
	...
	if (printerInstance->outCallbackFn != NULL) {
		...
		printerInstance->outCallbackFn(printerInstance->outCallbackParam,
					       printerInstance->stallDataBuffer,
					       printerInstance->stallDataLength,
					       kStatus_USB_TransferStall);
	}
					     ...
					     uint8_t				endpoint)
{
	...
	status = USB_HostSendSetup(printerInstance->hostHandle, printerInstance->controlPipe,
				   transfer);
					  ...
					  status)
{
	...
	if (status == kStatus_USB_TransferStall) {
		if (USB_HostPrinterClearHalt(
			    ...
			    (USB_REQUEST_TYPE_DIR_IN |
			     ((usb_host_pipe_t *)printerInstance->inPipe)->endpointAddress)) ==
		    kStatus_USB_Success) {
			(void)USB_HostFreeTransfer(printerInstance->hostHandle, transfer);
			return;
		}
	...
	if (printerInstance->inCallbackFn != NULL) {
		/* callback to application, callback function is initialized in USB_HostPrinterRecv */
		printerInstance->inCallbackFn(printerInstance->inCallbackParam,
	...
	if (status == kStatus_USB_TransferStall) {
		if (USB_HostPrinterClearHalt(
			    ...
			    (USB_REQUEST_TYPE_DIR_OUT |
			     ((usb_host_pipe_t *)printerInstance->outPipe)->endpointAddress)) ==
		    kStatus_USB_Success) {
			(void)USB_HostFreeTransfer(printerInstance->hostHandle, transfer);
			return;
		}
					       ...
					       usb_status_t status)
{
	...
	if (printerInstance->controlCallbackFn != NULL) {
		...
		printerInstance->controlCallbackFn(printerInstance->controlCallbackParam,
						   transfer->transferBuffer,
						   transfer->transferSofar, status);
	}
	(void)USB_HostFreeTransfer(printerInstance->hostHandle, transfer);
...
static usb_status_t USB_HostPrinterOpenInterface(usb_host_printer_instance_t *printerInstance)
{
	...
	for (epIndex = 0; epIndex < interfacePointer->epCount; ++epIndex) {
		...
		if (((epDesc->bEndpointAddress & USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) ==
			...
			status = USB_HostOpenPipe(printerInstance->hostHandle,
						  &printerInstance->inPipe, &pipeInit);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
			...
			status = USB_HostOpenPipe(printerInstance->hostHandle,
						  &printerInstance->outPipe, &pipeInit);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("USB_HostPrinterSetInterface fail to open pipe\r\n");
#endif
						...
						usb_status_t status)
{
	...
	if (printerInstance->controlCallbackFn != NULL) {
		 ...
		 * USB_HostPrinterControl,
		 * USB_HostPrinterSetInterface or USB_HostHubClassRequestCommon, but is the same function */
		printerInstance->controlCallbackFn(printerInstance->controlCallbackParam, NULL, 0,
						   status);
	}
	(void)USB_HostFreeTransfer(printerInstance->hostHandle, transfer);
}
...
usb_status_t USB_HostPrinterInit(usb_device_handle deviceHandle, usb_host_class_handle *classHandle)
{
	...
	usb_host_printer_instance_t *printerInstance =
		(usb_host_printer_instance_t *)OSA_MemoryAllocate(
			sizeof(usb_host_printer_instance_t));
					 ...
					 void *				callbackParam)
{
	...
	/* cancel transfers */
	if (printerInstance->inPipe != NULL) {
		status = USB_HostCancelTransfer(printerInstance->hostHandle,
						printerInstance->inPipe, NULL);
	...
	if (alternateSetting == 0U) { /* open interface directly */
		...
		/* malloc one transfer */
		if (USB_HostMallocTransfer(printerInstance->hostHandle, &transfer) !=
		    kStatus_USB_Success) {
#ifdef HOST_ECHO
		...
		transfer->setupPacket->wIndex = USB_SHORT_TO_LITTLE_ENDIAN(
			((usb_host_interface_t *)printerInstance->interfaceHandle)->interfaceDesc->
			bInterfaceNumber);
		transfer->setupPacket->wValue = USB_SHORT_TO_LITTLE_ENDIAN(alternateSetting);
		transfer->setupPacket->wLength = 0;
		transfer->transferBuffer = NULL;
		transfer->transferLength = 0;
		status = USB_HostSendSetup(printerInstance->hostHandle,
					   printerInstance->controlPipe, transfer);
...
usb_status_t USB_HostPrinterDeinit(usb_device_handle deviceHandle, usb_host_class_handle
				   classHandle)
{
	usb_status_t status;
	usb_host_printer_instance_t *printerInstance = (usb_host_printer_instance_t *)classHandle;
	...
	if (classHandle != NULL) { /* class instance has initialized */
		if (printerInstance->inPipe != NULL) {
			status =
				USB_HostCancelTransfer(printerInstance->hostHandle,
						       printerInstance->inPipe, NULL);              /* cancel pipe */
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("error when cancel pipe\r\n");
#endif
			}
			status = USB_HostClosePipe(printerInstance->hostHandle,
						   printerInstance->inPipe);                  /* close pipe */
		...
		if (printerInstance->outPipe != NULL) {
			status =
				USB_HostCancelTransfer(printerInstance->hostHandle,
						       printerInstance->outPipe, NULL);              /* cancel pipe */
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("error when cancel pipe\r\n");
#endif
			...
			status = USB_HostClosePipe(printerInstance->hostHandle,
						   printerInstance->outPipe);                  /* close pipe */
				 ...
				 void *			callbackParam)
{
	...
	if (USB_HostRecv(printerInstance->hostHandle, printerInstance->inPipe, transfer) !=
	    kStatus_USB_Success) {
#ifdef HOST_ECHO
...
uint16_t USB_HostPrinterGetPacketsize(usb_host_class_handle classHandle, uint8_t pipeType, uint8_t
				      direction)
{
				 ...
				 void *			callbackParam)
{
	...
	/* call host driver api */
	if (USB_HostSend(printerInstance->hostHandle, printerInstance->outPipe, transfer) !=
	    kStatus_USB_Success) {
#ifdef HOST_ECHO
					   ...
					   void *			callbackParam)
{
	...
	/* call host driver api */
	if (USB_HostSendSetup(printerInstance->hostHandle, printerInstance->controlPipe,
			      transfer) != kStatus_USB_Success) {
#ifdef HOST_ECHO
					...
					void *			callbackParam)
{
	...
	(void)USB_HostHelperGetPeripheralInformation(printerInstance->deviceHandle,
						     (uint32_t)kUSB_HostGetDeviceConfigIndex,
						     &infoValue);

	return USB_HostPrinterControl(
		printerInstance, USB_REQUEST_TYPE_DIR_IN | USB_REQUEST_TYPE_TYPE_CLASS |
		USB_REQUEST_TYPE_RECIPIENT_INTERFACE,
		USB_HOST_PRINTER_GET_DEVICE_ID, (uint16_t)infoValue,
		(uint16_t)((uint16_t)((uint16_t)interfaceIndex << 8U) | alternateSetting),
		(uint16_t)length, buffer, callbackFn,
		callbackParam);
}
					  ...
					  void *		callbackParam)
{
	...
	return USB_HostPrinterControl(
		...
		(uint16_t)(((usb_host_interface_t *)printerInstance->interfaceHandle)->interfaceDesc
			   ->bInterfaceNumber), 1,
