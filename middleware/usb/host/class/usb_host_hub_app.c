 ...
 * @param hubInstance  hub instance pointer.
 */
static void USB_HostHubProcessData(usb_host_hub_global_t *	hubGlobal,
 ...
 */
static void USB_HostHubControlCallback(void *param, uint8_t *data, uint32_t data_len, usb_status_t
				       status);
 ...
 * @param status      transfer status.
 */
void USB_HostHubInterruptInCallback(void *param, uint8_t *data, uint32_t data_len, usb_status_t
				    status);

#if ((defined(USB_HOST_CONFIG_LOW_POWER_MODE)) && (USB_HOST_CONFIG_LOW_POWER_MODE > 0U))

static usb_host_hub_instance_t *USB_HostHubGetHubDeviceHandle(usb_host_handle hostHandle, uint8_t
							      parentHubNo);

static usb_status_t USB_HostSendHubRequest(usb_device_handle deviceHandle, uint8_t requestType,
...
static void USB_HostHubGetInterruptStatus(usb_host_hub_instance_t *hubInstance)
{
	...
	if (USB_HostHubInterruptRecv(hubInstance, hubInstance->hubBitmapBuffer, (portNum + 1U),
				     USB_HostHubInterruptInCallback, hubInstance) !=
	    kStatus_USB_Success) {
#ifdef HOST_ECHO
...
static void USB_HostHubProcess(usb_host_hub_instance_t *hubInstance)
{
	...
	case kHubRunWaitSetInterface:
		...
		/* get hub descriptor */
		if (USB_HostHubGetDescriptor(hubInstance, hubInstance->hubDescriptor, 7,
	...
	case kHubRunGetDescriptor7:
		...
		/* get hub descriptor */
		portNum = (((uint16_t)hubInstance->portCount) >> 3U);
		if (USB_HostHubGetDescriptor(hubInstance, hubInstance->hubDescriptor,
					     ((uint16_t)7UL + (portNum) + 1U),
					     USB_HostHubControlCallback, hubInstance) ==
		    kStatus_USB_Success) {
			hubInstance->primeStatus = (uint8_t)kPrimeHubControl; /* control transfer is on-going */
			processSuccess = 1U;
#ifdef HOST_ECHO
	...
	case kHubRunSetPortPower:
		...
		if (NULL == hubInstance->portList) {
			hubInstance->portList = (usb_host_hub_port_instance_t *)OSA_MemoryAllocate(
				((uint32_t)hubInstance->portCount) *
				sizeof(usb_host_hub_port_instance_t));
		...
		if (hubInstance->portIndex < hubInstance->portCount) {
			...
			if (USB_HostHubSetPortFeature(hubInstance, hubInstance->portIndex,
						      PORT_POWER,
						      USB_HostHubControlCallback, hubInstance) ==
			    kStatus_USB_Success) {
	...
	case kHubRunGetStatusDone: /* process hub status change */
		...
		if (0U != ((1UL << C_HUB_LOCAL_POWER) & tmp)) { /* C_HUB_LOCAL_POWER */
			if (USB_HostHubClearFeature(hubInstance, C_HUB_LOCAL_POWER,
						    USB_HostHubControlCallback, hubInstance) ==
			    kStatus_USB_Success) {
			...
			if (USB_HostHubClearFeature(hubInstance, C_HUB_OVER_CURRENT,
						    USB_HostHubControlCallback, hubInstance) ==
			    kStatus_USB_Success) {
				hubInstance->primeStatus = (uint8_t)kPrimeHubControl;
				processSuccess = 1U;
...
static void USB_HostHubProcessPort(usb_host_hub_instance_t *hubInstance)
{
	usb_host_hub_port_instance_t *portInstance =
		&hubInstance->portList[hubInstance->portProcess - 1U];
...
static void USB_HostHubProcessPortAttach(usb_host_hub_instance_t *hubInstance)
{
	...
	case kPortRunWaitPortChange:                                                    /* (1) port is changed, and get port status */
		...
		if (USB_HostHubGetPortStatus(hubInstance, hubInstance->portProcess,
					     hubInstance->portStatusBuffer, 4,
					     USB_HostHubControlCallback, hubInstance) ==
		    kStatus_USB_Success) {
	...
	case kPortRunCheckCPortConnection: /* (2) check port status, and clear the status bits */
		...
		if (0U != ((1UL << C_PORT_CONNECTION) & specStatus)) {
			...
			if (USB_HostHubClearPortFeature(hubInstance, hubInstance->portProcess,
							C_PORT_CONNECTION,
							USB_HostHubControlCallback, hubInstance) ==
			    kStatus_USB_Success) {
				hubInstance->primeStatus = (uint8_t)kPrimePortControl;
			...
			/* set PORT_RESET */
			if (USB_HostHubSetPortFeature(hubInstance, hubInstance->portProcess,
						      PORT_RESET,
						      USB_HostHubControlCallback, hubInstance) ==
			    kStatus_USB_Success) {
				hubInstance->primeStatus = (uint8_t)kPrimePortControl;
	...
	case kPortRunPortAttached: /* (9) the port have one device attached */
		...
		(void)USB_HostHelperGetPeripheralInformation(hubInstance->deviceHandle,
							     ...
							     &infoValue);
		(void)USB_HostAttachDevice(hubInstance->hostHandle, portInstance->speed,
					   (uint8_t)infoValue,
...
static void USB_HostHubProcessPortDetach(usb_host_hub_instance_t *hubInstance)
{
	...
	case kPortRunPortAttached: /* (1) port is changed, then get port status */
		...
		/* get port status */
		if (USB_HostHubGetPortStatus(hubInstance, hubInstance->portProcess,
					     hubInstance->portStatusBuffer, 4U,
					     USB_HostHubControlCallback, hubInstance) ==
		    kStatus_USB_Success) {
			hubInstance->primeStatus = (uint8_t)kPrimePortControl;
			processSuccess = 1U;
				   ...
				   usb_host_hub_instance_t *	hubInstance)
{
	...
	for (portIndex = 0U; portIndex <= hubInstance->portCount; ++portIndex) {
		if (0U != ((0x01U << (portIndex & 0x07U)) &
			   (hubInstance->hubBitmapBuffer[portIndex >> 3U]))) {
			if (portIndex == 0U) { /* hub status change */
				if ((hubGlobal->hubProcess == NULL) ||
					...
					if (USB_HostHubGetStatus(hubInstance,
								 hubInstance->hubStatusBuffer, 4U,
								 USB_HostHubControlCallback,
								 hubInstance) !=
					    kStatus_USB_Success) {
#ifdef HOST_ECHO
						...
						hubInstance->primeStatus =
							(uint8_t)kPrimeHubControl;
...
static void USB_HostSetHubRequestCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					  status)
{
...
static void USB_HostClearHubRequestCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					    status)
{
	(void)USB_HostFreeTransfer(param, transfer);
...
static void USB_HostHubRemoteWakeupCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					    status)
{
	usb_host_hub_instance_t *hubInstance;
	...
	if (kStatus_USB_Success == status) {
		...
		while (NULL != hubInstance) {
			...
			if (0U != hubInstance->supportRemoteWakeup) {
				(void)usb_echo(
					"Set HUB remote wakeup feature: level %d, address %d.\r\n",
					((usb_host_device_instance_t *)hubInstance->
					 deviceHandle)->level,
					((usb_host_device_instance_t *)hubInstance->
					 deviceHandle)->setAddress);
				status = USB_HostSendHubRequest(
					hubInstance->deviceHandle,
					USB_REQUEST_TYPE_RECIPIENT_DEVICE |
					USB_REQUEST_TYPE_DIR_OUT | USB_REQUEST_TYPE_TYPE_STANDARD,
				...
				if (kStatus_USB_Success != status) {
					(void)usb_echo(
						"Send set remote wakeup request to HUB failed.\r\n");
				}
				break;
			}
			hubInstance = hubInstance->next;
		...
		if (0U != hubInstance->controlRetry) {
			...
			(void)usb_echo("Retry...\r\n",
				       ((usb_host_device_instance_t *)hubInstance->deviceHandle)->
				       level,
				       ((usb_host_device_instance_t *)hubInstance->deviceHandle)->
				       setAddress);
			status = USB_HostSendHubRequest(
				hubInstance->deviceHandle,
				USB_REQUEST_TYPE_RECIPIENT_DEVICE | USB_REQUEST_TYPE_DIR_OUT |
				USB_REQUEST_TYPE_TYPE_STANDARD,
				USB_REQUEST_STANDARD_SET_FEATURE,
				USB_REQUEST_STANDARD_FEATURE_SELECTOR_DEVICE_REMOTE_WAKEUP, 0,
				USB_HostHubRemoteWakeupCallback, hubInstance);
			if (kStatus_USB_Success != status) {
	...
	if (NULL == hubInstance) {
		...
		if (NULL == deviceInstance) {
			...
			status = hostInstance->controllerTable->controllerIoctl(
				hostInstance->controllerHandle, kUSB_HostBusControl,
				&type);
			if (kStatus_USB_Success != status) {
			...
			usb_host_hub_instance_t *hubInstance4Device =
				USB_HostHubGetHubDeviceHandle(hostInstance,
							      deviceInstance->hubNumber);
			if (NULL != hubInstance4Device) {
				status = USB_HostSendHubRequest(
					hubInstance4Device->deviceHandle,
					USB_REQUEST_TYPE_DIR_OUT | USB_REQUEST_TYPE_TYPE_CLASS |
					USB_REQUEST_TYPE_RECIPIENT_OTHER,
					USB_REQUEST_STANDARD_SET_FEATURE, PORT_SUSPEND,
					deviceInstance->portNumber,
					USB_HostSetHubRequestCallback, hostInstance);
				if (kStatus_USB_Success != status) {
					(void)usb_echo(
						"Send suspend request to HUB is failed.\r\n");
				}
			} else {
				    ...
				    uint32_t				eventCode)
{
	...
	case kUSB_HostEventEnumerationDone:
		...
		if ((s_HubDeviceHandle != NULL) && (s_HubInterfaceHandle != NULL)) {
			...
			if (hubGlobal->hubMutex == (osa_mutex_handle_t)NULL) {
				hubGlobal->hubMutex =
					(osa_mutex_handle_t)(&hubGlobal->mutexBuffer[0]);
			...
			if (status == kStatus_USB_Success) {
				...
				if (USB_HostHubSetInterface(hubClassHandle, s_HubInterfaceHandle, 0,
							    USB_HostHubControlCallback,
							    hubInstance) != kStatus_USB_Success) {
					hubInstance->hubStatus = (uint8_t)kHubRunInvalid;
				}
	...
	case kUSB_HostEventDetach:
		...
		if (hubInstance != NULL) {
			...
			for (uint8_t portIndex = 0U; portIndex < hubInstance->portCount;
			     ++portIndex) {
				if ((hubInstance->portList != NULL) &&
				    (hubInstance->portList[portIndex].deviceHandle != NULL)) {
					(void)USB_HostDetachDeviceInternal(hubInstance->hostHandle,
									   hubInstance->portList[
										   portIndex].
									   deviceHandle);
					hubInstance->portList[portIndex].deviceHandle = NULL;
				}
 ...
 */
usb_status_t USB_HostHubRemovePort(usb_host_handle hostHandle, uint8_t hubNumber, uint8_t
				   portNumber)
{
	usb_host_hub_instance_t *hubInstance;
	uint32_t infoValue;
	...
	while (hubInstance != NULL) {
		(void)USB_HostHelperGetPeripheralInformation(hubInstance->deviceHandle,
							     (uint8_t)kUSB_HostGetDeviceAddress,
							     &infoValue);
...
uint32_t USB_HostHubGetHsHubNumber(usb_host_handle hostHandle, uint8_t parentHubNo)
{
	...
	if (hubInstance != NULL) {
		(void)USB_HostHelperGetPeripheralInformation(hubInstance->deviceHandle,
							     (uint32_t)kUSB_HostGetDeviceSpeed,
							     &deviceInfo);
 ...
 * @return port number.
 */
uint32_t USB_HostHubGetHsHubPort(usb_host_handle hostHandle, uint8_t parentHubNo, uint8_t
				 parentPortNo)
{
	usb_host_hub_instance_t *hubInstance;
...
usb_status_t USB_HostHubSuspendDevice(usb_host_handle hostHandle)
{
	...
	if (NULL == hubInstance) {
		...
		status =
			hostInstance->controllerTable->controllerIoctl(
				hostInstance->controllerHandle, kUSB_HostBusControl, &type);
		if (kStatus_USB_Success != status) {
			(void)usb_echo("Suspend USB BUS failed.\r\n");
		}
		return status;
	...
	if (NULL == hubInstance) {
		usb_host_device_instance_t *deviceInstance =
			(usb_host_device_instance_t *)hostInstance->suspendedDevice;
		if (NULL == deviceInstance) {
			usb_host_bus_control_t type = kUSB_HostBusSuspend;
			/* the callbackFn is initialized in USB_HostGetControllerInterface */
			status = hostInstance->controllerTable->controllerIoctl(
			...
			usb_host_hub_instance_t *hubInstance4Device =
				USB_HostHubGetHubDeviceHandle(hostHandle,
							      deviceInstance->hubNumber);
			if (NULL != hubInstance4Device) {
				status = USB_HostSendHubRequest(
					hubInstance4Device->deviceHandle,
...
usb_status_t USB_HostHubResumeDevice(usb_host_handle hostHandle)
{
	...
	if (NULL != hubInstance) {
		status = USB_HostSendHubRequest(
			...
			USB_REQUEST_TYPE_DIR_OUT | USB_REQUEST_TYPE_TYPE_CLASS |
			USB_REQUEST_TYPE_RECIPIENT_OTHER,
			USB_REQUEST_STANDARD_CLEAR_FEATURE, PORT_SUSPEND,
			deviceInstance->portNumber,
