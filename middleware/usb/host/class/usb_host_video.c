 ...
 */
static usb_status_t USB_HostVideoControlOpenInterface(
 ...
 * @return kStatus_USB_Success or error codes.
 */
static usb_status_t USB_HostVideoStreamOpenInterface(
	usb_host_video_instance_struct_t *videoInstance);
 ...
 */
static void USB_HostVideoSetControlInterfaceCallback(void *param, usb_host_transfer_t *transfer,
						     usb_status_t status);
 ...
 * @param status      transfer status.
 */
static void USB_HostVideoSetStreamInterfaceCallback(void *param, usb_host_transfer_t *transfer,
						    usb_status_t status);
 ...
 * @param status      transfer status.
 */
static void USB_HostVideoControlCommandCallback(void *param, usb_host_transfer_t *transfer,
						usb_status_t status);
 ...
 * @param status      transfer status.
 */
static void USB_HostVideoStreamIsoInPipeCallback(void *param, usb_host_transfer_t *transfer,
						 usb_status_t status);
 ...
 */
static usb_status_t USB_HostVideoControl(usb_host_class_handle classHandle, uint8_t typeRequest,
					 uint8_t request, uint16_t wvalue, uint16_t windex, uint16_t
					 wlength, uint8_t *data, transfer_callback_t callbackFn,
					 void *callbackParam);
	...
	usb_host_video_instance_struct_t *videoInstance)
{
	...
	for (ep_index = 0U; ep_index < interface_ptr->epCount; ++ep_index) {
		...
		if (((ep_desc->bEndpointAddress & USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) ==
			...
			status = USB_HostOpenPipe(videoInstance->hostHandle,
						  &videoInstance->interruptPipe, &pipe_init);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo(
					"usb_host_video_control_set_interface fail to open pipe\r\n");
#endif
	...
	usb_host_video_instance_struct_t *videoInstance)
{
	...
	if (videoInstance->streamIsoInPipe != NULL) {
		status = USB_HostClosePipe(videoInstance->hostHandle,
					   videoInstance->streamIsoInPipe);
	...
	for (ep_index = 0U; ep_index < interface_ptr->epCount; ++ep_index) {
		...
		if (((ep_desc->bEndpointAddress & USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) ==
			...
			status = USB_HostOpenPipe(videoInstance->hostHandle,
						  &videoInstance->streamIsoInPipe, &pipe_init);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo(
					"usb_host_video_stream_set_interface fail to open pipe\r\n");
#endif
				return kStatus_USB_Error;
			}
		} else {
						     ...
						     usb_status_t status)
{
	...
	if (videoInstance->controlCallbackFn != NULL) {
		...
		videoInstance->controlCallbackFn(videoInstance->controlCallbackParam,
						    ...
						    usb_status_t status)
{
	...
	if (videoInstance->controlCallbackFn != NULL) {
		...
		videoInstance->controlCallbackFn(videoInstance->controlCallbackParam, NULL, 0U,
						 status);
	}
	(void)USB_HostFreeTransfer(videoInstance->hostHandle, transfer);
}
						 ...
						 usb_status_t status)
{
	...
	if (videoInstance->streamIsoInCallbackFn != NULL) {
		...
		videoInstance->streamIsoInCallbackFn(videoInstance->streamIsoInCallbackParam,
						     transfer->transferBuffer,
						     transfer->transferSofar, status);
	}
	(void)USB_HostFreeTransfer(videoInstance->hostHandle, transfer);
...
{
	usb_host_video_instance_struct_t *videoInstance =
		(usb_host_video_instance_struct_t *)classHandle;
	...
	if (USB_HostSendSetup(videoInstance->hostHandle, videoInstance->controlPipe, transfer) !=
	    kStatus_USB_Success) {
#ifdef HOST_ECHO
					     ...
					     void *			callbackParam)
{
	...
	if (videoInstance->streamIsoInPipe != NULL) {
		status = USB_HostCancelTransfer(videoInstance->hostHandle,
						videoInstance->streamIsoInPipe, NULL);
	...
	if (0U == alternateSetting) {
		...
		while (length < interface_ptr->interfaceExtensionLength) {
			if (descUnion.common->bDescriptorType == USB_HOST_DESC_CS_INTERFACE) {
				if (descUnion.common->bData[0] ==
				    USB_HOST_DESC_SUBTYPE_VS_INPUT_HEADER) {
					temp = (void *)descUnion.bufr;
					videoInstance->vsInputHeaderDesc =
						(usb_host_video_stream_input_header_desc_t *)temp;
					break;
				}
			}
		...
		while (ep < interface_ptr->epCount) {
			if (descUnion.common->bDescriptorType == USB_DESCRIPTOR_TYPE_ENDPOINT) {
				...
				if (USB_HOST_DESC_CS_ENDPOINT ==
				    descUnion.common->bDescriptorType) {
					interface_ptr->epList[ep].epExtension = descUnion.bufr;
					interface_ptr->epList[ep].epExtensionLength =
						descUnion.common->bLength;
				} else {
	...
	if (alternateSetting == 0U) {
		...
		if (USB_HostMallocTransfer(videoInstance->hostHandle, &transfer) !=
		    kStatus_USB_Success) {
#ifdef HOST_ECHO
			usb_echo("error to get transfer\r\n");
#endif
		...
		transfer->setupPacket->wIndex = USB_SHORT_TO_LITTLE_ENDIAN(
			((usb_host_interface_t *)videoInstance->streamIntfHandle)->interfaceDesc->
			bInterfaceNumber);
		transfer->setupPacket->wValue = USB_SHORT_TO_LITTLE_ENDIAN(alternateSetting);
		transfer->setupPacket->wLength = 0;
		transfer->transferBuffer = NULL;
		transfer->transferLength = 0;
		status = USB_HostSendSetup(videoInstance->hostHandle, videoInstance->controlPipe,
					   transfer);
					      ...
					      void *			callbackParam)
{
	...
	while (length < interface_ptr->interfaceExtensionLength) {
		if (((interface_ptr->interfaceDesc->bDescriptorType ==
	...
	while (length < interface_ptr->interfaceExtensionLength) {
		if (desc.common->bDescriptorType == USB_HOST_DESC_CS_INTERFACE) {
			...
			if (desc.common->bData[0] == USB_HOST_DESC_SUBTYPE_VC_HEADER) {
				videoInstance->vcHeaderDesc =
					(usb_host_video_ctrl_header_desc_t *)temp;
			} else if (desc.common->bData[0] ==
				   USB_HOST_DESC_SUBTYPE_VC_INPUT_TERMINAL) {
				videoInstance->vcInputTerminalDesc =
					(usb_host_video_ctrl_it_desc_t *)temp;
			} else if (desc.common->bData[0] ==
				   USB_HOST_DESC_SUBTYPE_VC_OUTPUT_TERMINAL) {
				videoInstance->vcOutputTerminalDesc =
					(usb_host_video_ctrl_ot_desc_t *)temp;
			} else if (desc.common->bData[0] ==
				   USB_HOST_DESC_SUBTYPE_VC_PROCESSING_UNIT) {
				videoInstance->vcProcessingUnitDesc =
					(usb_host_video_ctrl_pu_desc_t *)temp;
			} else {
				/*no action*/
			}
		}
	...
	if (alternateSetting == 0U) {
		...
		transfer->setupPacket->bmRequestType = USB_REQUEST_TYPE_RECIPIENT_INTERFACE;
		transfer->setupPacket->wIndex = USB_SHORT_TO_LITTLE_ENDIAN(
			((usb_host_interface_t *)videoInstance->controlIntfHandle)->interfaceDesc->
			bInterfaceNumber);
		transfer->setupPacket->wValue = USB_SHORT_TO_LITTLE_ENDIAN(alternateSetting);
		transfer->setupPacket->wLength = 0;
		transfer->transferBuffer = NULL;
				    ...
				    void *			callbackParam)
{
	...
	if (USB_HostRecv(videoInstance->hostHandle, videoInstance->streamIsoInPipe, transfer) !=
	    kStatus_USB_Success) {
#ifdef HOST_ECHO
...
{
	usb_host_video_instance_struct_t *videoInstance =
		(usb_host_video_instance_struct_t *)OSA_MemoryAllocate(
			sizeof(usb_host_video_instance_struct_t));
	uint32_t info_value;
	uint32_t *temp;
...
usb_status_t USB_HostVideoDeinit(usb_device_handle deviceHandle, usb_host_class_handle classHandle)
{
	...
	if (classHandle != NULL) {
		...
		if ((videoInstance->controlPipe != NULL) && (videoInstance->controlTransfer !=
							     NULL)) {
			status = USB_HostCancelTransfer(videoInstance->hostHandle,
							videoInstance->controlPipe,
...
usb_status_t USB_HostVideoStreamGetFrameDescriptor(
	usb_host_class_handle classHandle, void *formatDescriptor, uint8_t subType, uint8_t
	frameIndex, void **descriptor)
{
				   ...
				   void *			callbackParam)
{
	...
	status = USB_HostVideoControl(
		...
		request, (uint16_t)(USB_HOST_VS_PROBE_CONTROL << 8UL),
		streamInterface->interfaceDesc->bInterfaceNumber, 26U,
				    ...
				    void *			callbackParam)
{
	...
	status = USB_HostVideoControl(
		classHandle, USB_REQUEST_TYPE_DIR_IN | USB_REQUEST_TYPE_TYPE_CLASS |
		USB_REQUEST_TYPE_RECIPIENT_INTERFACE,
		brequest, (uint16_t)(USB_HOST_VS_COMMIT_CONTROL << 8UL),
		streamInterface->interfaceDesc->bInterfaceNumber, 26U,
		probe, callbackFn, callbackParam);
	return status;
}
