 ...
 */
static void USB_HostPhdcControlPipeCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					    status);
 ...
 * @param status      transfer status.
 */
static void USB_HostPhdcInterruptPipeCallback(void *param, usb_host_transfer_t *transfer,
					      usb_status_t status);
 ...
 * @param status      transfer status.
 */
static void USB_HostPhdcBulkInPipeCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					   status);
 ...
 * @param status      transfer status.
 */
static void USB_HostPhdcBulkOutPipeCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					    status);
 ...
 * @param status      transfer status.
 */
static void USB_HostPhdcSetInterfaceCallback(void *param, usb_host_transfer_t *transfer,
					     usb_status_t status);
...
static void USB_HostPhdcClearInHaltCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					    status)
{
	...
	if (phdcInstance->inCallbackFn != NULL) {
		...
		phdcInstance->inCallbackFn(phdcInstance->inCallbackParam,
					   phdcInstance->stallDataBuffer,
					   phdcInstance->stallDataLength,
					   kStatus_USB_TransferStall);
	}
	(void)USB_HostFreeTransfer(phdcInstance->hostHandle, transfer);
}

static void USB_HostPhdcClearOutHaltCallback(void *param, usb_host_transfer_t *transfer,
					     usb_status_t status)
{
	usb_host_phdc_instance_t *phdcInstance = (usb_host_phdc_instance_t *)param;
	...
	if (phdcInstance->outCallbackFn != NULL) {
		...
		phdcInstance->outCallbackFn(phdcInstance->outCallbackParam,
					    phdcInstance->stallDataBuffer,
					    phdcInstance->stallDataLength,
					    kStatus_USB_TransferStall);
	}
					    ...
					    status)
{
	...
	if (NULL != phdcInstance->controlCallbackFn) {
		...
		phdcInstance->controlCallbackFn(phdcInstance->controlCallbackParam,
							    ...
							    usb_status_t		status)
{
	...
	if (kStatus_USB_Success == status) {
		if ((transfer->setupPacket->bRequest == USB_REQUEST_STANDARD_SET_FEATURE) &&
			...
			featureParam.featureSelector =
				USB_REQUEST_STANDARD_FEATURE_SELECTOR_ENDPOINT_HALT;
			featureParam.interfaceOrEndpoint =
				phdcInstance->bulkInEndpointInformation.epDesc->bEndpointAddress;
			if (kStatus_USB_Success != USB_HostPhdcSetClearFeatureEndpointHalt(
				    phdcInstance->hostHandle,
					      ...
					      usb_status_t status)
{
	...
	if (status == kStatus_USB_TransferStall) {
		if (USB_HostPhdcClearHalt(
			    ...
			    (USB_REQUEST_TYPE_DIR_IN |
			     ((usb_host_pipe_t *)phdcInstance->interruptPipe)->endpointAddress)) ==
		    kStatus_USB_Success) {
			(void)USB_HostFreeTransfer(phdcInstance->hostHandle, transfer);
			return;
		}
	...
	if (NULL != phdcInstance->inCallbackFn) {
		/* callback to application, the callback function is initialized in USB_HostPhdcRecv */
		phdcInstance->inCallbackFn(phdcInstance->inCallbackParam, transfer->transferBuffer,
					   ...
					   status)
{
	...
	if (status == kStatus_USB_TransferStall) {
		if (USB_HostPhdcClearHalt(
			    phdcInstance, transfer, USB_HostPhdcClearInHaltCallback,
			    (USB_REQUEST_TYPE_DIR_IN |
			     ((usb_host_pipe_t *)phdcInstance->bulkInPipe)->endpointAddress)) ==
		    kStatus_USB_Success) {
			(void)USB_HostFreeTransfer(phdcInstance->hostHandle, transfer);
			return;
		}
		...
		if (phdcInstance->isMessagePreambleEnabled == 1U) {
			...
			if (0U != phdcInstance->numberTransferBulkIn) {
				...
				usb_host_phdc_metadata_preamble_t *metaDataMsgPreamble =
					(usb_host_phdc_metadata_preamble_t *)temp;
				/* Meta-data message preamble signature checking */
				for (uint8_t i = 0U; i <
				     USB_HOST_PHDC_MESSAGE_PREAMBLE_SIGNATURE_SIZE; i++) {
					if (*(transfer->transferBuffer + i) !=
					    (uint8_t)metaDataMsgPreambleSignature[i]) {
				...
				if (0U != preambleSignatureChecking) {
					      ...
					      0x20U) /* VeryHigh.Best latency, reliability bin */))
					{
						...
						usb_host_process_feature_param_t featureParam;
						featureParam.requestType =
							(uint8_t)kRequestEndpoint;
						featureParam.featureSelector =
							USB_REQUEST_STANDARD_FEATURE_SELECTOR_ENDPOINT_HALT;
						featureParam.interfaceOrEndpoint =
							phdcInstance->bulkInEndpointInformation.
						...
						phdcInstance->numberTransferBulkIn =
							metaDataMsgPreamble->bNumberTransfers;
					}
				}
			}
		}
	...
	if (status == kStatus_USB_TransferStall) {
		if (USB_HostPhdcClearHalt(
			    ...
			    (USB_REQUEST_TYPE_DIR_OUT |
			     ((usb_host_pipe_t *)phdcInstance->bulkOutPipe)->endpointAddress)) ==
		    kStatus_USB_Success) {
			(void)USB_HostFreeTransfer(phdcInstance->hostHandle, transfer);
			return;
		}
...
static usb_status_t USB_HostPhdcOpenInterface(usb_host_phdc_instance_t *phdcInstance)
{
	...
	for (epIndex = 0U; epIndex < interface->epCount; ++epIndex) {
		...
		if (((epDesc->bEndpointAddress & USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) ==
		    ...
		    ((epDesc->bmAttributes & USB_DESCRIPTOR_ENDPOINT_ATTRIBUTE_TYPE_MASK) ==
			...
			pipeInit.endpointAddress = (epDesc->bEndpointAddress &
			...
			status = USB_HostOpenPipe(phdcInstance->hostHandle,
						  &phdcInstance->interruptPipe, &pipeInit);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
			...
			pipeInit.nakCount = USB_HOST_CONFIG_MAX_NAK;
			status = USB_HostOpenPipe(phdcInstance->hostHandle,
						  &phdcInstance->bulkInPipe, &pipeInit);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("USB_HostPhdcOpenInterface: Error when open pipe\r\n");
#endif
			...
			pipeInit.nakCount = USB_HOST_CONFIG_MAX_NAK;
			status = USB_HostOpenPipe(phdcInstance->hostHandle,
						  &phdcInstance->bulkOutPipe, &pipeInit);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("USB_HostPhdcOpenInterface: Error when open pipe\r\n");
#endif
					     ...
					     usb_status_t status)
{
	...
	if (NULL != phdcInstance->controlCallbackFn) {
		...
		phdcInstance->controlCallbackFn(phdcInstance->controlCallbackParam, NULL, 0U,
						status);
	}
	(void)USB_HostFreeTransfer(phdcInstance->hostHandle, transfer);
}
				      ...
				      void *			callbackParam)
{
	...
	if (NULL != phdcInstance->interruptPipe) {
		status = USB_HostCancelTransfer(phdcInstance->hostHandle,
						phdcInstance->interruptPipe, NULL);
		if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
	...
	if (NULL != phdcInstance->bulkInPipe) {
		status = USB_HostCancelTransfer(phdcInstance->hostHandle, phdcInstance->bulkInPipe,
						NULL);
		if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
			usb_echo("USB_HostPhdcSetInterface: Error when cancel pipe\r\n");
	...
	if (NULL != phdcInstance->bulkOutPipe) {
		status = USB_HostCancelTransfer(phdcInstance->hostHandle, phdcInstance->bulkOutPipe,
						NULL);
		if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
			usb_echo("USB_HostPhdcSetInterface: Error when cancel pipe\r\n");
	...
	if (0U == alternateSetting) {
		...
		if (USB_HostMallocTransfer(phdcInstance->hostHandle, &transfer) !=
		    kStatus_USB_Success) {
#ifdef HOST_ECHO
		...
		transfer->setupPacket->wIndex = USB_SHORT_TO_LITTLE_ENDIAN(
			((usb_host_interface_t *)phdcInstance->interfaceHandle)->interfaceDesc->
			bInterfaceNumber);
		...
		status = USB_HostSendSetup(phdcInstance->hostHandle, phdcInstance->controlPipe,
					   transfer);
		if (status == kStatus_USB_Success) {
			phdcInstance->controlTransfer = transfer;
		} else {
...
usb_status_t USB_HostPhdcDeinit(usb_host_handle deviceHandle, usb_host_class_handle classHandle)
{
	...
	if (NULL != classHandle) {
		if (NULL != phdcInstance->interruptPipe) {
			...
			status = USB_HostClosePipe(phdcInstance->hostHandle,
						   phdcInstance->interruptPipe);
		...
		if ((NULL != phdcInstance->controlPipe) && (NULL !=
							    phdcInstance->controlTransfer)) {
			/* Cancel control transfers */
			status = USB_HostCancelTransfer(phdcInstance->hostHandle,
							phdcInstance->controlPipe,
							phdcInstance->controlTransfer);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("error when cancel pipe\r\n");
			      ...
			      void *			callbackParam)
{
	...
	if (phdcInstance->isMessagePreambleEnabled == 1U) {
		...
		if (0U != phdcInstance->numberTransferBulkOut) {
			...
			usb_host_phdc_qos_descriptor_t *qosDesc =
				(usb_host_phdc_qos_descriptor_t *)temp;
			temp = (void *)buffer;
...
#ifdef HOST_ECHO
				usb_echo(
					"USB_HostPhdcSend, Error: the latency reliability is not supported by Bulk OUT endpoint");
#endif
				return status;
			}
...
#ifdef HOST_ECHO
				usb_echo(
					"USB_HostPhdcSend, Error: the numTransfer should never zero");
#endif
				return status;
			}
	...
	transfer->callbackParam = phdcInstance;
	if (USB_HostSend(phdcInstance->hostHandle, phdcInstance->bulkOutPipe, transfer) !=
	    kStatus_USB_Success) {
#ifdef HOST_ECHO
					    ...
					    void *			callbackParam)
{
	...
	case USB_HOST_PHDC_GET_STATUS_REQUEST:
		...
		transfer->setupPacket->bmRequestType =
			USB_REQUEST_TYPE_DIR_IN | USB_REQUEST_TYPE_TYPE_CLASS |
	...
	case USB_HOST_PHDC_CLEAR_FEATURE_REQUEST:
		...
		transfer->setupPacket->bmRequestType =
			USB_REQUEST_TYPE_DIR_OUT | USB_REQUEST_TYPE_TYPE_CLASS |
			USB_REQUEST_TYPE_RECIPIENT_INTERFACE;
		transfer->setupPacket->wValue = USB_SHORT_TO_LITTLE_ENDIAN(
			USB_HOST_PHDC_FEATURE_METADATA |
			(uint16_t)((uint16_t)USB_HOST_PHDC_QOS_ENCODING_VERSION << 8U));
	...
	if (USB_HostSendSetup(phdcInstance->hostHandle, phdcInstance->controlPipe, transfer) !=
	    kStatus_USB_Success) {
#ifdef HOST_ECHO
						     ...
						     void *			callbackParam)
{
	...
	transfer->callbackParam = phdcInstance;
	if (kStatus_USB_Success != USB_HostRequestControl(phdcInstance->deviceHandle, request,
							  transfer, param)) {
#ifdef HOST_ECHO
	...
	if (NULL != classHandle) {
		if (pipeType == USB_ENDPOINT_BULK) {
			if (direction == USB_IN) {
				...
				endpointReturn =
					(usb_host_ep_t *)&phdcInstance->bulkInEndpointInformation;
			} else {
				/* bulk out endpoint information */
				endpointReturn =
					(usb_host_ep_t *)&phdcInstance->bulkOutEndpointInformation;
			}
		} else if (pipeType == USB_ENDPOINT_INTERRUPT) {
			/* interrupt in endpoint information */
			endpointReturn =
				(usb_host_ep_t *)&phdcInstance->interruptInEndpointInformation;
		} else {
			/*no action*/
		}
	}
