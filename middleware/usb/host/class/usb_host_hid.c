 ...
 */
static void USB_HostHidInPipeCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
				      status);
 ...
 * @param status      transfer status.
 */
static void USB_HostHidOutPipeCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
				       status);
 ...
 * @param status      transfer status.
 */
static void USB_HostHidControlCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
				       status);
 ...
 * @param status      transfer status.
 */
static void USB_HostHidSetInterfaceCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					    status);
...
static usb_status_t USB_HostHidControl(usb_host_class_handle classHandle, uint8_t requestType,
				       uint8_t request, uint8_t wvalueL, uint8_t wvalueH, uint16_t
...
static void USB_HostHidClearInHaltCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					   status)
{
	...
	if (hidInstance->inCallbackFn != NULL) {
		...
		hidInstance->inCallbackFn(hidInstance->inCallbackParam,
					  hidInstance->stallDataBuffer,
					  hidInstance->stallDataLength, kStatus_USB_TransferStall);
	}
	(void)USB_HostFreeTransfer(hidInstance->hostHandle, transfer);
}

static void USB_HostHidClearOutHaltCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					    status)
{
	usb_host_hid_instance_t *hidInstance = (usb_host_hid_instance_t *)param;
	...
	if (hidInstance->outCallbackFn != NULL) {
		...
		hidInstance->outCallbackFn(hidInstance->outCallbackParam,
					   hidInstance->stallDataBuffer,
					   hidInstance->stallDataLength, kStatus_USB_TransferStall);
	}
	(void)USB_HostFreeTransfer(hidInstance->hostHandle, transfer);
				      ...
				      status)
{
	...
	if (status == kStatus_USB_TransferStall) {
		if (USB_HostHidClearHalt(hidInstance, transfer, USB_HostHidClearInHaltCallback,
					 (USB_REQUEST_TYPE_DIR_IN |
					  ((usb_host_pipe_t *)hidInstance->inPipe)->endpointAddress))
		    == kStatus_USB_Success) {
			(void)USB_HostFreeTransfer(hidInstance->hostHandle, transfer);
			return;
		}
	...
	if (hidInstance->inCallbackFn != NULL) {
		/* callback to application, callback function is initialized in the USB_HostHidRecv */
		hidInstance->inCallbackFn(hidInstance->inCallbackParam, transfer->transferBuffer,
				       ...
				       status)
{
	...
	if (status == kStatus_USB_TransferStall) {
		if (USB_HostHidClearHalt(hidInstance, transfer, USB_HostHidClearOutHaltCallback,
					 (USB_REQUEST_TYPE_DIR_OUT |
					  ((usb_host_pipe_t *)hidInstance->outPipe)->endpointAddress))
		    == kStatus_USB_Success) {
			(void)USB_HostFreeTransfer(hidInstance->hostHandle, transfer);
			return;
		}
	...
	if (hidInstance->outCallbackFn != NULL) {
		/* callback to application, callback function is initialized in USB_HostHidSend */
		hidInstance->outCallbackFn(hidInstance->outCallbackParam, transfer->transferBuffer,
					   transfer->transferSofar,
				       ...
				       status)
{
	...
	if (hidInstance->controlCallbackFn != NULL) {
		...
		hidInstance->controlCallbackFn(hidInstance->controlCallbackParam,
...
static usb_status_t USB_HostHidOpenInterface(usb_host_hid_instance_t *hidInstance)
{
	...
	for (epIndex = 0; epIndex < interfacePointer->epCount; ++epIndex) {
		...
		if (((epDesc->bEndpointAddress & USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) ==
			...
			status = USB_HostOpenPipe(hidInstance->hostHandle, &hidInstance->inPipe,
						  &pipeInit);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
			...
			status = USB_HostOpenPipe(hidInstance->hostHandle, &hidInstance->outPipe,
						  &pipeInit);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("USB_HostHidSetInterface fail to open pipe\r\n");
				     ...
				     void *			callbackParam)
{
	...
	if (hidInstance->outPipe != NULL) {
		status = USB_HostCancelTransfer(hidInstance->hostHandle, hidInstance->outPipe,
						NULL);
	...
	if (alternateSetting == 0U) { /* open interface directly */
		...
		/* malloc one transfer */
		if (USB_HostMallocTransfer(hidInstance->hostHandle, &transfer) !=
		    kStatus_USB_Success) {
#ifdef HOST_ECHO
		...
		transfer->setupPacket->wIndex = USB_SHORT_TO_LITTLE_ENDIAN(
			((usb_host_interface_t *)hidInstance->interfaceHandle)->interfaceDesc->
			bInterfaceNumber);
		...
		status = USB_HostSendSetup(hidInstance->hostHandle, hidInstance->controlPipe,
					   transfer);
...
usb_status_t USB_HostHidDeinit(usb_device_handle deviceHandle, usb_host_class_handle classHandle)
{
	...
	if (classHandle != NULL) { /* class instance has initialized */
		if (hidInstance->inPipe != NULL) {
			status = USB_HostCancelTransfer(hidInstance->hostHandle,
							hidInstance->inPipe, NULL);              /* cancel pipe */
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("error when cancel pipe\r\n");
#endif
		...
		if (hidInstance->outPipe != NULL) {
			status = USB_HostCancelTransfer(hidInstance->hostHandle,
							hidInstance->outPipe, NULL);              /* cancel pipe */
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("error when cancel pipe\r\n");
#endif
		...
		if ((hidInstance->controlPipe != NULL) &&
			...
			status =
				USB_HostCancelTransfer(hidInstance->hostHandle,
						       hidInstance->controlPipe,
						       hidInstance->controlTransfer);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("error when cancel pipe\r\n");
#endif
...
uint16_t USB_HostHidGetPacketsize(usb_host_class_handle classHandle, uint8_t pipeType, uint8_t
				  direction)
{
	usb_host_hid_instance_t *hidInstance = (usb_host_hid_instance_t *)classHandle;
	if (classHandle == NULL) {
				       ...
				       void *			callbackParam)
{
	...
	transfer->setupPacket->wValue = USB_SHORT_TO_LITTLE_ENDIAN(wvalueL |
								   (uint16_t)((uint16_t)wvalueH <<
									      8));
	transfer->setupPacket->wIndex = USB_SHORT_TO_LITTLE_ENDIAN(
		((usb_host_interface_t *)hidInstance->interfaceHandle)->interfaceDesc->
		bInterfaceNumber);
					    ...
					    void *			callbackParam)
{
	return USB_HostHidControl(
		classHandle, USB_REQUEST_TYPE_DIR_IN | USB_REQUEST_TYPE_TYPE_STANDARD |
		USB_REQUEST_TYPE_RECIPIENT_INTERFACE,
		USB_REQUEST_STANDARD_GET_DESCRIPTOR, 0, USB_DESCRIPTOR_TYPE_HID_REPORT, buffer_len,
		buffer, callbackFn,
		callbackParam);
}
				  ...
				  void *		callbackParam)
{
	return USB_HostHidControl(
		classHandle, USB_REQUEST_TYPE_DIR_IN | USB_REQUEST_TYPE_TYPE_CLASS |
		USB_REQUEST_TYPE_RECIPIENT_INTERFACE,
		USB_HOST_HID_GET_REPORT, reportId, reportType, (uint16_t)bufferLength, buffer,
				  ...
				  void *		callbackParam)
{
	return USB_HostHidControl(
		classHandle, USB_REQUEST_TYPE_DIR_OUT | USB_REQUEST_TYPE_TYPE_CLASS |
		USB_REQUEST_TYPE_RECIPIENT_INTERFACE,
		USB_HOST_HID_SET_REPORT, reportId, reportType, (uint16_t)bufferLength, buffer,
		callbackFn, callbackParam);
}
