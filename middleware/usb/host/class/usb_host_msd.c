 ...
 */
static void USB_HostMsdClearHaltCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					 status);
...
static usb_status_t USB_HostMsdClearHalt(usb_host_msd_instance_t *msdInstance,
					 host_inner_transfer_callback_t callbackFn, uint8_t
					 endpoint);
 ...
 * @param status       result status.
 */
static void USB_HostMsdMassResetClearOutCallback(void *param, usb_host_transfer_t *transfer,
						 usb_status_t status);
 ...
 * @param status       result status.
 */
static void USB_HostMsdMassResetClearInCallback(void *param, usb_host_transfer_t *transfer,
						usb_status_t status);
 ...
 * @param status         result status.
 */
static void USB_HostMsdMassResetCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					 status);
 ...
 * @param status         result status.
 */
static void USB_HostMsdControlCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
				       status);
 ...
 * @param status         result status.
 */
static void USB_HostMsdDataCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
				    status);
 ...
 */
static void USB_HostMsdSetInterfaceCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					    status);
...
static usb_status_t USB_HostMsdControl(usb_host_msd_instance_t *msdInstance,
				       ...
				       uint8_t *buffer, uint16_t bufferLength, uint8_t requestType,
...
static void USB_HostMsdResetDone(usb_host_msd_instance_t *msdInstance, usb_status_t status)
{
	if (msdInstance->internalResetRecovery == 1U) { /* internal mass reset recovery */
		...
		if ((status != kStatus_USB_Success) || (msdInstance->commandStatus ==
							(uint8_t)kMSD_CommandErrorDone)) {
			USB_HostMsdCommandDone(msdInstance, kStatus_USB_Error); /* command fail */
		} else {
			(void)USB_HostMsdProcessCommand(msdInstance);           /* continue to process ufi command */
		}
		...
		if (msdInstance->outPipe != NULL) {
			...
			(void)USB_HostMsdClearHalt(
				...
				(USB_REQUEST_TYPE_DIR_OUT |
				 ((usb_host_pipe_t *)msdInstance->outPipe)->endpointAddress));
		}
	} else {
		USB_HostMsdResetDone(msdInstance, status); /* mass storage reset done */
	}
		...
		if (msdInstance->inPipe != NULL) {
			...
			(void)USB_HostMsdClearHalt(
				...
				(USB_REQUEST_TYPE_DIR_IN |
				 ((usb_host_pipe_t *)msdInstance->inPipe)->endpointAddress));
		}
	} else {
		USB_HostMsdResetDone(msdInstance, status); /* mass storage reset done */
	}
				       ...
				       status)
{
	...
	if (msdInstance->controlCallbackFn != NULL) {
		/* callback to application, callback function is initialized in the USB_HostMsdControl,
		 * or USB_HostMsdSetInterface, but is the same function */
		msdInstance->controlCallbackFn(msdInstance->controlCallbackParam,
...
static void USB_HostMsdCommandDone(usb_host_msd_instance_t *msdInstance, usb_status_t status)
{
	if (msdInstance->commandCallbackFn != NULL) {
		...
		msdInstance->commandCallbackFn(msdInstance->commandCallbackParam,
					       msdInstance->msdCommand.dataBuffer,
...
static void USB_HostMsdCswCallback(void *param, usb_host_transfer_t *transfer, usb_status_t status)
{
	...
	if (status == kStatus_USB_Success) {
		...
		if ((transfer->transferSofar == USB_HOST_UFI_CSW_LENGTH) &&
		    (msdInstance->msdCommand.cswBlock.CSWSignature == USB_LONG_TO_LITTLE_ENDIAN(
			     USB_HOST_MSD_CSW_SIGNATURE))) {
			...
			case 2:
				...
				if (USB_HostMsdMassStorageReset(msdInstance, NULL, NULL) !=
				    kStatus_USB_Success) {
		...
		if (status == kStatus_USB_TransferStall) {              /* case 1: stall */
			...
			if (msdInstance->msdCommand.retryTime > 0U) {
				...
				if (USB_HostMsdClearHalt(
					    ...
					    (USB_REQUEST_TYPE_DIR_IN |
					     ((usb_host_pipe_t *)msdInstance->inPipe)->
					     endpointAddress)) !=
				    kStatus_USB_Success) {
					USB_HostMsdCommandDone(msdInstance, kStatus_USB_Error);
				}
	...
	if (status == kStatus_USB_Success) {
		...
		if (transfer->transferSofar == USB_HOST_UFI_CBW_LENGTH) {
			...
			if (msdInstance->msdCommand.retryTime > 0U) {
				...
				if (USB_HostMsdMassStorageReset(msdInstance, NULL, NULL) !=
				    kStatus_USB_Success) {
					(void)USB_HostMsdCommandDone(msdInstance,
								     kStatus_USB_Error);
				}
			}
		}
...
static usb_status_t USB_HostMsdProcessCommand(usb_host_msd_instance_t *msdInstance)
{
	...
	if (msdInstance->msdCommand.transfer == NULL) {
		/* malloc one transfer */
		status = USB_HostMallocTransfer(msdInstance->hostHandle,
						&(msdInstance->msdCommand.transfer));
		if (status != kStatus_USB_Success) {
	...
	case kMSD_CommandTransferData: /* ufi DATA phase */
		if (msdInstance->msdCommand.dataBuffer != NULL) {
			...
			transfer->transferBuffer = (msdInstance->msdCommand.dataBuffer +
						    msdInstance->msdCommand.dataSofar);
			transfer->transferLength = (msdInstance->msdCommand.dataLength -
						    msdInstance->msdCommand.dataSofar);
			transfer->callbackParam = msdInstance;
			if (msdInstance->msdCommand.dataSofar !=
			    msdInstance->msdCommand.dataLength) {
				if (transfer->direction == USB_OUT) {
					transfer->callbackFn = USB_HostMsdDataCallback;
					status = USB_HostSend(msdInstance->hostHandle,
							      msdInstance->outPipe, transfer);
					if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
						usb_echo("host send error\r\n");
#endif
					...
					transfer->callbackFn = USB_HostMsdDataCallback;
					status = USB_HostRecv(msdInstance->hostHandle,
							      msdInstance->inPipe, transfer);
					if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
...
static usb_status_t USB_HostMsdOpenInterface(usb_host_msd_instance_t *msdInstance)
{
	...
	for (epIndex = 0; epIndex < interfacePointer->epCount; ++epIndex) {
		...
		if (((epDesc->bEndpointAddress & USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) ==
			...
			status = USB_HostOpenPipe(msdInstance->hostHandle, &msdInstance->inPipe,
						  &pipeInit);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
			...
			status = USB_HostOpenPipe(msdInstance->hostHandle, &msdInstance->outPipe,
						  &pipeInit);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("usb_host_hid_set_interface fail to open pipe\r\n");
...
usb_status_t USB_HostMsdInit(usb_device_handle deviceHandle, usb_host_class_handle *classHandle)
{
	...
	msdInstance->msdCommand.cbwBlock.CBWSignature = USB_LONG_TO_LITTLE_ENDIAN(
		USB_HOST_MSD_CBW_SIGNATURE);
	status = USB_HostMallocTransfer(msdInstance->hostHandle,
					&(msdInstance->msdCommand.transfer));
	if (status != kStatus_USB_Success) {
		msdInstance->msdCommand.transfer = NULL;
				     ...
				     void *			callbackParam)
{
	...
	if (msdInstance->outPipe != NULL) {
		status = USB_HostCancelTransfer(msdInstance->hostHandle, msdInstance->outPipe,
						NULL);
		if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
			usb_echo("error when cancel pipe\r\n");
	...
	if (alternateSetting == 0U) { /* open interface directly */
		...
		/* malloc one transfer */
		if (USB_HostMallocTransfer(msdInstance->hostHandle, &transfer) !=
		    kStatus_USB_Success) {
#ifdef HOST_ECHO
		...
		transfer->setupPacket->wIndex = USB_SHORT_TO_LITTLE_ENDIAN(
			((usb_host_interface_t *)msdInstance->interfaceHandle)->interfaceDesc->
			bInterfaceNumber);
		transfer->setupPacket->wValue = USB_SHORT_TO_LITTLE_ENDIAN(alternateSetting);
		transfer->setupPacket->wLength = 0;
		transfer->transferBuffer = NULL;
		transfer->transferLength = 0;
		status = USB_HostSendSetup(msdInstance->hostHandle, msdInstance->controlPipe,
					   transfer);
...
usb_status_t USB_HostMsdDeinit(usb_device_handle deviceHandle, usb_host_class_handle classHandle)
{
	...
	if (classHandle != NULL) {
		if (msdInstance->inPipe != NULL) {
			status = USB_HostCancelTransfer(msdInstance->hostHandle,
							msdInstance->inPipe, NULL);              /* cancel pipe */
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("error when cancel pipe\r\n");
#endif
		...
		if (msdInstance->outPipe != NULL) {
			status = USB_HostCancelTransfer(msdInstance->hostHandle,
							msdInstance->outPipe, NULL);              /* cancel pipe */
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("error when cancel pipe\r\n");
#endif
		...
		if ((msdInstance->controlPipe != NULL) &&
			...
			status =
				USB_HostCancelTransfer(msdInstance->hostHandle,
						       msdInstance->controlPipe,
						       msdInstance->controlTransfer);
			if (status != kStatus_USB_Success) {
#ifdef HOST_ECHO
				usb_echo("error when cancel pipe\r\n");
#endif
		...
		if (NULL != msdInstance->msdCommand.transfer) {
			(void)USB_HostFreeTransfer(msdInstance->hostHandle,
						   msdInstance->msdCommand.transfer);
		}
					 ...
					 void *			callbackParam)
{
	...
	return USB_HostMsdControl(msdInstance, USB_HostMsdMassResetCallback, callbackFn,
				  callbackParam, NULL, 0,
				  ...
				  void *		callbackParam)
{
	...
	return USB_HostMsdControl(
		msdInstance, USB_HostMsdControlCallback, callbackFn, callbackParam,
		logicalUnitNumber, 1,
