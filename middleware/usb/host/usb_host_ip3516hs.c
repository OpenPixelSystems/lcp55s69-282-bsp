...
static usb_status_t USB_HostIp3516HsGetBuffer(usb_host_ip3516hs_state_struct_t *usbHostState,
					      uint32_t length, uint32_t MaxPacketSize,
					      uint32_t *index, uint32_t *bufferLength);
 ...
 ******************************************************************************/
#if ((defined USB_HOST_CONFIG_COMPLIANCE_TEST) && (USB_HOST_CONFIG_COMPLIANCE_TEST))
static usb_status_t USB_HostIp3516HsTestSetMode(
...
static void USB_HostIp3516HsTestCallback(void *param, usb_host_transfer_t *transfer, usb_status_t
					 status)
{
...
static void USB_HostIp3516HsTestSingleStepGetDeviceDesc(
	usb_host_ip3516hs_state_struct_t *usbHostState,
	usb_device_handle
	deviceHandle)
{
	...
	usbHostState->usbRegBase->USBCMD &= ~(USB_HOST_IP3516HS_USBCMD_INT_EN_MASK |
					      USB_HOST_IP3516HS_USBCMD_ISO_EN_MASK);
...
static void USB_HostIp3516HsTestSingleStepGetDeviceDescData(
	usb_host_ip3516hs_state_struct_t *usbHostState,
	usb_device_handle
	deviceHandle)
{
...
static void USB_HostIp3516HsTestModeInit(usb_device_handle deviceHandle)
{
	...
	usb_host_device_instance_t *deviceInstance = (usb_host_device_instance_t *)deviceHandle;
	usb_host_ip3516hs_state_struct_t *usbHostState =
		(usb_host_ip3516hs_state_struct_t *)(((usb_host_instance_t *)(deviceInstance->
									      hostHandle))->
						     controllerHandle);

	(void)USB_HostHelperGetPeripheralInformation(deviceHandle, (uint32_t)kUSB_HostGetDevicePID,
						     &productId);
	...
	switch (productId) {
	case 0x0101U:
		(void)USB_HostIp3516HsTestSetMode(usbHostState,
						  USB_HOST_IP3516HS_PORTSC_PTC_SE0_NAK);
		break;
	case 0x0102U:
		(void)USB_HostIp3516HsTestSetMode(usbHostState,
						  USB_HOST_IP3516HS_PORTSC_PTC_J_STATE);
		break;
	case 0x0103U:
		(void)USB_HostIp3516HsTestSetMode(usbHostState,
						  USB_HOST_IP3516HS_PORTSC_PTC_K_STATE);
		break;
	case 0x0104U:
		(void)USB_HostIp3516HsTestSetMode(usbHostState,
						  USB_HOST_IP3516HS_PORTSC_PTC_PACKET);
		break;
	case 0x0105U:
		(void)usb_echo("set test mode FORCE_ENALBE\r\n");
		(void)USB_HostIp3516HsTestSetMode(usbHostState,
						  USB_HOST_IP3516HS_PORTSC_PTC_FORCE_ENABLE);
		break;
	case 0x0106U:
...
static void USB_HostIp3516HsSuspendBus(usb_host_ip3516hs_state_struct_t *usbHostState)
{
	...
	if (0U != (portStatus & USB_HOST_IP3516HS_PORTSC1_PED_MASK)) {
		...
		portStatus &= ~(USB_HOST_IP3516HS_PORTSC1_WIC |
				USB_HOST_IP3516HS_PORTSC1_SUS_L1_MASK);
...
static void USB_HostIp3516HsResumeBus(usb_host_ip3516hs_state_struct_t *usbHostState)
{
	...
	if (portStatus & USB_HOST_IP3516HS_PORTSC1_PED_MASK) {
		...
		portStatus |= USB_HOST_IP3516HS_PORTSC1_PP_MASK |
			      USB_HOST_IP3516HS_PORTSC1_PED_MASK;
		usbHostState->usbRegBase->PORTSC1 = portStatus;
	}
	USB_HostIp3516HsUnlock();
}
...
static void USB_HostIp3516HsDelay(usb_host_ip3516hs_state_struct_t *usbHostState, uint32_t ms)
{
	...
	sofStart = (uint32_t)((usbHostState->usbRegBase->FLADJ_FRINDEX &
			       USB_HOST_IP3516HS_FLADJ_FRINDEX_MASK) >>
			      USB_HOST_IP3516HS_FLADJ_FRINDEX_SHIFT);

	do{
		SofEnd = (uint32_t)((usbHostState->usbRegBase->FLADJ_FRINDEX &
				     USB_HOST_IP3516HS_FLADJ_FRINDEX_MASK) >>
				    USB_HOST_IP3516HS_FLADJ_FRINDEX_SHIFT);
		distance = (uint32_t)(
			(SofEnd - sofStart + (USB_HOST_IP3516HS_FLADJ_FRINDEX_MASK >>
					      USB_HOST_IP3516HS_FLADJ_FRINDEX_SHIFT) + 1U) &
			(USB_HOST_IP3516HS_FLADJ_FRINDEX_MASK >>
			 USB_HOST_IP3516HS_FLADJ_FRINDEX_SHIFT));
...
static usb_status_t USB_HostIp3516HsControlBus(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	uint8_t					busControl)
{
	...
	case kUSB_HostBusSuspend:
		if (0U != (usbHostState->usbRegBase->PORTSC1 &
			   USB_HOST_IP3516HS_PORTSC1_CCS_MASK)) {
			usb_host_instance_t *hostPointer =
				(usb_host_instance_t *)usbHostState->hostHandle;
	...
	case kUSB_HostBusResume:
		if (0U != (usbHostState->usbRegBase->PORTSC1 &
			...
			portStatus = usbHostState->usbRegBase->PORTSC1;
			portStatus &= ~(USB_HOST_IP3516HS_PORTSC1_WIC |
					USB_HOST_IP3516HS_PORTSC1_FPR_MASK);
			portStatus |= USB_HOST_IP3516HS_PORTSC1_PP_MASK |
				      USB_HOST_IP3516HS_PORTSC1_PED_MASK;
			usbHostState->usbRegBase->PORTSC1 = portStatus;
			/* call host callback function, function is initialized in USB_HostInit */
	...
	case kUSB_HostBusL1Sleep:
		if (0U != (usbHostState->usbRegBase->PORTSC1 &
			...
			portStatus = usbHostState->usbRegBase->USBCMD;
			portStatus &= ~(USB_HOST_IP3516HS_USBCMD_HIRD_MASK |
					USB_HOST_IP3516HS_USBCMD_LPM_RWU_MASK);
			portStatus |= (usbHostState->hirdValue <<
				       USB_HOST_IP3516HS_USBCMD_HIRD_SHIFT) |
				      (usbHostState->L1remoteWakeupEnable <<
				       USB_HOST_IP3516HS_USBCMD_LPM_RWU_SHIFT);
			usbHostState->usbRegBase->USBCMD = portStatus;
			portStatus = usbHostState->usbRegBase->PORTSC1;
			portStatus &= ~(USB_HOST_IP3516HS_PORTSC1_WIC |
					USB_HOST_IP3516HS_PORTSC1_SUS_L1_MASK |
			...
			usbHostState->usbRegBase->PORTSC1 |=
				USB_HOST_IP3516HS_PORTSC1_SUSP_MASK |
				USB_HOST_IP3516HS_PORTSC1_SUS_L1_MASK |
				((deviceInstance->setAddress <<
					USB_HOST_IP3516HS_PORTSC1_DEV_ADD_SHIFT) &
				 USB_HOST_IP3516HS_PORTSC1_DEV_ADD_MASK);
#if (defined(FSL_FEATURE_USBHSH_VERSION) && (FSL_FEATURE_USBHSH_VERSION >= 300U))
#else
			while ((0U == (usbHostState->usbRegBase->PORTSC1 &
				       USB_HOST_IP3516HS_PORTSC1_SUSP_MASK)) &&
			       (0U == (usbHostState->usbRegBase->PORTSC1 &
				       USB_HOST_IP3516HS_PORTSC1_SUS_STAT_MASK))) {
				__NOP();
			}
			...
			if ((0U != (usbHostState->usbRegBase->PORTSC1 &
				...
				/* call host callback function, function is initialized in USB_HostInit */
				(void)hostPointer->deviceCallback(hostPointer->suspendedDevice,
								  NULL,
								  kUSB_HostEventL1Sleeped); /* call host callback function */
			} else if (0x02U == ((portStatus &
					      USB_HOST_IP3516HS_PORTSC1_SUS_STAT_MASK) >>
					     USB_HOST_IP3516HS_PORTSC1_SUS_STAT_SHIFT)) {
				usbHostState->busSuspendStatus = (uint8_t)kBus_Ip3516HsIdle;
				/* call host callback function, function is initialized in USB_HostInit */
	...
	case kUSB_HostBusEnableAttach:
		if (0UL != (usbHostState->usbRegBase->HCSPARAMS &
			...
			usbHostState->usbRegBase->PORTSC1 = (tmp |
							     USB_HOST_IP3516HS_PORTSC1_PP_MASK); /* turn on port power */
		}
		(void)USB_HostIp3516HsControlBusReset(usbHostState);
		usbHostState->portState[0].portStatus = (uint8_t)kUSB_DeviceIp3516HsPortDetached;
		(void)OSA_EventSet(usbHostState->ip3516HsEvent,
				   USB_HOST_IP3516HS_EVENT_PORT_CHANGE);
		break;
...
#if (defined(USB_HOST_CONFIG_IP3516HS_MAX_ISO) && (USB_HOST_CONFIG_IP3516HS_MAX_ISO > 0U)) || \
	(defined(USB_HOST_CONFIG_IP3516HS_MAX_INT) && (USB_HOST_CONFIG_IP3516HS_MAX_INT > 0U))
static uint32_t USB_HostIp3516HsBusTime(uint8_t speed, uint8_t pipeType, uint8_t direction, uint32_t
					dataLength)
{
	...
	if (pipeType == USB_ENDPOINT_ISOCHRONOUS) { /* iso */
		...
		if (speed == USB_SPEED_HIGH) {
			...
			if (direction == USB_IN) {
				result = 64060000U + 2000U *
					 USB_HOST_IP3516HS_HUB_LS_SETUP_TIME_DELAY + 676670U *
					 result +
					 USB_HOST_IP3516HS_TIME_DELAY;
			} else {
				result = 64107000U + 2000U *
					 USB_HOST_IP3516HS_HUB_LS_SETUP_TIME_DELAY + 667000U *
					 result +
					 USB_HOST_IP3516HS_TIME_DELAY;
			}
		} else {
...
static usb_status_t USB_HostIp3516HsPortChange(usb_host_ip3516hs_state_struct_t *usbHostState)
{
	...
	for (i = 0U; i < usbHostState->portNumber; i++) {
		...
		if (0U != (portStatus & USB_HOST_IP3516HS_PORTSC1_CSC_MASK)) {
			...
			if ((uint8_t)kBus_Ip3516HsIdle != usbHostState->busSuspendStatus) {
				...
				usbHostState->usbRegBase->USBCMD |=
					USB_HOST_IP3516HS_USBCMD_RS_MASK;
				portStatus = usbHostState->usbRegBase->PORTSC1;
				portStatus &= ~(USB_HOST_IP3516HS_PORTSC1_WIC |
						USB_HOST_IP3516HS_PORTSC1_FPR_MASK);
				portStatus |= USB_HOST_IP3516HS_PORTSC1_PP_MASK |
...
#endif
			uint32_t sofStart =
				(uint32_t)((usbHostState->usbRegBase->FLADJ_FRINDEX &
					    USB_HOST_IP3516HS_FLADJ_FRINDEX_MASK) >>
					   USB_HOST_IP3516HS_FLADJ_FRINDEX_SHIFT);
			uint32_t sof;
			while (1U == 1U) {
				...
				if (0U != (portStatus & USB_HOST_IP3516HS_PORTSC1_CSC_MASK)) {
					portStatus &= ~USB_HOST_IP3516HS_PORTSC1_WIC;
					usbHostState->usbRegBase->PORTSC1 = portStatus |
									    USB_HOST_IP3516HS_PORTSC1_CSC_MASK;
				}
				sof = (uint32_t)((usbHostState->usbRegBase->FLADJ_FRINDEX &
						  USB_HOST_IP3516HS_FLADJ_FRINDEX_MASK) >>
						 USB_HOST_IP3516HS_FLADJ_FRINDEX_SHIFT);
				if ((((sof + USB_HOST_IP3516HS_FLADJ_FRINDEX_MASK + 1U) -
				      sofStart) &
		...
		if (0U != (portStatus & USB_HOST_IP3516HS_PORTSC1_CCS_MASK)) {
			uint32_t index;
#if (defined(USB_HOST_CONFIG_BATTERY_CHARGER) && (USB_HOST_CONFIG_BATTERY_CHARGER > 0U)) && \
			(defined(FSL_FEATURE_SOC_USBHSDCD_COUNT) && \
...
#if ((defined(USB_HOST_CONFIG_LOW_POWER_MODE)) && (USB_HOST_CONFIG_LOW_POWER_MODE > 0U))
				if (((uint8_t)kBus_Ip3516HsSuspended ==
				     usbHostState->busSuspendStatus)) {
					usb_host_instance_t *hostPointer =
						(usb_host_instance_t *)usbHostState->hostHandle;

					usbHostState->busSuspendStatus =
						(uint8_t)kBus_Ip3516HsStartResume;
					/* call host callback function, function is initialized in USB_HostInit */
					(void)hostPointer->deviceCallback(
						hostPointer->suspendedDevice, NULL,
					...
					usbHostState->usbRegBase->PORTSC1 = portStatus;
					usbHostState->matchTick = hostPointer->hwTick;
					while ((hostPointer->hwTick - usbHostState->matchTick) <=
					       20U) {
					}
					portStatus = usbHostState->usbRegBase->PORTSC1;
					portStatus &= ~(USB_HOST_IP3516HS_PORTSC1_WIC |
...
#if ((defined(USB_HOST_CONFIG_LPM_L1)) && (USB_HOST_CONFIG_LPM_L1 > 0U))
				if (((uint8_t)kBus_Ip3516HsL1Sleeped ==
				     usbHostState->busSuspendStatus)) {
					usb_host_instance_t *hostPointer =
						(usb_host_instance_t *)usbHostState->hostHandle;
					...
					while (0U != ((portStatus) &
						...
						portStatus = portStatus &
							     USB_HOST_IP3516HS_PORTSC1_FPR_MASK;
						if ((hostPointer->hwTick -
						     usbHostState->matchTick) >= 1U) {
							break;
						}
					}
...
#if (defined(FSL_FEATURE_USBHSH_VERSION) && (FSL_FEATURE_USBHSH_VERSION >= 300U))
				else if (((uint8_t)kBus_Ip3516HsL1StartSleep ==
					  usbHostState->busSuspendStatus)) {
					usb_host_instance_t *hostPointer =
						(usb_host_instance_t *)usbHostState->hostHandle;
					portStatus = usbHostState->usbRegBase->PORTSC1;

					if ((usbHostState->usbRegBase->PORTSC1 &
					     USB_HOST_IP3516HS_PORTSC1_SUSP_MASK) &&
					    (0x00U == ((portStatus &
							USB_HOST_IP3516HS_PORTSC1_SUS_STAT_MASK) >>
						       USB_HOST_IP3516HS_PORTSC1_SUS_STAT_SHIFT))) {
						...
						usbHostState->busSuspendStatus =
							(uint8_t)kBus_Ip3516HsL1Sleeped;
						/* call host callback function, function is initialized in USB_HostInit */
						(void)hostPointer->deviceCallback(
							hostPointer->suspendedDevice, NULL,
							kUSB_HostEventL1Sleeped);   /* call host callback function */
			...
			for (index = 0U; index < USB_HOST_IP3516HS_PORT_CONNECT_DEBOUNCE_DELAY;
			     index++) {
			...
			if ((0U == (portStatus & USB_HOST_IP3516HS_PORTSC1_CCS_MASK)) ||
				...
				usbHostState->portState[i].portStatus =
					(uint8_t)kUSB_DeviceIp3516HsPortDetached;
				continue;
			}
			...
			(defined(FSL_FEATURE_SOC_USBHSDCD_COUNT) && \
			(FSL_FEATURE_SOC_USBHSDCD_COUNT > 0U))
			USB_HSDCD_Control(usbHostState->dcdHandle, kUSB_HostHSDcdSetType,
					  &(chargerType));
#endif
			...
			usbHostState->portState[i].portSpeed =
				(uint8_t)((usbHostState->usbRegBase->PORTSC1 &
					   USB_HOST_IP3516HS_PORTSC1_PSPD_MASK) >>
			...
			if (0x00U == usbHostState->portState[i].portSpeed) {
				...
				USB_EhcihostPhyDisconnectDetectCmd(
					(uint8_t)kUSB_ControllerIp3516Hs0 +
					usbHostState->controllerId, 1U);
#endif
			}
			/* do bus recovery delay */
			USB_HostIp3516HsDelay(usbHostState,
					      USB_HOST_IP3516HS_PORT_RESET_RECOVERY_DELAY * 8U);
			usbHostState->portState[i].portStatus =
				(uint8_t)kUSB_DeviceIp3516HsPortPhyAttached;
			(void)OSA_EventSet(usbHostState->ip3516HsEvent,
					   USB_HOST_IP3516HS_EVENT_ATTACH);
		} else {
			if ((uint8_t)kUSB_DeviceIp3516HsPortDetached ==
			    usbHostState->portState[i].portStatus) {
				continue;
			}

			if ((uint8_t)kUSB_DeviceIp3516HsPortAttached ==
			    usbHostState->portState[i].portStatus) {
#if ((defined FSL_FEATURE_SOC_USBPHY_COUNT) && (FSL_FEATURE_SOC_USBPHY_COUNT > 0U))
				/* disable phy disconnection */
				USB_EhcihostPhyDisconnectDetectCmd(
					(uint8_t)kUSB_ControllerIp3516Hs0 +
					usbHostState->controllerId, 0U);
#endif
				usbHostState->portState[i].portStatus =
					(uint8_t)kUSB_DeviceIp3516HsPortPhyDetached;
#if (defined(USB_HOST_CONFIG_BATTERY_CHARGER) && (USB_HOST_CONFIG_BATTERY_CHARGER > 0U)) && \
				(defined(FSL_FEATURE_SOC_USBHSDCD_COUNT) && \
				(FSL_FEATURE_SOC_USBHSDCD_COUNT > 0U))
				USB_HSDCD_Control(usbHostState->dcdHandle, kUSB_HostHSDcdSetType,
						  &(usbHostState->chargerType));
#endif
				(void)OSA_EventSet(usbHostState->ip3516HsEvent,
						   USB_HOST_IP3516HS_EVENT_DETACH);
			} else {
				usbHostState->portState[i].portStatus =
					(uint8_t)kUSB_DeviceIp3516HsPortDetached;
			}
...
#if (defined(USB_HOST_CONFIG_IP3516HS_MAX_ISO) && (USB_HOST_CONFIG_IP3516HS_MAX_ISO > 0U)) || \
	(defined(USB_HOST_CONFIG_IP3516HS_MAX_INT) && (USB_HOST_CONFIG_IP3516HS_MAX_INT > 0U))
static usb_status_t USB_HostIp3516HsFillSlotBusTime(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	usb_host_ip3516hs_pipe_struct_t *	pipe,
	...
	*csSlot = 0U;
	...
	if (USB_ENDPOINT_ISOCHRONOUS == pipe->pipeCommon.pipeType) {
		if (USB_OUT == pipe->pipeCommon.direction) {
			...
			slotTime[startSlot + slots - 1U] =
				(uint8_t)(((pipe->pipeCommon.maxPacketSize % 188U) * 125U) / 188U);
		} else {
			...
			*ssSlot = 1U << startSlot;
			...
			slotTime[startSlot + 1U + slots - 1U] =
				(uint8_t)(((pipe->pipeCommon.maxPacketSize % 188U) * 125U) / 188U);
		}
...
static usb_status_t USB_HostIp3516HsFindStartFrame(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	usb_host_ip3516hs_pipe_struct_t *	pipe)
{
	...
	for (startUFrame = 0U; startUFrame < ((uint32_t)pipe->pipeCommon.interval); startUFrame++) {
		for (frame = startUFrame; frame < USB_HOST_IP3516HS_MAX_UFRAME; frame +=
			...
			while (NULL != temp) {
				p = (usb_host_ip3516hs_pipe_struct_t *)temp;
				temp = (void *)p->pipeCommon.next;
				if ((frame >= p->startUFrame) && ((p->pipeCommon.pipeType ==
								   USB_ENDPOINT_ISOCHRONOUS) ||
								  (p->pipeCommon.pipeType ==
								   USB_ENDPOINT_INTERRUPT))) {
					if (USB_SPEED_HIGH == speed) {
						if (0U == ((frame - p->startUFrame) %
							   p->pipeCommon.interval)) {
							total += p->busHsTime;
						} else {
						}
					} else {
		...
		do{
			if ((frame >= USB_HOST_IP3516HS_MAX_UFRAME) && (USB_SPEED_HIGH != speed)) {
				...
				error = USB_HostIp3516HsFillSlotBusTime(usbHostState, pipe,
									startUFrame, slotTime,
									&pipe->ssSlot,
									&pipe->csSlot);
				...
				(void)USB_HostHelperGetPeripheralInformation(
					pipe->pipeCommon.deviceHandle,
					(uint32_t)
					kUSB_HostGetDeviceHSHubNumber, &hsHubNumber);
				for (frame = startUFrame; frame < USB_HOST_IP3516HS_MAX_UFRAME;
				     frame += pipe->pipeCommon.interval) {
					total = 0U;
					temp = (void *)usbHostState->pipeListInUsing;
					while (NULL != temp) {
						p = (usb_host_ip3516hs_pipe_struct_t *)temp;
						temp = (void *)p->pipeCommon.next;
						if ((frame >= p->startUFrame) &&
						    ((p->pipeCommon.pipeType ==
						      USB_ENDPOINT_ISOCHRONOUS) ||
						     (p->pipeCommon.
						      pipeType == USB_ENDPOINT_INTERRUPT))) {
							uint32_t deviceHsHubNumber = 0U;
							(void)USB_HostHelperGetPeripheralInformation(
								p->pipeCommon.deviceHandle,
								(
									uint32_t)
							...
							if ((deviceHsHubNumber == hsHubNumber) &&
							    (0U == (((frame & 0xFFFFFFF8U) -
								     (p->startUFrame & 0xFFF8U)) %
								    p->pipeCommon.interval))) {
								uint8_t ss;
								uint8_t cs;
								(void)
								USB_HostIp3516HsFillSlotBusTime(
									usbHostState, pipe,
									startUFrame,
									deviceSlotTime,
									&
					...
					for (uint32_t i = 0; i < 8U; i++) {
						if ((totalSlotTime[i] + slotTime[i]) >
						    s_SlotMaxBandwidth[i]) {
							break;
						}
					}
				}
...
static usb_status_t USB_HostIp3516BaudWidthCheck(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	usb_host_ip3516hs_pipe_struct_t *	pipe)
{
	usb_status_t error;
	...
	pipe->busHsTime = (uint16_t)USB_HostIp3516HsBusTime(
		...
		((uint32_t)pipe->pipeCommon.maxPacketSize) *
		((uint32_t)pipe->pipeCommon.numberPerUframe));

	if (USB_SPEED_HIGH != speed) {
		uint32_t thinkTime;
		pipe->busNoneHsTime = (uint16_t)USB_HostIp3516HsBusTime(
			speed, pipe->pipeCommon.pipeType, pipe->pipeCommon.direction,
...
static usb_status_t USB_HostIp3516HsOpenControlBulkPipe(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	usb_host_ip3516hs_pipe_struct_t *	pipe)
{
	...
	OSA_ENTER_CRITICAL();
	for (uint8_t i = 0;
	     i < (sizeof(s_UsbHostIp3516HsPtd[usbHostState->controllerId].atl) /
		  sizeof(usb_host_ip3516hs_atl_struct_t));
	     i++) {
		if (0U ==
		    s_UsbHostIp3516HsPtd[usbHostState->controllerId].atl[i].control1Union.
		    stateBitField.
		    MaxPacketLength) {
			pipe->tdIndex = i;
			s_UsbHostIp3516HsPtd[usbHostState->controllerId].atl[i].control1Union.
			stateBitField.MaxPacketLength =
				pipe->pipeCommon.maxPacketSize;
			break;
		}
	}
	...
	if (USB_SPEED_HIGH == usbHostState->portState[0].portSpeed) {
		atl->control2Union.stateBitField.S =
			(USB_SPEED_HIGH ==
			 ((usb_host_device_instance_t *)pipe->pipeCommon.deviceHandle)->speed) ?
			0U : 1U;
	} else {
	...
	atl->control2Union.stateBitField.SE =
		(USB_SPEED_LOW ==
		 ((usb_host_device_instance_t *)pipe->pipeCommon.deviceHandle)->speed) ? 2U : 0U;
...
#if (defined(USB_HOST_CONFIG_IP3516HS_MAX_ISO) && (USB_HOST_CONFIG_IP3516HS_MAX_ISO > 0U))
static usb_status_t USB_HostIp3516HsOpenIsoPipe(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	usb_host_ip3516hs_pipe_struct_t *	pipe)
{
	...
	for (uint32_t i = 0U; i < USB_HOST_CONFIG_IP3516HS_MAX_ISO; i++) {
		for (uint32_t j = i;
		     (j < (USB_HOST_IP3516HS_ISO_MULTIPLE_TRANSFER + i)) && (j <
									     USB_HOST_CONFIG_IP3516HS_MAX_ISO);
		     j++) {
			if (0U ==
			    s_UsbHostIp3516HsPtd[usbHostState->controllerId].iso[j].control1Union.
			    stateBitField.MaxPacketLength) {
				count++;
			}
		}
		if (count >= USB_HOST_IP3516HS_ISO_MULTIPLE_TRANSFER) {
			...
			for (uint32_t j = i;
			     (j < (USB_HOST_IP3516HS_ISO_MULTIPLE_TRANSFER + i)) && (j <
										     USB_HOST_CONFIG_IP3516HS_MAX_ISO);
			     j++) {
				s_UsbHostIp3516HsPtd[usbHostState->controllerId].iso[j].
	...
	for (uint32_t j = pipe->tdIndex; (j < ((uint32_t)USB_HOST_IP3516HS_ISO_MULTIPLE_TRANSFER +
					       pipe->tdIndex)) &&
		...
		if (USB_SPEED_HIGH == usbHostState->portState[0].portSpeed) {
			ptl->control2Union.stateBitField.S =
				(USB_SPEED_HIGH ==
				 ((usb_host_device_instance_t *)pipe->pipeCommon.deviceHandle)->
				 speed) ? 0U :
				1U;
		} else {
...
static usb_status_t USB_HostIp3516HsOpenInterruptPipe(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	usb_host_ip3516hs_pipe_struct_t *	pipe)
{
	usb_host_ip3516hs_ptl_struct_t *ptl;
	...
	for (uint8_t i = 0; i < USB_HOST_CONFIG_IP3516HS_MAX_INT; i++) {
		if (0U ==
		    s_UsbHostIp3516HsPtd[usbHostState->controllerId].interrupt[i].control1Union.
		    stateBitField.MaxPacketLength) {
			pipe->tdIndex = i;
			s_UsbHostIp3516HsPtd[usbHostState->controllerId].interrupt[i].control1Union.
			stateBitField.MaxPacketLength =
...
static usb_status_t USB_HostIp3516HsCancelPipe(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	usb_host_ip3516hs_pipe_struct_t *	pipe,
	usb_host_transfer_t *			tr)
{
	...
	case USB_ENDPOINT_ISOCHRONOUS:
		usbHostState->usbRegBase->ISO_PTD_SKIP_MAP |= ((1UL <<
								USB_HOST_IP3516HS_ISO_MULTIPLE_TRANSFER)
							       - 1U)
							      << pipe->tdIndex;
		break;
#endif
	...
	startUFrame = (uint32_t)((usbHostState->usbRegBase->FLADJ_FRINDEX &
				  USB_HOST_IP3516HS_FLADJ_FRINDEX_MASK) >>
				 USB_HOST_IP3516HS_FLADJ_FRINDEX_SHIFT);
	currentUFrame = startUFrame;

	while (currentUFrame == startUFrame) {
		currentUFrame = (uint32_t)((usbHostState->usbRegBase->FLADJ_FRINDEX &
					    USB_HOST_IP3516HS_FLADJ_FRINDEX_MASK) >>
					   USB_HOST_IP3516HS_FLADJ_FRINDEX_SHIFT);
	}
	...
	while (NULL != trCurrent) {
		...
		if ((NULL == tr) || (tr == trCurrent)) {
			...
			case USB_ENDPOINT_BULK:
			case USB_ENDPOINT_CONTROL:
				trCurrent->transferSofar +=
					s_UsbHostIp3516HsPtd[usbHostState->controllerId]
					.atl[pipe->tdIndex]
					.stateUnion.stateBitField.
					NrBytesToTransfer;
				if (1U ==
				    s_UsbHostIp3516HsPtd[usbHostState->controllerId].atl[pipe->
											 tdIndex].
					...
					if (((trCurrent->transferSofar !=
					      trCurrent->transferLength) ||
					     (pipe->pipeCommon.pipeType == USB_ENDPOINT_CONTROL)) &&
					    (kStatus_USB_Success == trStatus)) {
						if (kStatus_USB_Success ==
						    (usb_status_t)trCurrent->union1.transferResult)
						{
							trStatus = kStatus_USB_TransferCancel;
						} else {
							trStatus = kStatus_USB_TransferFailed;
							trCurrent->union1.transferResult =
								(int32_t)kStatus_USB_Success;
						}
					}
				}
				s_UsbHostIp3516HsPtd[usbHostState->controllerId].atl[pipe->tdIndex].
			...
			if (0UL != pipe->bufferLength) {
				(void)USB_HostIp3516HsFreeBuffer(usbHostState, pipe->bufferIndex,
								 pipe->bufferLength);
...
static usb_status_t USB_HostIp3516HsCloseControlBulkPipe(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	usb_host_ip3516hs_pipe_struct_t *	pipe)
{
	OSA_SR_ALLOC();
	...
	/* Enter critical */
	OSA_ENTER_CRITICAL();
	s_UsbHostIp3516HsPtd[usbHostState->controllerId].atl[pipe->tdIndex].control1Union.
	stateBitField.V = 0U;
	s_UsbHostIp3516HsPtd[usbHostState->controllerId].atl[pipe->tdIndex].control1Union.
	stateBitField.MaxPacketLength =
		0U;
...
#if ((defined(USB_HOST_CONFIG_IP3516HS_MAX_ISO)) && (USB_HOST_CONFIG_IP3516HS_MAX_ISO > 0U))
static usb_status_t USB_HostIp3516HsCloseIsoPipe(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	usb_host_ip3516hs_pipe_struct_t *	pipe)
{
	OSA_SR_ALLOC();
	...
	for (uint32_t i = 0; i < USB_HOST_IP3516HS_ISO_MULTIPLE_TRANSFER; i++) {
		s_UsbHostIp3516HsPtd[usbHostState->controllerId].iso[pipe->tdIndex +
								     i].control1Union.stateBitField.
		V = 0U;
		s_UsbHostIp3516HsPtd[usbHostState->controllerId]
		.iso[pipe->tdIndex + i]
...
#if ((defined(USB_HOST_CONFIG_IP3516HS_MAX_INT)) && (USB_HOST_CONFIG_IP3516HS_MAX_INT > 0U))
static usb_status_t USB_HostIp3516HsCloseInterruptPipe(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	usb_host_ip3516hs_pipe_struct_t *	pipe)
{
	OSA_SR_ALLOC();
	...
	/* Enter critical */
	OSA_ENTER_CRITICAL();
	s_UsbHostIp3516HsPtd[usbHostState->controllerId].interrupt[pipe->tdIndex].control1Union.
	stateBitField.V = 0U;
	s_UsbHostIp3516HsPtd[usbHostState->controllerId]
	.interrupt[pipe->tdIndex]
	.control1Union.stateBitField.MaxPacketLength = 0U;
	...
	if (0xFFFFFFFFU != startIndex) {
		...
		*bufferLength = freeBufferSize;
		for (; startIndex <= i; startIndex++) {
			usbHostState->bufferArrayBitMap[startIndex >> 5U] |= (1UL << (startIndex %
										      32U));
		}
		OSA_EXIT_CRITICAL();
		return kStatus_USB_Success;
...
static usb_status_t USB_HostIp3516HsFreeBuffer(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	uint32_t				index,
	uint32_t				bufferLength)
{
	...
	if ((indexCount + index) > (sizeof(s_UsbHostIp3516HsBufferArray) / (sizeof(uint8_t) *
									    64U))) {
		indexCount = (sizeof(s_UsbHostIp3516HsBufferArray) / (sizeof(uint8_t) * 64U)) -
			     index;
	}
	...
	usb_host_transfer_t *			tr)
{
	...
	case kStatus_UsbHostIp3516Hs_Idle:
		if (0U != pipe->bufferLength) {
			bufferAddress =
				(uint8_t *)&s_UsbHostIp3516HsBufferArray[pipe->bufferIndex][0];
		} else {
			transferLength = pipe->pipeCommon.maxPacketSize;
			if (kStatus_USB_Success == USB_HostIp3516HsGetBuffer(usbHostState,
									     transferLength,
									     pipe->pipeCommon.
									     maxPacketSize,
									     &pipe->bufferIndex,
									     &pipe->bufferLength)) {
				bufferAddress =
					(uint8_t *)&s_UsbHostIp3516HsBufferArray[pipe->bufferIndex][
						0];
			} else {
				OSA_ENTER_CRITICAL();
				pipe->isBusy = 2U;
		...
		atl->dataUnion.dataBitField.DataStartAddress = ((uint32_t)(bufferAddress)) &
							       0x0000FFFFU;
		atl->dataUnion.dataBitField.I = 1U;
	...
	case kStatus_UsbHostIp3516Hs_Setup:
		...
		if (0U != transferLength) {
			if (USB_IN == tr->direction) {
				atl->stateUnion.stateBitField.Token =
					USB_HOST_IP3516HS_PTD_TOKEN_IN;
				atl->stateUnion.stateBitField.P = 0x00U;
			} else {
				atl->stateUnion.stateBitField.Token =
					USB_HOST_IP3516HS_PTD_TOKEN_OUT;
			}
...
static usb_status_t USB_HostIp3516HsWriteBulkPipe(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	usb_host_ip3516hs_pipe_struct_t *	pipe,
	usb_host_transfer_t *			tr)
{
	...
	if ((USB_OUT == tr->direction) && (transferLength > 0U)) {
		(void)memcpy((void *)bufferAddress,
			     (void *)(&tr->transferBuffer[tr->transferSofar]), transferLength);
	}
...
#if ((defined(USB_HOST_CONFIG_IP3516HS_MAX_ISO)) && (USB_HOST_CONFIG_IP3516HS_MAX_ISO > 0U))
static usb_status_t USB_HostIp3516HsWriteIsoPipe(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	usb_host_ip3516hs_pipe_struct_t *	pipe,
	usb_host_transfer_t *			tr)
{
	...
	while (NULL != currentTr) {
		...
		if (0U == indexLength.indexLength) {
			...
			for (uint32_t i = 0U; i < (USB_HOST_IP3516HS_ISO_MULTIPLE_TRANSFER); i++) {
				ptl =
					&s_UsbHostIp3516HsPtd[usbHostState->controllerId].iso[pipe->
											      tdIndex
											      + i];
			...
			if (kStatus_USB_Success == USB_HostIp3516HsGetBuffer(usbHostState,
				...
				bufferAddress =
					(uint8_t *)&s_UsbHostIp3516HsBufferArray[indexLength.state.
										 bufferIndex][0];
			} else {
				break;
			}
		...
		if (USB_OUT == currentTr->direction) {
			for (uint32_t i = 0; i < transferLength; i++) {
				bufferAddress[i] =
					currentTr->transferBuffer[currentTr->transferSofar + i];
			}
		}
		...
		ptl->dataUnion.dataBitField.DataStartAddress = ((uint32_t)(bufferAddress)) &
							       0x0000FFFFU;
		ptl->dataUnion.dataBitField.I = 1U;
		...
		primedUFrame = ((uint32_t)pipe->lastPrimedIndex + pipe->pipeCommon.interval) &
			       0xFFU;
		...
		if (primedUFrame >= (currentUFrame + USB_HOST_IP3516HS_PERIODIC_TRANSFER_GAP)) {
			...
			insertUFrame =
				pipe->startUFrame + ((currentUFrame - pipe->startUFrame +
						      USB_HOST_IP3516HS_PERIODIC_TRANSFER_GAP +
		...
		if (USB_SPEED_HIGH == speed) {
			...
			for (uint16_t i = (uint16_t)(insertUFrame & 0x07UL); (i < 8U); i +=
				     pipe->pipeCommon.interval) {
				primedLength += (uint32_t)pipe->pipeCommon.maxPacketSize *
						pipe->pipeCommon.numberPerUframe;
			...
			if (0U != (ptl->control2Union.stateBitField.S)) {
				...
				if (USB_IN == currentTr->direction) {
					if (transferLength <= 192U) {
						sptl->control1Union.stateBitField.MaxPacketLength =
							transferLength;
					} else {
						sptl->control1Union.stateBitField.MaxPacketLength =
							192U;
...
#if ((defined(USB_HOST_CONFIG_IP3516HS_MAX_INT)) && (USB_HOST_CONFIG_IP3516HS_MAX_INT > 0U))
static usb_status_t USB_HostIp3516HsWriteInterruptPipe(
	usb_host_ip3516hs_state_struct_t *	usbHostState,
	usb_host_ip3516hs_pipe_struct_t *	pipe,
	usb_host_transfer_t *			tr)
{
	usb_host_ip3516hs_ptl_struct_t *ptl =
		&s_UsbHostIp3516HsPtd[usbHostState->controllerId].interrupt[pipe->tdIndex];
	usb_host_ip3516hs_sptl_struct_t *sptl;
	uint32_t transferLength;
	uint32_t currentUFrame;
	uint32_t insertUFrame;
	...
	insertUFrame = pipe->startUFrame + ((currentUFrame - pipe->startUFrame +
					     USB_HOST_IP3516HS_PERIODIC_TRANSFER_GAP +
					     pipe->pipeCommon.interval - 1U) &
					    (~(pipe->pipeCommon.interval - 1U)));
...
static usb_host_ip3516hs_pipe_struct_t *USB_HostIp3516HsGetPipe(
	usb_host_ip3516hs_state_struct_t *usbHostState,
	uint8_t
	pipeType,
	uint8_t
	tdIndex)
{
	...
	while (p != NULL) {
		...
		if ((p->pipeCommon.pipeType == pipeType) && (USB_ENDPOINT_ISOCHRONOUS ==
							     pipeType) && (p->tdIndex >= tdIndex) &&
...
#if ((defined(USB_HOST_CONFIG_IP3516HS_MAX_ISO)) && (USB_HOST_CONFIG_IP3516HS_MAX_ISO > 0U))
static usb_status_t USB_HostIp3516HsCheckIsoTransferSofar(
	usb_host_ip3516hs_state_struct_t *usbHostState)
{
	...
	for (uint8_t i = 0; i < 32U; i++) {
		if (0U != (tokenDoneBits & (1UL << i))) {
			if ((0U !=
			     s_UsbHostIp3516HsPtd[usbHostState->controllerId].iso[i].stateUnion.
			     stateBitField.A) &&
			    (0U !=
			     s_UsbHostIp3516HsPtd[usbHostState->controllerId].iso[i].control1Union.
			     stateBitField.V)) {
				continue;
			}
			...
			if (NULL != p) {
				...
				if (NULL != p->trList) {
					...
					if ((s_UsbHostIp3516HsPtd[usbHostState->controllerId].iso[p
												  ->
												  tdIndex
					     ].stateUnion.stateBitField.B) ||
					    (s_UsbHostIp3516HsPtd[usbHostState->controllerId].iso[p
												  ->
												  tdIndex
					     ].stateUnion.stateBitField.X)) {
						trCurrent->union1.transferResult =
					...
					if (USB_IN == trCurrent->direction) {
						memcpy(
							&trCurrent->transferBuffer[trCurrent->
										   transferSofar],
							&s_UsbHostIp3516HsBufferArray[p->bufferIndex
							][0],
							s_UsbHostIp3516HsPtd[usbHostState->
									     controllerId].iso
							[p->tdIndex].stateUnion.stateBitField.
							NrBytesToTransfer);
					}
					trCurrent->transferSofar +=
						s_UsbHostIp3516HsPtd[usbHostState->controllerId]
					...
					if (p->currentTr) {
						...
						USB_HostIp3516HsWriteIsoPipe(usbHostState, p,
									     (usb_host_transfer_t *)
									     trValue);
					}
				}
...
#if ((defined(USB_HOST_CONFIG_IP3516HS_MAX_INT)) && (USB_HOST_CONFIG_IP3516HS_MAX_INT > 0U))
static usb_status_t USB_HostIp3516HsCheckIntTransferSofar(
	usb_host_ip3516hs_state_struct_t *usbHostState)
{
	...
	for (uint8_t i = 0; i < 32U; i++) {
		if (0U != (tokenDoneBits & (1UL << i))) {
			if ((0U !=
			     s_UsbHostIp3516HsPtd[usbHostState->controllerId].interrupt[i].
			     stateUnion.
			     stateBitField.A) &&
			    (0U !=
			     s_UsbHostIp3516HsPtd[usbHostState->controllerId].interrupt[i].
...
static usb_status_t USB_HostIp3516HsCheckAtlTransferSofar(
	usb_host_ip3516hs_state_struct_t *usbHostState)
{
	...
	while (NULL != pipe) {
		if (2U == pipe->isBusy) {
			if (NULL != pipe->trList) {
				switch (pipe->pipeCommon.pipeType) {
				case USB_ENDPOINT_BULK:
					(void)USB_HostIp3516HsWriteBulkPipe(usbHostState, pipe,
									    pipe->trList);
					break;

				case USB_ENDPOINT_CONTROL:
					(void)USB_HostIp3516HsWriteControlPipe(usbHostState, pipe,
									       pipe->trList);
					break;

#if ((defined(USB_HOST_CONFIG_IP3516HS_MAX_ISO)) && (USB_HOST_CONFIG_IP3516HS_MAX_ISO > 0U))
				case USB_ENDPOINT_ISOCHRONOUS:
					(void)USB_HostIp3516HsWriteIsoPipe(usbHostState, pipe,
									   pipe->trList);
					break;
#endif
...
#if ((defined(USB_HOST_CONFIG_IP3516HS_MAX_INT)) && (USB_HOST_CONFIG_IP3516HS_MAX_INT > 0U))
				case USB_ENDPOINT_INTERRUPT:
					(void)USB_HostIp3516HsWriteInterruptPipe(usbHostState, pipe,
										 pipe->trList);
					break;
#endif
	...
	while (NULL != pipe) {
		...
		if (NULL == trCurrent) {
			...
			case USB_ENDPOINT_CONTROL:
				...
				transferStatus =
					(usb_host_ip3516hs_transfer_status_t)trCurrent->setupStatus;
				...
				case kStatus_UsbHostIp3516Hs_Data2:
					if ((0U != s_UsbHostIp3516HsPtd[usbHostState->controllerId]
						...
						trCurrent->setupStatus =
							(uint8_t)kStatus_UsbHostIp3516Hs_Data;
					}
					if (USB_IN == trCurrent->direction) {
						(void)memcpy(
							(void *)(&trCurrent->transferBuffer[
									 trCurrent->
									 transferSofar
								 ]),
							(void *)(&s_UsbHostIp3516HsBufferArray[
									 pipe->bufferIndex][0]),
							s_UsbHostIp3516HsPtd[usbHostState->
									     controllerId]
							.atl[pipe->tdIndex]
...
#endif
						(void)USB_HostIp3516HsWriteControlPipe(usbHostState,
										       pipe,
										       pipe->trList);
					}
				}
			...
			case USB_ENDPOINT_BULK:
				...
				if (USB_IN == trCurrent->direction) {
					uint32_t length =
						...
						NrBytesToTransfer;
					(void)memcpy(
						(void *)(&trCurrent->transferBuffer[trCurrent->
										    transferSofar
							 ]),
						(void *)(&s_UsbHostIp3516HsBufferArray[pipe->
										       bufferIndex][
								 0]), length);
				}
				trCurrent->transferSofar +=
				...
				if (0U !=
					...
					if (0U != trDone) {
						if ((0U == (pipe->trList->transferLength %
							    pipe->pipeCommon.maxPacketSize)) &&
						    (pipe->trList->transferSofar <
						     pipe->trList->transferLength)) {
							(void)USB_HostIp3516HsWriteBulkPipe(
								usbHostState, pipe, pipe->trList);
						}
					}
				}
			...
			case USB_ENDPOINT_ISOCHRONOUS:
				...
				if ((0U != s_UsbHostIp3516HsPtd[usbHostState->controllerId]
				    ...
				    (0U != s_UsbHostIp3516HsPtd[usbHostState->controllerId]
				     ...
				     .stateUnion.stateBitField.X)) {
					trCurrent->union1.transferResult =
						(int32_t)kStatus_USB_TransferFailed;
				} else {
					trCurrent->union1.transferResult =
						(int32_t)kStatus_USB_Success;
				}
			...
			if (0U != trDone) {
...
#endif
				{
					(void)USB_HostIp3516HsFreeBuffer(usbHostState,
									 pipe->bufferIndex,
									 pipe->bufferLength);
					pipe->bufferLength = 0U;
				}
				...
				if ((kStatus_USB_Success == (trStatus)) && (USB_ENDPOINT_CONTROL ==
				    ...
				    (USB_REQUEST_STANDARD_CLEAR_FEATURE ==
				     trCurrent->setupPacket->bRequest) &&
				    (USB_REQUEST_TYPE_RECIPIENT_ENDPOINT ==
				     trCurrent->setupPacket->bmRequestType) &&
				    (USB_REQUEST_STANDARD_FEATURE_SELECTOR_ENDPOINT_HALT ==
				     (USB_SHORT_FROM_LITTLE_ENDIAN(trCurrent->setupPacket->wValue) &
				      0x00FFU))) {
					p = usbHostState->pipeListInUsing;
					while (p != NULL) {
						/* only compute bulk and interrupt pipe */
						if ((p->pipeCommon.deviceHandle ==
						     pipe->pipeCommon.deviceHandle) &&
						    ((p->pipeCommon.endpointAddress |
						      (p->pipeCommon.direction <<
							USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT))
						     ==
						     ((uint8_t)(USB_SHORT_FROM_LITTLE_ENDIAN(
									trCurrent->setupPacket->
									wIndex))))) {
							break;
						}
						temp = (void *)p->pipeCommon.next;
					...
					if ((NULL != p) && ((USB_ENDPOINT_BULK ==
							     p->pipeCommon.pipeType) ||
							    (USB_ENDPOINT_INTERRUPT ==
							     p->pipeCommon.pipeType))) {
						if (USB_ENDPOINT_BULK == p->pipeCommon.pipeType) {
							usbHostState->usbRegBase->ATL_PTD_SKIP_MAP
								|= 1UL << p->tdIndex;
						}
#if ((defined(USB_HOST_CONFIG_IP3516HS_MAX_INT)) && (USB_HOST_CONFIG_IP3516HS_MAX_INT > 0U))
						else if (USB_ENDPOINT_INTERRUPT ==
							 p->pipeCommon.pipeType) {
							usbHostState->usbRegBase->INT_PTD_SKIP_MAP
								|= 1UL << p->tdIndex;
						}
#endif
						else {
						...
						startUFrame = (uint32_t)(
							(usbHostState->usbRegBase->FLADJ_FRINDEX &
							 USB_HOST_IP3516HS_FLADJ_FRINDEX_MASK) >>
							USB_HOST_IP3516HS_FLADJ_FRINDEX_SHIFT);
						currentUFrame = startUFrame;
				...
				trCurrent->callbackFn(trCurrent->callbackParam, trCurrent,
						      trStatus);                      /* transfer callback */
				USB_HostIp3516HsCheckGetBufferFailedPipe(usbHostState);
				if (NULL != pipe->trList) {
					switch (pipe->pipeCommon.pipeType) {
					case USB_ENDPOINT_BULK:
						(void)USB_HostIp3516HsWriteBulkPipe(usbHostState,
										    pipe,
										    pipe->trList);
						break;
...
#if ((defined(USB_HOST_CONFIG_IP3516HS_MAX_ISO)) && (USB_HOST_CONFIG_IP3516HS_MAX_ISO > 0U))
					case USB_ENDPOINT_ISOCHRONOUS:
						(void)USB_HostIp3516HsWriteIsoPipe(usbHostState,
										   pipe,
										   pipe->trList);
						break;
#endif
...
#if ((defined(USB_HOST_CONFIG_IP3516HS_MAX_INT)) && (USB_HOST_CONFIG_IP3516HS_MAX_INT > 0U))
					case USB_ENDPOINT_INTERRUPT:
						(void)USB_HostIp3516HsWriteInterruptPipe(
							usbHostState, pipe, pipe->trList);
						break;
#endif
...
static usb_status_t USB_HostIp3516HsSof(usb_host_ip3516hs_state_struct_t *usbHostState)
{
	...
	pipe = usbHostState->pipeListInUsing;
	while (NULL != pipe) {
		if ((USB_ENDPOINT_CONTROL == pipe->pipeCommon.pipeType) || (USB_ENDPOINT_BULK ==
									    pipe->pipeCommon.
									    pipeType)) {
			if (NULL != pipe->trList) {
				pipe->cutOffTime--;
				if (0U == pipe->cutOffTime) {
					pipe->trList->union1.transferResult =
						(int)kStatus_USB_TransferFailed;
					(void)USB_HostIp3516HsCancelPipe(usbHostState, pipe,
									 pipe->trList);
				}
			}
		}
...
static usb_status_t USB_HostIp3516HsControllerInit(usb_host_ip3516hs_state_struct_t *usbHostState)
{
	...
	for (i = 0U;
	     i < (sizeof(s_UsbHostIp3516HsPtd[usbHostState->controllerId].atl) /
		  sizeof(usb_host_ip3516hs_atl_struct_t));
	     i++) {
		s_UsbHostIp3516HsPtd[usbHostState->controllerId].atl[i].control1Union.controlState =
			0U;
		s_UsbHostIp3516HsPtd[usbHostState->controllerId].atl[i].control2Union.controlState =
			0U;
	...
	usbHostState->usbRegBase->LAST_PTD_IN_USE = (i - 1U) &
						    USB_HOST_IP3516HS_LAST_PTD_IN_USE_ATL_LAST_MASK;
	...
	for (i = 0U; i < USB_HOST_CONFIG_IP3516HS_MAX_ISO; i++) {
		s_UsbHostIp3516HsPtd[usbHostState->controllerId].iso[i].control1Union.controlState =
			0U;
		s_UsbHostIp3516HsPtd[usbHostState->controllerId].iso[i].control2Union.controlState =
			0U;
	...
	usbHostState->usbRegBase->LAST_PTD_IN_USE |=
		((USB_HOST_CONFIG_IP3516HS_MAX_ISO - 1UL) <<
			USB_HOST_IP3516HS_LAST_PTD_IN_USE_ISO_LAST_SHIFT) &
	...
	for (i = 0U; i < USB_HOST_CONFIG_IP3516HS_MAX_INT; i++) {
		s_UsbHostIp3516HsPtd[usbHostState->controllerId].interrupt[i].control1Union.
		controlState = 0U;
		s_UsbHostIp3516HsPtd[usbHostState->controllerId].interrupt[i].control2Union.
		controlState = 0U;
	...
	usbHostState->usbRegBase->LAST_PTD_IN_USE |=
		(((uint32_t)USB_HOST_CONFIG_IP3516HS_MAX_INT - 1U) <<
			USB_HOST_IP3516HS_LAST_PTD_IN_USE_INT_LAST_SHIFT) &
	...
	usbHostState->usbRegBase->USBINTR =
		interruptState | USB_HOST_IP3516HS_USBINTR_PCDE_MASK |
		USB_HOST_IP3516HS_USBINTR_SOF_E_MASK;
				    ...
				    usb_host_controller_handle *controllerHandle)
{
	...
	if (((controllerId - (uint8_t)kUSB_ControllerIp3516Hs0) >=
	     (uint8_t)USB_HOST_CONFIG_IP3516HS)
#if (defined(USB_HOST_CONFIG_IP3516HS) && (USB_HOST_CONFIG_IP3516HS > 1U))
	    ||
	    ((controllerId - (uint8_t)kUSB_ControllerIp3516Hs0) >=
	     (uint8_t)((sizeof(usb_base_addrs) / sizeof(uint32_t))))
#endif
	    ) {
	...
	usbHostState->usbRegBase =
		(usb_host_ip3516hs_register_struct_t *)usb_base_addrs[usbHostState->controllerId];
	...
	usbHostState->portNumber =
		(uint8_t)(usbHostState->usbRegBase->HCSPARAMS &
			  USB_HOST_IP3516HS_HCSPARAMS_N_PORTS_MASK);
...
{
	usb_host_ip3516hs_state_struct_t *usbHostState =
		(usb_host_ip3516hs_state_struct_t *)controllerHandle;
	...
	if (USB_ENDPOINT_ISOCHRONOUS == pipe->pipeCommon.pipeType) {
		...
		if (USB_SPEED_HIGH !=
			...
			pipe->pipeCommon.interval = ((uint16_t)(
							     1UL <<
								     ((uint32_t)pipe->pipeCommon.
								      interval - 1UL))); /* HS interrupt interval is the power of 2 */
		}
	} else {
...
usb_status_t USB_HostIp3516HsClosePipe(usb_host_controller_handle	controllerHandle,
				       usb_host_pipe_handle		pipeHandle)
{
	usb_host_ip3516hs_state_struct_t *usbHostState =
		(usb_host_ip3516hs_state_struct_t *)controllerHandle;
...
usb_status_t USB_HostIp3516HsIoctl(usb_host_controller_handle controllerHandle, uint32_t ioctlEvent,
				   void *ioctlParam)
{
	usb_host_ip3516hs_state_struct_t *usbHostState =
		(usb_host_ip3516hs_state_struct_t *)controllerHandle;
	...
	case kUSB_HostCancelTransfer:
		...
		status = USB_HostIp3516HsCancelPipe(usbHostState,
						    (usb_host_ip3516hs_pipe_struct_t *)param->
						    pipeHandle,
						    param->transfer);
		break;
#if ((defined(USB_HOST_CONFIG_LOW_POWER_MODE)) && (USB_HOST_CONFIG_LOW_POWER_MODE > 0U))
	...
	case kUSB_HostUpdateControlEndpointAddress:
		...
		/* update address */
		(void)USB_HostHelperGetPeripheralInformation(pipe->pipeCommon.deviceHandle,
							     (uint32_t)kUSB_HostGetDeviceAddress,
							     &deviceAddress);
		switch (pipe->pipeCommon.pipeType) {
		case USB_ENDPOINT_BULK:
		case USB_ENDPOINT_CONTROL:
	...
	case kUSB_HostPortAttachEnable:
		if ((*(uint8_t *)ioctlParam) < usbHostState->portNumber) {
			usbHostState->portState[*(uint8_t *)ioctlParam].portStatus =
				(uint8_t)kUSB_DeviceIp3516HsPortDetached;
		}
		break;
...
void USB_HostIp3516HsTaskFunction(void *hostHandle)
{
	...
	usbHostState =
		(usb_host_ip3516hs_state_struct_t *)((usb_host_instance_t *)hostHandle)->
		controllerHandle;

	/* wait all event */
	if (KOSA_StatusSuccess == OSA_EventWait(usbHostState->ip3516HsEvent, 0xFFU, 0,
		...
		if (0U != (bitSet & USB_HOST_IP3516HS_EVENT_ATTACH)) {
			for (uint8_t i = 0; i < usbHostState->portNumber; i++) {
				if ((uint8_t)kUSB_DeviceIp3516HsPortPhyAttached ==
				    usbHostState->portState[i].portStatus) {
					void *deviceHandle;
					if (kStatus_USB_Success == USB_HostAttachDevice(
						    ...
						    deviceHandle)) {
						usbHostState->portState[i].portStatus =
							(uint8_t)kUSB_DeviceIp3516HsPortAttached;
					}
				}
			}
		}
		if (0U != (bitSet & USB_HOST_IP3516HS_EVENT_DETACH)) {
			for (uint8_t i = 0; i < usbHostState->portNumber; i++) {
				if ((uint8_t)kUSB_DeviceIp3516HsPortPhyDetached ==
				    usbHostState->portState[i].portStatus) {
...
void USB_HostIp3516HsIsrFunction(void *hostHandle)
{
	...
	if (0U != (interruptStatus & USB_HOST_IP3516HS_USBSTS_ISO_IRQ_MASK)) { /* Write back done head */
		...
		(void)OSA_EventSet(usbHostState->ip3516HsEvent,
				   USB_HOST_IP3516HS_EVENT_ISO_TOKEN_DONE);
#endif
	}
	...
	if (0U != (interruptStatus & USB_HOST_IP3516HS_USBSTS_INT_IRQ_MASK)) { /* Write back done head */
		...
		(void)OSA_EventSet(usbHostState->ip3516HsEvent,
				   USB_HOST_IP3516HS_EVENT_INT_TOKEN_DONE);
#endif
	}
	...
	if (0U != (interruptStatus & USB_HOST_IP3516HS_USBSTS_ATL_IRQ_MASK)) { /* Write back done head */
		...
		(void)OSA_EventSet(usbHostState->ip3516HsEvent,
				   USB_HOST_IP3516HS_EVENT_ATL_TOKEN_DONE);
	}
