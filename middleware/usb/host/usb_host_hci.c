 ...
 */
static void USB_HostGetControllerInterface(uint8_t controllerId, const
					   usb_host_controller_interface_t **controllerTable);
...
static const usb_host_controller_interface_t s_EhciInterface = {
	USB_HostEhciCreate,    USB_HostEhciDestory,  USB_HostEhciOpenPipe,
...
static const usb_host_controller_interface_t s_KhciInterface = {
	USB_HostKhciCreate,    USB_HostKhciDestory,  USB_HostKhciOpenPipe,
	USB_HostKhciClosePipe,
...
static const usb_host_controller_interface_t s_OhciInterface = {
	USB_HostOhciCreate,    USB_HostOhciDestory,  USB_HostOhciOpenPipe,
	USB_HostOhciClosePipe,
...
static uint8_t
s_Setupbuffer[USB_HOST_CONFIG_MAX_HOST][USB_HOST_CONFIG_MAX_TRANSFERS][USB_DATA_ALIGN_SIZE_MULTIPLE(
									       8)];
/*******************************************************************************
 * Code
 ******************************************************************************/
...
usb_status_t USB_HostTestModeInit(usb_device_handle deviceHandle)
{
	...
	(void)USB_HostHelperGetPeripheralInformation(deviceHandle, (uint32_t)kUSB_HostGetDevicePID,
						     &productId);
	(void)USB_HostHelperGetPeripheralInformation(deviceHandle, (uint32_t)kUSB_HostGetDeviceVID,
						     &vendorId);
	(void)usb_echo(" vendor id :0x%x product id:0x%x \r\n", vendorId, productId);

	if ((productId != 0x0200U) && (productId != 0x0101U) && (productId != 0x0102U) &&
	    (productId != 0x0103U) &&
	    (productId != 0x0104U) && (productId != 0x0105U) && (productId != 0x0106U) &&
	    (productId != 0x0107U) &&
	    (productId != 0x0108U)) {
	...
	if (productId == 0x0200U) {
		...
		if (hostInstance->controllerTable == &s_EhciInterface) {
			(void)hostInstance->controllerTable->controllerIoctl(
				hostInstance->controllerHandle, kUSB_HostTestModeInit,
				(void *)deviceHandle);
		}
...
static void USB_HostGetControllerInterface(uint8_t
					   controllerId,
...
#if ((defined USB_HOST_CONFIG_EHCI) && (USB_HOST_CONFIG_EHCI))
	if ((controllerId == (uint8_t)kUSB_ControllerEhci0) || (controllerId ==
								(uint8_t)kUSB_ControllerEhci1)) {
			  ...
			  callbackFn)
{
	...
	status =
		hostInstance->controllerTable->controllerCreate(controllerId, hostInstance,
			      ...
			      usb_host_pipe_init_t *	pipeInit)
{
	...
	status = hostInstance->controllerTable->controllerOpenPipe(hostInstance->controllerHandle,
								   pipeHandle, pipeInit);
...
usb_status_t USB_HostClosePipe(usb_host_handle hostHandle, usb_host_pipe_handle pipeHandle)
{
	...
	status = hostInstance->controllerTable->controllerClosePipe(hostInstance->controllerHandle,
								    pipeHandle);

	return status;
}

usb_status_t USB_HostSend(usb_host_handle hostHandle, usb_host_pipe_handle pipeHandle,
 ...
 */
#if 0
	if ((USB_HostValidateDevice(pipe_ptr->deviceHandle) != kStatus_USB_Success) ||
	    (!(USB_HostGetDeviceAttachState(pipe_ptr->deviceHandle)))) {
		USB_HostUnlock();
		return status;
	}
#endif
/* call controller write pipe interface */
	...
	status = hostInstance->controllerTable->controllerWritePipe(hostInstance->controllerHandle,
								    pipeHandle, transfer);
			       ...
			       usb_host_transfer_t *	transfer)
{
	...
	if ((transfer->setupPacket->bmRequestType & USB_REQUEST_TYPE_DIR_MASK) ==
...
#if ((defined USB_HOST_CONFIG_BUFFER_PROPERTY_CACHEABLE) && \
	(USB_HOST_CONFIG_BUFFER_PROPERTY_CACHEABLE))
	DCACHE_CleanByRange((uint32_t)&transfer->setupPacket->bmRequestType,
			    sizeof(usb_setup_struct_t));
	if (transfer->transferLength > 0) {
		DCACHE_CleanInvalidateByRange((uint32_t)transfer->transferBuffer,
...
usb_status_t USB_HostRecv(usb_host_handle hostHandle, usb_host_pipe_handle pipeHandle,
			  usb_host_transfer_t *transfer)
{
	usb_status_t status = kStatus_USB_Success;
	usb_host_instance_t *hostInstance = (usb_host_instance_t *)hostHandle;
...
#endif
	/* the callbackFn is initialized in USB_HostGetControllerInterface */
	status = hostInstance->controllerTable->controllerReadPipe(hostInstance->controllerHandle,
								   pipeHandle, transfer);
						    ...
						    uint32_t *		infoValue)
{
	...
	case kUSB_HostGetHubThinkTime: /* device hub think time */
		*infoValue = USB_HostHubGetTotalThinkTime(deviceInstance->hostHandle,
	...
	case kUSB_HostGetDevicePID: /* device pid */
		*infoValue = (uint32_t)USB_SHORT_FROM_LITTLE_ENDIAN_ADDRESS(
			deviceInstance->deviceDescriptor->idProduct);
		break;

	case kUSB_HostGetDeviceVID: /* device vid */
		*infoValue = (uint32_t)USB_SHORT_FROM_LITTLE_ENDIAN_ADDRESS(
			deviceInstance->deviceDescriptor->idVendor);
		break;
						 ...
						 usb_host_interface_t *		interface)
{
	...
	if (interface->interfaceDesc->bNumEndpoints != 0U) {
		...
		for (; interface->epCount < interface->interfaceDesc->bNumEndpoints;
		     (interface->epCount)++) {
			if (((uint32_t)unionDes >= endPosition) ||
			    (unionDes->common.bDescriptorType != USB_DESCRIPTOR_TYPE_ENDPOINT)) {
#ifdef HOST_ECHO
...
void USB_HostGetVersion(uint32_t *version)
{
	if (NULL != version) {
		*version =
			(uint32_t)USB_MAKE_VERSION(USB_STACK_VERSION_MAJOR, USB_STACK_VERSION_MINOR,
...
usb_status_t USB_HostSuspendDeviceResquest(usb_host_handle hostHandle, usb_device_handle
					   deviceHandle)
{
...
usb_status_t USB_HostResumeDeviceResquest(usb_host_handle hostHandle, usb_device_handle
					  deviceHandle)
{
	usb_host_instance_t *hostInstance;
	usb_host_device_instance_t *deviceInstance;
...
usb_status_t USB_HostSetChargerType(usb_host_handle hostHandle, uint8_t type)
{
	...
	hostInstance = (usb_host_instance_t *)hostHandle;
	return hostInstance->controllerTable->controllerIoctl(hostInstance->controllerHandle,
