				       ...
				       void *				param)
{
	...
	return USB_HostCh9RequestCommon(deviceInstance, transfer, statusParam->statusBuffer,
					length);
}
					      ...
					      void *				param)
{
	...
	transfer->setupPacket->wValue = USB_SHORT_TO_LITTLE_ENDIAN(
		(uint16_t)((uint16_t)descriptorParam->descriptorType << 8) |
		descriptorParam->descriptorIndex);
					  ...
					  void *			param)
{
	...
	return USB_HostCh9RequestCommon(deviceInstance, transfer,
					interfaceParam->alternateInterfaceBuffer, 1);
}
				    ...
				    void *			param)
{
	...
	case USB_REQUEST_STANDARD_GET_CONFIGURATION: /* standard get configuration descriptor request */
		transfer->setupPacket->bmRequestType |= USB_REQUEST_TYPE_DIR_IN;
		status =
			USB_HostCh9RequestCommon((usb_host_device_instance_t *)deviceHandle,
						 transfer, (uint8_t *)param, 1);
		break;
	...
	case USB_REQUEST_STANDARD_SET_CONFIGURATION: /* standard set configuration request */
		...
		status = USB_HostCh9RequestCommon((usb_host_device_instance_t *)deviceHandle,
						  transfer, NULL, 0);
		break;
