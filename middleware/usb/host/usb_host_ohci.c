...
static usb_status_t USB_HostOhciLinkItdTr(usb_host_ohci_state_struct_t *usbHostState,
					  usb_host_ohci_pipe_struct_t * pipe,
					  usb_host_transfer_t *		tr);
static usb_status_t USB_HostOhciFreeItd(
...
static usb_status_t USB_HostOhciLinkGtdTr(usb_host_ohci_state_struct_t *usbHostState,
					  usb_host_ohci_pipe_struct_t * pipe,
					  usb_host_transfer_t *		tr);
static usb_status_t USB_HostOhciFreeGtd(
	usb_host_ohci_state_struct_t *				usbHostState,
...
static void USB_HostOhciTdDoneHandle(usb_host_ohci_state_struct_t *usbHostState,
				     usb_host_ohci_pipe_struct_t *pipe, usb_host_transfer_t *
				     currentTr,
				     usb_host_ohci_general_transfer_descritpor_struct_t *gtd,
				     usb_host_ohci_isochronous_transfer_descritpor_struct_t *itd);
...
static void USB_HostOhciDelay(usb_host_ohci_state_struct_t *usbHostState, uint32_t ms)
{
	...
	sofStart =
		((uint32_t)s_UsbHostOhciHcca[usbHostState->controllerId].HccaFrameNumber &
		 USB_HOST_OHCI_FMNUMBER_FN_MASK);

	do{
		SofEnd =
			(uint32_t)((uint32_t)s_UsbHostOhciHcca[usbHostState->controllerId].
				   HccaFrameNumber &
				   USB_HOST_OHCI_FMNUMBER_FN_MASK);
		distance = (uint32_t)(SofEnd - sofStart + USB_HOST_OHCI_FMNUMBER_FN_MASK + 1U) &
			   USB_HOST_OHCI_FMNUMBER_FN_MASK;
	} while ((distance) < (ms)); /* compute the distance between sofStart and SofEnd */
}

static usb_status_t USB_HostOhciControlBus(usb_host_ohci_state_struct_t *usbHostState, uint8_t
					   busControl)
{
	...
	case kUSB_HostBusSuspend:
		if (0U != (usbHostState->usbRegBase->HcRhPortStatus[0] &
			   USB_HOST_OHCI_RHPORTSTATUS_CCS_MASK)) {
			...
			portStatus |= USB_HOST_OHCI_CONTROL_HCFS(
				USB_HOST_OHCI_CONTROL_HCFS_SUSPEND) |
				      USB_HOST_OHCI_CONTROL_RWC_MASK |
				      USB_HOST_OHCI_CONTROL_RWE_MASK;
	...
	case kUSB_HostBusResume:
		if (0u != (usbHostState->usbRegBase->HcRhPortStatus[0] &
			...
			portStatus &= ~(USB_HOST_OHCI_CONTROL_HCFS_MASK |
					...
					USB_HOST_OHCI_CONTROL_RWC_MASK);
			portStatus |= USB_HOST_OHCI_CONTROL_HCFS(
				USB_HOST_OHCI_CONTROL_HCFS_OPERATIONAL);
			usbHostState->usbRegBase->HcControl = portStatus;
			portStatus = usbHostState->usbRegBase->HcRhPortStatus[0];
			portStatus &= ~(USB_HOST_OHCI_RHPORTSTATUS_WIC);
...
static uint32_t USB_HostOhciBusTime(uint8_t speed, uint8_t pipeType, uint8_t direction, uint32_t
				    dataLength)
{
	...
	if (pipeType == USB_ENDPOINT_ISOCHRONOUS) { /* iso */
		...
		if (speed == USB_SPEED_FULL) {
			...
			if (direction == USB_IN) {
				result = 64060000U + 2000U * USB_HOST_OHCI_HUB_LS_SETUP_TIME_DELAY +
					 676670U * result +
					 USB_HOST_OHCI_TIME_DELAY;
			} else {
				result = 64107000U + 2000U * USB_HOST_OHCI_HUB_LS_SETUP_TIME_DELAY +
					 667000U * result +
					 USB_HOST_OHCI_TIME_DELAY;
			}
		} else {
...
static usb_status_t USB_HostOhciGetPipe(usb_host_ohci_pipe_struct_t **	pipeQueue,
					usb_host_ohci_pipe_struct_t **	pipe)
{
...
static usb_status_t USB_HostOhciRemovePipe(usb_host_ohci_pipe_struct_t **	pipeQueue,
					   usb_host_ohci_pipe_struct_t *	pipe)
{
...
static usb_status_t USB_HostOhciInsertPipe(usb_host_ohci_pipe_struct_t **	pipeQueue,
					   usb_host_ohci_pipe_struct_t *	pipe)
{
	usb_host_ohci_pipe_struct_t *p = *pipeQueue;
	void *temp;
...
static usb_status_t USB_HostOhciGetGtd(
	usb_host_ohci_state_struct_t *				usbHostState,
	usb_host_ohci_general_transfer_descritpor_struct_t **	gtd,
	uint32_t
	count)
{
	...
	*gtd = NULL;
	while ((0U != count) && (NULL != usbHostState->gtdList)) {
		...
		usbHostState->gtdList =
			(usb_host_ohci_general_transfer_descritpor_struct_t *)usbHostState->gtdList
			->nextGtd;
		temp = (void *)(p);
		tmp = (uint32_t *)(temp);
		usbHostState->gtdCount--;
		for (uint8_t i = 0; i <
		     (sizeof(usb_host_ohci_general_transfer_descritpor_struct_t) /
		      sizeof(uint32_t));
		     i++) {
			tmp[i] = 0U;
		}
...
static usb_status_t USB_HostOhciGetItd(
	usb_host_ohci_state_struct_t *					usbHostState,
	usb_host_ohci_isochronous_transfer_descritpor_struct_t **	itd,
	uint32_t
	count)
{
	...
	*itd = NULL;
	while ((0U != count) && (NULL != usbHostState->itdList)) {
		...
		usbHostState->itdList =
			(usb_host_ohci_isochronous_transfer_descritpor_struct_t *)usbHostState->
			itdList->
			NextTD;
		temp = (void *)(p);
		tmp = (uint32_t *)(temp);
		...
		for (uint32_t i = 0; i <
		     (sizeof(usb_host_ohci_isochronous_transfer_descritpor_struct_t) /
		      sizeof(uint32_t));
		     i++) {
			tmp[i] = 0U;
		}
...
static void USB_HostOhciLinkTdToDoneList(usb_host_ohci_state_struct_t *usbHostState)
{
	...
	tdList = (usb_host_ohci_general_transfer_descritpor_struct_t
		  *)(s_UsbHostOhciHcca[usbHostState->controllerId].HccaDoneHead &
		     USB_HOST_OHCI_HD_MASK);

	while (NULL != tdList) {
		tdListPos = (usb_host_ohci_general_transfer_descritpor_struct_t *)(tdList->NextTD &
										   USB_HOST_OHCI_HD_MASK);
		{
...
static usb_status_t USB_HostOhciPortChange(usb_host_ohci_state_struct_t *usbHostState)
{
	uint32_t portStatus;
	uint32_t i;
	uint32_t portCount = (usbHostState->usbRegBase->HcRhDescriptorA &
			      USB_HOST_OHCI_RHDESCRIPTORA_NDP_MASK);
	...
	for (i = 0U; i < portCount; i++) {
		...
		if (0U != (portStatus & USB_HOST_OHCI_RHPORTSTATUS_CSC_MASK)) {
			...
			if (kBus_OhciIdle != usbHostState->busSuspendStatus) {
				...
				usbHostState->usbRegBase->HcRhPortStatus[i] = portStatus |
									      USB_HOST_OHCI_RHPORTSTATUS_PSSC_MASK;
				while (0U != (usbHostState->usbRegBase->HcRhPortStatus[i] &
					      USB_HOST_OHCI_RHPORTSTATUS_PSS_MASK)) {
				}
				portStatus = usbHostState->usbRegBase->HcControl;
				portStatus &= ~(USB_HOST_OHCI_CONTROL_HCFS_MASK |
						USB_HOST_OHCI_CONTROL_RWE_MASK |
...
#endif
			uint32_t sofStart =
				(uint32_t)s_UsbHostOhciHcca[usbHostState->controllerId].
				HccaFrameNumber &
				USB_HOST_OHCI_FMNUMBER_FN_MASK;
			uint32_t sof;
			while (1U == 1U) {
				...
				if (0U != (portStatus & USB_HOST_OHCI_RHPORTSTATUS_CSC_MASK)) {
					portStatus &= ~USB_HOST_OHCI_RHPORTSTATUS_WIC;
					usbHostState->usbRegBase->HcRhPortStatus[i] = portStatus |
										      USB_HOST_OHCI_RHPORTSTATUS_CSC_MASK;
				}
				sof =
					(uint32_t)((uint32_t)s_UsbHostOhciHcca[usbHostState->
									       controllerId].
						   HccaFrameNumber &
						   USB_HOST_OHCI_FMNUMBER_FN_MASK);
				if ((((sof + USB_HOST_OHCI_FMNUMBER_FN_MASK + 1U) - sofStart) &
				     USB_HOST_OHCI_FMNUMBER_FN_MASK) > 1U) {
					break;
				}
			}
		}
		...
		if (0U != (portStatus & USB_HOST_OHCI_RHPORTSTATUS_CCS_MASK)) {
			uint32_t index;
			if (((uint8_t)kUSB_DeviceOhciPortDetached) !=
				...
				if ((kBus_OhciSuspended == usbHostState->busSuspendStatus) ||
					...
					if (0U != (portStatus &
						   USB_HOST_OHCI_RHPORTSTATUS_PSSC_MASK)) {
						usb_host_instance_t *hostPointer =
							(usb_host_instance_t *)usbHostState->
							hostHandle;
						...
						portStatus &= ~USB_HOST_OHCI_RHPORTSTATUS_WIC;
						usbHostState->usbRegBase->HcRhPortStatus[i] =
							portStatus |
							USB_HOST_OHCI_RHPORTSTATUS_PSSC_MASK;
						while (0U !=
						       (usbHostState->usbRegBase->HcRhPortStatus[i]
			...
			usbHostState->busSuspendStatus = kBus_OhciIdle;
#endif
			for (index = 0U; index < USB_HOST_OHCI_PORT_CONNECT_DEBOUNCE_DELAY;
			     index++) {
				(void)USB_HostOhciDelay(usbHostState, 1);
				if (0U == ((usbHostState->usbRegBase->HcRhPortStatus[i] &
					    USB_HOST_OHCI_RHPORTSTATUS_CCS_MASK))) {
					break;
				}
			}
			...
			portStatus &= ~USB_HOST_OHCI_RHPORTSTATUS_WIC;
			usbHostState->usbRegBase->HcRhPortStatus[i] = portStatus |
								      USB_HOST_OHCI_RHPORTSTATUS_PRS_MASK;
			...
			usbHostState->portState[i].portSpeed =
				(0U != (usbHostState->usbRegBase->HcRhPortStatus[i] &
					USB_HOST_OHCI_RHPORTSTATUS_LSDA_MASK)) ?
				USB_SPEED_LOW :
				USB_SPEED_FULL;
			/* do bus recovery delay */
			(void)USB_HostOhciDelay(usbHostState,
						USB_HOST_OHCI_PORT_RESET_RECOVERY_DELAY);
			usbHostState->portState[i].portStatus =
				(uint8_t)kUSB_DeviceOhciPortPhyAttached;
...
#endif
			usbHostState->portState[i].portStatus =
				(uint8_t)kUSB_DeviceOhciPortPhyDetached;
	...
	for (startFrame = 0U; startFrame < pipe->pipeCommon.interval; startFrame++) {
		for (frame = startFrame; frame < USB_HOST_OHCI_HCCA_SIZE; frame++) {
			...
			while (NULL != temp) {
				p = (usb_host_ohci_pipe_struct_t *)temp;
				temp = (void *)p->pipeCommon.next;
				if ((frame >= p->startFrame) && (0U == ((frame - p->startFrame) %
						  ...
						  usb_host_ohci_pipe_struct_t * pipe)
{
	uint32_t i;
	uint32_t dummy =
		(uint32_t)&s_UsbHostOhciTd[usbHostState->controllerId].ed[
			USB_HOST_OHCI_PERIODIC_ED_DUMMY];
	...
	for (i = pipe->startFrame; i < USB_HOST_OHCI_HCCA_SIZE; i += pipe->pipeCommon.interval) {
		ed =
			(usb_host_ohci_endpoint_descritpor_struct_t *)s_UsbHostOhciHcca[usbHostState
											->
											controllerId
			]
...
static usb_status_t USB_HostOhciOpenControlBulkPipe(
	usb_host_ohci_state_struct_t *	usbHostState,
	usb_host_ohci_pipe_struct_t *	pipe)
{
	usb_host_ohci_general_transfer_descritpor_struct_t *gtd;
	...
	if (USB_ENDPOINT_CONTROL == pipe->pipeCommon.pipeType) {
		pipe->ed->NextED =
			s_UsbHostOhciTd[usbHostState->controllerId].ed[
				USB_HOST_OHCI_CONTROL_ED_DUMMY].
			NextED;
		s_UsbHostOhciTd[usbHostState->controllerId].ed[USB_HOST_OHCI_CONTROL_ED_DUMMY].
		NextED = (uint32_t)pipe->ed;
		usbHostState->usbRegBase->HcControl |= USB_HOST_OHCI_CONTROL_CLE_MASK;
	} else {
		pipe->ed->NextED =
			s_UsbHostOhciTd[usbHostState->controllerId].ed[USB_HOST_OHCI_BULK_ED_DUMMY].
			NextED;
		s_UsbHostOhciTd[usbHostState->controllerId].ed[USB_HOST_OHCI_BULK_ED_DUMMY].NextED =
			(uint32_t)pipe->ed;
...
static void USB_HostOhciTdDoneHandle(
	usb_host_ohci_state_struct_t *	usbHostState,
	usb_host_ohci_pipe_struct_t *	pipe,
	usb_host_transfer_t *
	currentTr,
	...
	usb_host_ohci_isochronous_transfer_descritpor_struct_t *itd)
{
	...
	if (NULL != itd) {
		...
		itdP =
			(usb_host_ohci_isochronous_transfer_descritpor_struct_t *)currentTr->union1.
			unitHead;
		...
		for (uint32_t i = 0; i <= itd->stateUnion.stateBitField.FC; i++) {
			currentTr->transferSofar += ((uint32_t)itd->OffsetPSW[i] &
						     (USB_HOST_OHCI_ITD_TRANSFER_SIZE_MASK));
			conditionCode = (((uint32_t)itd->OffsetPSW[i] &
					  (USB_HOST_OHCI_ITD_CONDITION_CODE_MASK)) >>
					 USB_HOST_OHCI_ITD_CONDITION_CODE_SHIFT);
			if (0U != conditionCode) {
				if (conditionCode != USB_HOST_OHCI_CC_DATA_UNDERRUN) {
					if (kStatus_USB_Success ==
					    (usb_status_t)currentTr->union2.frame) {
						if (conditionCode ==
						    USB_HOST_OHCI_CC_DATA_OVERRUN) {
							currentTr->union2.frame =
								(uint32_t)kStatus_USB_DataOverRun;
						} else {
							currentTr->union2.frame =
...
#endif
	{
		...
		gtdP =
			(usb_host_ohci_general_transfer_descritpor_struct_t *)currentTr->union1.
			unitHead;
		while (NULL != gtdP) {
			if (gtd == gtdP) {
				if (NULL != gtdQ) {
					...
					uint32_t tail = pipe->ed->TailP &
							USB_HOST_OHCI_ED_TAILP_MASK;
					uint32_t head = pipe->ed->HeadP &
							USB_HOST_OHCI_ED_HEADP_MASK;
		...
		if ((0U != gtd->CBP) && (USB_HOST_OHCI_GTD_DP_SETUP !=
					 gtd->stateUnion.stateBitField.DP)) {
			currentTr->transferLength -= ((0U != (((gtd->BE) ^ (gtd->CBP)) &
							      0xFFFFF000U)) ? 0x00001000U : 0U) +
						     ((gtd->BE) & 0x00000FFFU) - ((gtd->CBP) &
										  0x00000FFFU) + 1U;
		}
		currentTr->transferSofar = currentTr->transferLength;
	...
	if (NULL == (void *)(temp)) {
		if ((0U == pipe->isCanceling) && ((uint32_t)kStatus_USB_Success ==
						  currentTr->union2.frame)) {
			if (USB_ENDPOINT_CONTROL == pipe->pipeCommon.pipeType) {
				if (NULL != pipe->ed->trListHead) {
					if (kStatus_USB_Success == USB_HostOhciLinkGtdControlTr(
						    usbHostState, pipe, pipe->ed->trListHead)) {
	...
	if (NULL == (void *)(temp)) {
		...
		if (0U != trIsFound) {
			...
			if ((kStatus_USB_Success == ((usb_status_t)currentTr->union2.frame)) &&
			    ...
			    (USB_REQUEST_STANDARD_CLEAR_FEATURE ==
			     currentTr->setupPacket->bRequest) &&
			    (USB_REQUEST_TYPE_RECIPIENT_ENDPOINT ==
			     currentTr->setupPacket->bmRequestType) &&
			    (USB_REQUEST_STANDARD_FEATURE_SELECTOR_ENDPOINT_HALT ==
			     (USB_SHORT_FROM_LITTLE_ENDIAN(currentTr->setupPacket->wValue) &
			      0x00FFU))) {
				...
				if ((NULL != p) && ((p->pipeCommon.pipeType ==
						     USB_ENDPOINT_INTERRUPT) ||
						    (p->pipeCommon.pipeType ==
						     USB_ENDPOINT_BULK))) {
					p->ed->stateUnion.stateBitField.K = 1U;
					startFrame =
						s_UsbHostOhciHcca[usbHostState->controllerId].
						HccaFrameNumber;
			...
			while (NULL != p) {
				if (USB_ENDPOINT_CONTROL == p->pipeCommon.pipeType) {
					if (NULL != p->ed->trListHead) {
						...
						if (kStatus_USB_Success ==
						    USB_HostOhciLinkGtdControlTr(usbHostState, p,
										 p->ed->trListHead))
						{
		...
		if (NULL != pipe->ed->trListHead) {
			...
			case USB_ENDPOINT_INTERRUPT:
				/* initialize gtd for control/bulk/interrupt transfer */
				if (kStatus_USB_Success != USB_HostOhciLinkGtdTr(usbHostState, pipe,
										 pipe->ed->
										 trListHead)) {
					   ...
					   usb_host_transfer_t *		tr)
{
	...
	gtd =
		(usb_host_ohci_general_transfer_descritpor_struct_t *)((uint32_t)usbHostState->
								       tdDoneListHead);
	gtdPre = NULL;
	while (NULL != gtd) {
		if (((uint32_t)gtd) <
		    ((uint32_t)&s_UsbHostOhciTd[usbHostState->controllerId].gtd[0])) {
			temp = (void *)gtd;
		...
		if ((NULL == tr) || (tr == trCurrent)) {
			if (0UL != trCurrent->union1.unitHead) {
				...
				while (NULL != gtd) {
					...
					USB_HostOhciTdDoneHandle(usbHostState, pipe, trCurrent, gtd,
								 itd);
					gtd = gtdPos;
				}
			} else {
...
static usb_status_t USB_HostOhciRemoveFromPeriodicList(
	usb_host_ohci_state_struct_t *	usbHostState,
	usb_host_ohci_pipe_struct_t *	pipe)
{
	uint32_t i;
	...
	for (i = pipe->startFrame; i < USB_HOST_OHCI_HCCA_SIZE; i += pipe->pipeCommon.interval) {
		...
		while (NULL != ed) {
			if (ed == pipe->ed) {
				if (NULL == pre) {
					s_UsbHostOhciHcca[usbHostState->controllerId].
					HccaInterrruptTable[i] = ed->NextED;
				} else {
...
static usb_status_t USB_HostOhciCloseControlBulkPipe(
	usb_host_ohci_state_struct_t *	usbHostState,
	usb_host_ohci_pipe_struct_t *	pipe)
{
	...
	(void)USB_HostOhciFreeGtd(
		usbHostState,
		(usb_host_ohci_general_transfer_descritpor_struct_t *)(pipe->ed->TailP &
								       USB_HOST_OHCI_ED_HEADP_MASK));
...
static usb_status_t USB_HostOhciCloseIsoInterruptPipe(
	usb_host_ohci_state_struct_t *	usbHostState,
	usb_host_ohci_pipe_struct_t *	pipe)
{
	(void)USB_HostOhciCancelPipe(usbHostState, pipe, NULL);
	...
	if (USB_ENDPOINT_ISOCHRONOUS == pipe->pipeCommon.pipeType) {
		...
		(void)USB_HostOhciFreeItd(
			usbHostState,
			(usb_host_ohci_isochronous_transfer_descritpor_struct_t *)(pipe->ed->TailP &
										   USB_HOST_OHCI_ED_HEADP_MASK));
#endif
	} else {
		(void)USB_HostOhciFreeGtd(
						 ...
						 usb_host_transfer_t *		tr)
{
	...
	OSA_ENTER_CRITICAL();
	if ((0U != pipe->isBusy) && (kStatus_UsbHostOhci_Idle ==
				     (usb_host_ohci_transfer_status_t)tr->setupStatus)) {
		OSA_EXIT_CRITICAL();
		return kStatus_USB_Success;
	}
	pipe->isBusy = 1;
	...
	case kStatus_UsbHostOhci_Idle:
		...
		USB_HostOhciFillGtd(pipe, tr, p, (uint8_t *)tr->setupPacket, 8U,
				    USB_HOST_OHCI_GTD_DP_SETUP, 0x02U, 0U);
	...
	case kStatus_UsbHostOhci_Setup:
		...
		p = head;
		tr->union1.unitHead = (uint32_t)p;
		dirPid = (tr->direction == USB_OUT) ? USB_HOST_OHCI_GTD_DP_OUT :
			 USB_HOST_OHCI_GTD_DP_IN;
		USB_HostOhciFillGtd(pipe, tr, p, (uint8_t *)tr->transferBuffer, tr->transferLength,
				    dirPid, 0x03U, 1U);
		tr->setupStatus = (uint8_t)kStatus_UsbHostOhci_Data;
		status = kStatus_USB_Success;
		break;
	case kStatus_UsbHostOhci_Data:
		...
		if (tr->setupPacket->wLength > 0u) {
			statePid = (tr->direction == USB_OUT) ? USB_HOST_OHCI_GTD_DP_IN :
				   USB_HOST_OHCI_GTD_DP_OUT;
		} else {
					  ...
					  usb_host_transfer_t *		tr)
{
	...
	while (0U != remainingLength) {
		tdLength = USB_HOST_OHCI_GTD_MAX_LENGTH - (bufferAddress &
							   USB_HOST_OHCI_GTD_BUFFER_ALIGN_MASK);
	...
	while (0U != remainingLength) {
		...
		if (0U != remainingLength) {
			USB_HostOhciFillGtd(pipe, tr, p, (uint8_t *)bufferAddress, tdLength, dirPid,
					    0x00U, 0U);
			p->nextGtd =
				(usb_host_ohci_general_transfer_descritpor_struct_t *)p->NextTD;
			p = (usb_host_ohci_general_transfer_descritpor_struct_t *)p->NextTD;
		} else {
			USB_HostOhciFillGtd(pipe, tr, p, (uint8_t *)bufferAddress, tdLength, dirPid,
					    0x00U, 1U);
				...
				uint32_t						startFrame)
{
	...
	for (uint32_t i = 0; i < 8U; i++) {
		itd->OffsetPSW[i] = (uint16_t)((((uint32_t)buffer + (i *
								     pipe->pipeCommon.maxPacketSize))
						& 0xFFFU) |
					       ((uint32_t)USB_HOST_OHCI_CC_NOT_ACCESSED << 12U));
		if ((((uint32_t)buffer + (i * pipe->pipeCommon.maxPacketSize)) &
		     USB_HOST_OHCI_ITD_BP0_MASK) != itd->BP0) {
					  ...
					  usb_host_transfer_t *		tr)
{
	...
	while (0U != remainingLength) {
		...
		if (0U != remainingLength) {
			p->nextItd =
				(usb_host_ohci_isochronous_transfer_descritpor_struct_t *)p->NextTD;
			p = p->nextItd;
			startFrame += ((uint32_t)tansaction) *
				      ((uint32_t)pipe->pipeCommon.interval);
		}
		bufferAddress += tdLength;
	}
	p->nextItd = NULL;
		...
		if ((USB_ENDPOINT_CONTROL == pipe->pipeCommon.pipeType) || (USB_ENDPOINT_BULK ==
			...
			if (NULL != pipe->ed->trListHead) {
				...
				if (0U == pipe->cutOffTime) {
					(void)USB_HostOhciCancelPipe(usbHostState, pipe,
								     pipe->ed->trListHead);
				}
			}
		} else {
			...
			if ((0U != pipe->isDone) && (pipe->currentTr == pipe->ed->trListHead)) {
				if (pipe->startWriteBackCount != usbHostState->writeBackCount) {
					...
					while (NULL != gtd) {
						...
						USB_HostOhciTdDoneHandle(usbHostState, pipe,
									 pipe->ed->trListHead, gtd,
									 itd);
						gtd = gtdP;
					}
				}
...
static usb_status_t UBS_HostOhciControllerReset(usb_host_ohci_state_struct_t *usbHostState)
{
	...
	while (0U != (usbHostState->usbRegBase->HcCommandStatus &
		      USB_HOST_OHCI_COMMAND_STATUS_HCR_MASK)) {
		__NOP();
	}
	return kStatus_USB_Success;
}
...
static usb_status_t UBS_HostOhciControllerInit(usb_host_ohci_state_struct_t *usbHostState)
{
	...
	usbHostState->usbRegBase->HcControl |= USB_HOST_OHCI_CONTROL_HCFS(
		USB_HOST_OHCI_CONTROL_HCFS_OPERATIONAL);
	usbHostState->usbRegBase->HcFmInterval =
		(USB_HOST_OHCI_FMINTERVAL_FSMPS((6UL * (USB_HOST_OHCI_SOF_INTERVAL - 210UL)) /
						7UL)) |
	...
	usbHostState->usbRegBase->HcPeriodicStart = USB_HOST_OHCI_PERIODIC_START_PS(
		(uint32_t)((float)USB_HOST_OHCI_SOF_INTERVAL *
			   (float)USB_HOST_OHCI_PERIODIC_BANDWIDTH_PERCENT));
	...
	usbHostState->usbRegBase->HcControl |= USB_HOST_OHCI_CONTROL_CBSR(
		USB_HOST_OHCI_CONTROL_CBSR_C1_B1);
	if (0U != (usbHostState->usbRegBase->HcRhDescriptorA &
		   USB_HOST_OHCI_RHDESCRIPTORA_NPS_MASK)) {
	} else {
/* Use global power to turn on the port power for all ports */
#if 0
		if (usbHostState->usbRegBase->HcRhDescriptorA &
		    USB_HOST_OHCI_RHDESCRIPTORA_PSM_MASK) {
			usbHostState->usbRegBase->HcRhDescriptorB |=
				USB_HOST_OHCI_RHDESCRIPTORB_PPCM(0xFFFEUL);
			for (i = 0; i < (usbHostState->usbRegBase->HcRhDescriptorA &
					 USB_HOST_OHCI_RHDESCRIPTORA_NDP_MASK); i++) {
...
#else
		usbHostState->usbRegBase->HcRhDescriptorB &= ~USB_HOST_OHCI_RHDESCRIPTORB_PPCM(
			0xFFFEUL);
		usbHostState->usbRegBase->HcRhStatus = USB_HOST_OHCI_RHSTATUS_LPSC_MASK;
#endif
	}
				...
				usb_host_controller_handle *	controllerHandle)
{
	...
	if (((controllerId - (uint8_t)kUSB_ControllerOhci0) >= (uint8_t)USB_HOST_CONFIG_OHCI) ||
	    ((controllerId - (uint8_t)kUSB_ControllerOhci0) >= (sizeof(usb_base_addrs) /
								sizeof(uint32_t)))) {
	...
	usbHostState->usbRegBase =
		(usb_host_ohci_hcor_struct_t *)usb_base_addrs[usbHostState->controllerId];
	...
	usbHostState->portNumber =
		(uint8_t)(usbHostState->usbRegBase->HcRhDescriptorA &
			  USB_HOST_OHCI_RHDESCRIPTORA_NDP_MASK);
...
{
	usb_host_ohci_state_struct_t *usbHostState =
		(usb_host_ohci_state_struct_t *)controllerHandle;
				  ...
				  usb_host_pipe_init_t *	pipeInit)
{
	...
	pipe->ed->stateUnion.stateBitField.FA =
		((usb_host_device_instance_t *)pipeInit->devInstance)->setAddress;
	pipe->ed->stateUnion.stateBitField.S =
		(USB_SPEED_FULL == ((usb_host_device_instance_t *)pipeInit->devInstance)->speed) ?
		0U : 1U;
...
usb_status_t USB_HostOhciClosePipe(usb_host_controller_handle controllerHandle, usb_host_pipe_handle
				   pipeHandle)
{
	usb_host_ohci_state_struct_t *usbHostState =
		(usb_host_ohci_state_struct_t *)controllerHandle;
...
usb_status_t USB_HostOhciIoctl(usb_host_controller_handle controllerHandle, uint32_t ioctlEvent,
			       void *ioctlParam)
{
	usb_host_ohci_state_struct_t *usbHostState =
		(usb_host_ohci_state_struct_t *)controllerHandle;
	...
	case kUSB_HostCancelTransfer:
		...
		status =
			USB_HostOhciCancelPipe(usbHostState,
					       (usb_host_ohci_pipe_struct_t *)param->pipeHandle,
					       param->transfer);
		break;
	...
	case kUSB_HostGetFrameNumber:
		/* get frame number */
		*((uint32_t *)ioctlParam) =
			(s_UsbHostOhciHcca[usbHostState->controllerId].HccaFrameNumber);
		break;
...
void USB_HostOhciTaskFunction(void *hostHandle)
{
	...
	usbHostState =
		(usb_host_ohci_state_struct_t *)((usb_host_instance_t *)hostHandle)->
		controllerHandle;
...
void USB_HostOhciIsrFunction(void *hostHandle)
{
	...
	if (0U != (interruptStatus & USB_HOST_OHCI_INTERRUPT_STATUS_SF_MASK)) { /* SOF interrupt */
		...
		while (NULL != temp) {
			pipe = (usb_host_ohci_pipe_struct_t *)temp;
			temp = (void *)pipe->pipeCommon.next;
			if ((USB_ENDPOINT_ISOCHRONOUS == pipe->pipeCommon.pipeType) && (NULL !=
											pipe->ed->
											trListHead))
			{
				if ((NULL != pipe->ed->dealTr)) {
					...
					uint32_t head = pipe->ed->HeadP &
							USB_HOST_OHCI_ED_HEADP_MASK;
					if (head == tail) {
						if (kStatus_USB_Success == USB_HostOhciLinkItdTr(
							    usbHostState, pipe, pipe->ed->dealTr)) {
