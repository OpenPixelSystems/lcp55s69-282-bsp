...
#define USB_LPC3511IP_ENDPOINT_SET_ENDPOINT_AND(lpcState, index, odd, value)                           \
	*((volatile uint32_t *)(((uint32_t)((lpcState)->epCommandStatusList)) | \
				((uint32_t)(index) << 3) | \
...
#define USB_LPC3511IP_ENDPOINT_SET_ENDPOINT(lpcState, index, odd, value, NBytes, address)                \
	...
	*((volatile uint32_t *)(((uint32_t)((lpcState)->epCommandStatusList)) | \
		...
		((0U != lpc3511IpState->controllerSpeed) ?                                                       \
                                                                                                         \
		 ((uint32_t)(value) | ((uint32_t)(NBytes) << \
...
static usb_status_t USB_DeviceLpc3511IpTransaction(
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState,
	usb_device_lpc3511ip_endpoint_state_struct_t *epState, uint8_t endpointIndex);
static usb_status_t USB_DeviceLpc3511IpControlPreSetDeviceAddress(usb_device_controller_handle
								  controllerHandle, void *param);
/*******************************************************************************
 * Variables
 ******************************************************************************/
...
#define ZERO_TRANSFER_DATA_OFFSET ((USB_DATA_ALIGN_SIZE_MULTIPLE(8U) >> 2) + \
				   (USB_DATA_ALIGN_SIZE_MULTIPLE(64U) >> 2))
...
#else
USB_GLOBAL USB_RAM_ADDRESS_ALIGNMENT(USB_DATA_ALIGN_SIZE) static uint32_t
s_SetupAndEpReservedData[USB_DEVICE_CONFIG_LPCIP3511FS +
			 USB_DEVICE_CONFIG_LPCIP3511HS][RESERVED_EP_DATA_OFFSET];
#endif
...
static inline uint8_t USB_DeviceLpcIp3511MaxPacketNeedCopy(
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState)
{
#if (USB_DEVICE_IP3511_RESERVED_BUFFER_FOR_COPY == (USB_DEVICE_CONFIG_LPCIP3511HS + \
						    USB_DEVICE_CONFIG_LPCIP3511FS))
	return 1U;
#elif (USB_DEVICE_IP3511_RESERVED_BUFFER_FOR_COPY == USB_DEVICE_CONFIG_LPCIP3511HS)
...
static uint8_t *USB_DeviceLpcIp3511MallocMaxPacketBuffer(
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState,
	uint32_t
	multile64)
{
	...
	if (numIndex >= multile64) {
		...
		for (numIndex = 0U; numIndex < multile64; ++numIndex) {
			lpc3511IpState->epReservedBufferBits[((bitsIndex - multile64 + numIndex) /
							      8U)] |=
				(uint8_t)(0x01U << ((bitsIndex - multile64 + numIndex) &
						    0x00000007U));
		}
		OSA_EXIT_CRITICAL();
...
static void USB_DeviceLpcIp3511ReleaseMaxPacketBuffer(
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState,
	uint8_t *
	buffer,
	uint32_t
	bufferSize)
{
	...
	if ((buffer < lpc3511IpState->epReservedBuffer) ||
	    (buffer >= (lpc3511IpState->epReservedBuffer +
 ...
 */
static usb_status_t USB_DeviceLpc3511IpEndpointPrime(
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState,
...
static usb_status_t USB_DeviceLpc3511IpEndpointInit(
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState,
	...
	epInit)
{
...
#endif
		(0U != lpc3511IpState->controllerSpeed) && (USB_ENDPOINT_INTERRUPT ==
							    epInit->transferType)) {
		epState->stateUnion.stateBitField.epControlDefault =
			((USB_LPC3511IP_ENDPOINT_ENDPOINT_TYPE_MASK |
	...
	USB_LPC3511IP_ENDPOINT_SET_ENDPOINT(
		...
		(epState->stateUnion.stateBitField.epControlDefault <<
			USB_LPC3511IP_ENDPOINT_CONFIGURE_BITS_SHIFT), 0U, 0U);
	if ((epInit->endpointAddress & USB_ENDPOINT_NUMBER_MASK) == USB_CONTROL_ENDPOINT) {
		if (0U == (epInit->endpointAddress &
			   USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK)) {
			/* Prime setup packet when the endpoint is control out endpoint. */
			USB_LPC3511IP_ENDPOINT_SET_ENDPOINT(lpc3511IpState, 0U, 1U, 0U, 0U,
							    (uint32_t)lpc3511IpState->setupData);
		}
	} else {
		USB_LPC3511IP_ENDPOINT_SET_ENDPOINT(
			lpc3511IpState, endpointIndex, 1U,
			(epState->stateUnion.stateBitField.epControlDefault <<
				USB_LPC3511IP_ENDPOINT_CONFIGURE_BITS_SHIFT), 0U,
			0U);
	}
	...
	(USB_DEVICE_IP3511_RESERVED_BUFFER_FOR_COPY)
		...
		if (0U != USB_DeviceLpcIp3511MaxPacketNeedCopy(lpc3511IpState)) {
			...
			uint8_t *maxPacketBuffer = USB_DeviceLpcIp3511MallocMaxPacketBuffer(
				lpc3511IpState, USB_LPC3511IP_GET_MULTIPLE_OF_64(maxPacketSize) *
				2U);
#else
			uint8_t *maxPacketBuffer = USB_DeviceLpcIp3511MallocMaxPacketBuffer(
				lpc3511IpState, maxPacketSize);
#endif
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
static usb_status_t USB_DeviceLpc3511IpEndpointDeinit(
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState, uint8_t ep)
{
		...
		if ((endpointIndex >> 1U) != USB_CONTROL_ENDPOINT) { /* control endpoint */
			...
			USB_DeviceLpcIp3511ReleaseMaxPacketBuffer(
				...
				USB_LPC3511IP_GET_MULTIPLE_OF_64(
					epState->stateUnion.stateBitField.maxPacketSize) * 2U);
#else
			USB_DeviceLpcIp3511ReleaseMaxPacketBuffer(lpc3511IpState,
								  epState->epPacketBuffer,
								  epState->stateUnion.stateBitField.
								  maxPacketSize);
#endif
		}
		epState->epPacketBuffer = NULL;
	...
	USB_LPC3511IP_ENDPOINT_SET_ENDPOINT(lpc3511IpState, endpointIndex, 0U,
					    USB_LPC3511IP_ENDPOINT_DISABLE_MASK, 0U, 0U);
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
static usb_status_t USB_DeviceLpc3511IpEndpointStall(
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState, uint8_t ep)
{
	uint8_t endpointIndex = USB_LPC3511IP_ENDPOINT_DES_INDEX(ep);
	usb_device_lpc3511ip_endpoint_state_struct_t *epState =
	...
	/* stall the endpoint */
	USB_LPC3511IP_ENDPOINT_SET_ENDPOINT(lpc3511IpState, endpointIndex, 0U,
					    USB_LPC3511IP_ENDPOINT_STALL_MASK, 0U, 0U);
	if ((ep & USB_ENDPOINT_NUMBER_MASK) != USB_CONTROL_ENDPOINT) {
		/* toggle reset for the toggle */
		epState->stateUnion.stateBitField.epControlDefault |=
			((USB_LPC3511IP_ENDPOINT_TOGGLE_RESET_MASK) >>
			 USB_LPC3511IP_ENDPOINT_CONFIGURE_BITS_SHIFT);
		USB_LPC3511IP_ENDPOINT_SET_ENDPOINT(lpc3511IpState, endpointIndex, 1U,
						    USB_LPC3511IP_ENDPOINT_STALL_MASK, 0U,
						    0U);
	}
#if (defined(FSL_FEATURE_USBHSD_VERSION) && (FSL_FEATURE_USBHSD_VERSION >= 300U)) && \
	...
	if ((0U != lpc3511IpState->controllerSpeed) &&
		...
		lpc3511IpState->registerBase->DEVCMDSTAT |=
			(USB_LPC3511IP_DEVCMDSTAT_INTONNAK_AO_MASK |
	...
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState, uint8_t ep)
{
	...
	USB_LPC3511IP_ENDPOINT_SET_ENDPOINT_AND(lpc3511IpState, endpointIndex, 0,
						(~USB_LPC3511IP_ENDPOINT_STALL_MASK));
	if ((ep & USB_ENDPOINT_NUMBER_MASK) != USB_CONTROL_ENDPOINT) {
		USB_LPC3511IP_ENDPOINT_SET_ENDPOINT_AND(lpc3511IpState, endpointIndex, 1,
							(~USB_LPC3511IP_ENDPOINT_STALL_MASK));
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
static usb_status_t USB_DeviceLpc3511IpInterruptResume(
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState)
{
	usb_device_callback_message_struct_t message;
	...
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState)
{
	...
	lpc3511IpState->lpmRemoteWakeUp =
		(uint8_t)((lpc3511IpState->registerBase->DEVCMDSTAT &
			   USB_LPC3511IP_DEVCMDSTAT_LPM_REWP_MASK) >>
...
static void USB_DeviceLpc3511IpDoPreviousTransactionMemcpy(
	usb_device_lpc3511ip_state_struct_t *	lpc3511IpState,
	usb_device_lpc3511ip_endpoint_state_struct_t
	*					epState,
	uint32_t
	...
	odd)
{
	...
	if ((0U != epState->epBufferStatusUnion[odd].epBufferStatusField.epPacketCopyed) &&
	    (length > 0U) &&
	    ((endpointIndex & 0x01U) == 0U))
#else
	if ((0U != USB_DeviceLpcIp3511MaxPacketNeedCopy(lpc3511IpState)) &&
	    (0U != epState->epBufferStatusUnion[odd].epBufferStatusField.epPacketCopyed) &&
	    (length > 0U) &&
	    ((endpointIndex & 0x01U) == 0U))
#endif
#else
	...
	if (((endpointIndex >> 1U) == USB_CONTROL_ENDPOINT) &&
	    (epState->epBufferStatusUnion[odd].epBufferStatusField.epPacketCopyed) && (length >
										       0U) &&
	    ((endpointIndex & 0x01u) == 0U))
#endif
	{
#if ((defined(USB_DEVICE_IP3511_DISABLE_OUT_DOUBLE_BUFFER)) && \
		(USB_DEVICE_IP3511_DISABLE_OUT_DOUBLE_BUFFER > 0U))
...
static uint32_t USB_DeviceLpc3511IpTokenUpdate(
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState,
	usb_device_lpc3511ip_endpoint_state_struct_t *
	epState,
	uint8_t
...
/* for OUT packet, compute the actual packet size. */
...
#endif
	{
		...
		if (0U != lpc3511IpState->controllerSpeed) {
			length =
				(length & USB_LPC3511IPHS_ENDPOINT_BUFFER_NBYTES_MASK) >>
				USB_LPC3511IPHS_ENDPOINT_BUFFER_NBYTES_SHIFT;
		} else
#endif
		{
			length =
				(length & USB_LPC3511IPFS_ENDPOINT_BUFFER_NBYTES_MASK) >>
				USB_LPC3511IPFS_ENDPOINT_BUFFER_NBYTES_SHIFT;
		}
...
#if !((defined(USB_DEVICE_IP3511_DISABLE_OUT_DOUBLE_BUFFER)) && \
	(USB_DEVICE_IP3511_DISABLE_OUT_DOUBLE_BUFFER > 0U))
	USB_DeviceLpc3511IpDoPreviousTransactionMemcpy(lpc3511IpState, epState, length,
						       endpointIndex, odd);
#endif
...
static void USB_DeviceLpc3511IpInterruptToken(
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState,
	uint8_t
	endpointIndex,
	uint8_t		isSetup,
	uint32_t	errorStatus)
{
	...
	if ((0U != (epState->stateUnion.stateBitField.epControlDefault &
	    ...
	    (0U != lpc3511IpState->controllerSpeed) &&
	    (0U !=
	     (lpc3511IpState->epCommandStatusList[epState->stateUnion.stateBitField.consumerOdd +
						  (((uint32_t)endpointIndex) * 2U)] &
	      USB_LPC3511IP_ENDPOINT_TOGGLE_RESET_MASK))) {
		if (0U == (lpc3511IpState->registerBase->EPTOGGLE & ((uint32_t)(0x01UL <<
										endpointIndex)))) {
			uint32_t index;
			length = 0U;
			for (index = 0U; index < ((uint32_t)USB_DEVICE_IP3511_ENDPOINTS_NUM) * 4U;
			     ++index) {
				if ((0U !=
				     (lpc3511IpState->epCommandStatusList[index] &
				      USB_LPC3511IP_ENDPOINT_TOGGLE_RESET_MASK)) &&
				    (USB_ENDPOINT_INTERRUPT ==
				     lpc3511IpState->endpointState[index /
								   2U].stateUnion.stateBitField.
				     endpointType)) {
					length++;
				}
			}
			...
			epState->stateUnion.stateBitField.epControlDefault |=
				((USB_LPC3511IP_ENDPOINT_RFTV_MASK |
				  USB_LPC3511IP_ENDPOINT_ENDPOINT_TYPE_MASK) >>
				 USB_LPC3511IP_ENDPOINT_CONFIGURE_BITS_SHIFT);
			(void)USB_DeviceLpc3511IpTransaction(lpc3511IpState, epState,
							     endpointIndex);
#endif
		}
		return;
	...
	if (0U != isSetup) {
		...
		if (0U != (epState->stateUnion.stateBitField.transferring)) {
			...
			if (0U != (lpc3511IpState->epCommandStatusList[0] &
				   USB_LPC3511IP_ENDPOINT_ACTIVE_MASK)) {
				(void)USB_DeviceLpc3511IpCancel(lpc3511IpState,
								USB_CONTROL_ENDPOINT);
			}
		}
		if (0U !=
		    (lpc3511IpState->endpointState[1].stateUnion.stateBitField.transferring)) {
			lpc3511IpState->endpointState[1].stateUnion.stateBitField.transferring = 0U;
			if (0U != (lpc3511IpState->epCommandStatusList[2] &
				   USB_LPC3511IP_ENDPOINT_ACTIVE_MASK)) {
				(void)USB_DeviceLpc3511IpCancel(lpc3511IpState, (0x80u |
										 USB_CONTROL_ENDPOINT));
			}
		}
		...
		USB_LPC3511IP_ENDPOINT_SET_ENDPOINT_AND(
			lpc3511IpState, 0, 0, (~(USB_LPC3511IP_ENDPOINT_STALL_MASK |
						 USB_LPC3511IP_ENDPOINT_ACTIVE_MASK)));
		USB_LPC3511IP_ENDPOINT_SET_ENDPOINT_AND(
			lpc3511IpState, 1, 0, (~(USB_LPC3511IP_ENDPOINT_STALL_MASK |
						 USB_LPC3511IP_ENDPOINT_ACTIVE_MASK)));
...
#if (defined USB_DEVICE_IP3511_DOUBLE_BUFFER_ENABLE) && (USB_DEVICE_IP3511_DOUBLE_BUFFER_ENABLE)
		if (0U !=
		    (lpc3511IpState->epCommandStatusList[epState->stateUnion.stateBitField.
							 consumerOdd +
							 (((uint32_t)endpointIndex) * 2U)] &
		     USB_LPC3511IP_ENDPOINT_ACTIVE_MASK)) {
			return;
...
#else
		if (0U != (lpc3511IpState->epCommandStatusList[endpointIndex * 2U] &
			   USB_LPC3511IP_ENDPOINT_ACTIVE_MASK)) {
			return;
		}
#endif
...
#if (defined USB_DEVICE_IP3511_DOUBLE_BUFFER_ENABLE) && (USB_DEVICE_IP3511_DOUBLE_BUFFER_ENABLE)
		if ((endpointIndex >> 1U) != USB_CONTROL_ENDPOINT) {
			len = USB_DeviceLpc3511IpTokenUpdate(lpc3511IpState, epState, endpointIndex,
							     1U);
			length += len;
			...
			if ((epState->stateUnion.stateBitField.doubleBufferBusy > 0U) &&
				...
				USB_DeviceLpc3511IpDoPreviousTransactionMemcpy(
					...
					(uint8_t)(epState->stateUnion.stateBitField.consumerOdd ^
						  1U));
#endif
				len = USB_DeviceLpc3511IpTokenUpdate(lpc3511IpState, epState,
								     endpointIndex, 1U);
				length += len;
			}
		} else
#endif
		{
			length = USB_DeviceLpc3511IpTokenUpdate(lpc3511IpState, epState,
								endpointIndex, 0U);
			len = length;
		}
		...
		if ((length > 0U) && (0U == (length %
			...
			if ((0U != (endpointIndex & 0x01U)) && (0U != length) &&
				...
				if ((endpointIndex >> 1U) == USB_CONTROL_ENDPOINT) {
					 ...
					 */
					if (USB_SHORT_FROM_LITTLE_ENDIAN(setupPacket->wLength) >
					    epState->transferLength) {
						(void)USB_DeviceLpc3511IpEndpointPrime(
							lpc3511IpState, epState, 1U, NULL, 0U);
						return;
					}
				} else if ((0U != epState->stateUnion.stateBitField.zlt)) {
					(void)USB_DeviceLpc3511IpEndpointPrime(lpc3511IpState,
									       epState,
									       endpointIndex, NULL,
									       0U);
					return;
	...
	message.code = ((uint8_t)(endpointIndex >> 1) | (uint8_t)(((endpointIndex & 0x01U) <<
								   0x07U)));

#if ((defined(USB_DEVICE_IP3511_DISABLE_OUT_DOUBLE_BUFFER)) && \
	(USB_DEVICE_IP3511_DISABLE_OUT_DOUBLE_BUFFER > 0U))
	USB_DeviceLpc3511IpDoPreviousTransactionMemcpy(lpc3511IpState, epState, len, endpointIndex,
						       (uint8_t)(epState->stateUnion.stateBitField.
								 consumerOdd ^ 1U));
#endif
...
static void USB_DeviceLpc3511IpInterruptReset(usb_device_lpc3511ip_state_struct_t *lpc3511IpState)
{
	...
	if (0U != lpc3511IpState->controllerSpeed) {
		...
		} else if (((lpc3511IpState->registerBase->DEVCMDSTAT &
			     USBHSD_DEVCMDSTAT_Speed_MASK) >>
...
static usb_hsdcd_status_t USB_DeviceLpcIp3511IsrHSDCDCallback(void *handle, uint32_t event,
							      void *param)
{
	usb_hsdcd_status_t error = kStatus_hsdcd_Success;
	usb_device_callback_message_struct_t message;
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState =
		(usb_device_lpc3511ip_state_struct_t *)handle;
				     ...
				     usb_device_controller_handle *	controllerHandle)
{
	...
	if ((controllerId >= (uint8_t)kUSB_ControllerLpcIp3511Hs0) &&
		...
		lpc3511IpState = &s_UsbDeviceLpc3511IpState[controllerId -
							    (uint8_t)kUSB_ControllerLpcIp3511Hs0 +
							    USB_DEVICE_CONFIG_LPCIP3511FS];
		lpc3511IpState->controlData =
			(uint8_t *)&s_SetupAndEpReservedData[controllerId -
							     (uint8_t)kUSB_ControllerLpcIp3511Hs0 +
							     USB_DEVICE_CONFIG_LPCIP3511FS][
		...
		lpc3511IpState->epCommandStatusList =
			endpointListArray[controllerId - (uint8_t)kUSB_ControllerLpcIp3511Hs0 +
					  USB_DEVICE_CONFIG_LPCIP3511FS];
		/* get the ip base address */
		lpc3511IpState->registerBase =
			(USB_LPC3511IP_Type *)ip3511HsBases[controllerId -
							    (uint8_t)kUSB_ControllerLpcIp3511Hs0];
#if (defined(USB_DEVICE_CONFIG_CHARGER_DETECT) && (USB_DEVICE_CONFIG_CHARGER_DETECT > 0U)) && \
		(defined(FSL_FEATURE_SOC_USBHSDCD_COUNT) && (FSL_FEATURE_SOC_USBHSDCD_COUNT > 0U))
		base = (USBHSDCD_Type *)hsdcd_base[controllerId -
						   (uint8_t)kUSB_ControllerLpcIp3511Hs0];
...
#if ((defined(USB_DEVICE_CONFIG_LPCIP3511FS)) && (USB_DEVICE_CONFIG_LPCIP3511FS > 0U))
	{
		...
		if ((controllerId < (uint8_t)kUSB_ControllerLpcIp3511Fs0) ||
		    ((controllerId - (uint8_t)kUSB_ControllerLpcIp3511Fs0) >=
		     (uint8_t)USB_DEVICE_CONFIG_LPCIP3511FS) ||
		    (((uint32_t)controllerId - (uint32_t)kUSB_ControllerLpcIp3511Fs0) >=
		     (sizeof(ip3511FsBases) / sizeof(uint32_t)))) {
			return kStatus_USB_ControllerNotFound;
		}
		lpc3511IpState = &s_UsbDeviceLpc3511IpState[controllerId -
							    (uint8_t)kUSB_ControllerLpcIp3511Fs0];
		lpc3511IpState->controlData =
			(uint8_t *)&s_SetupAndEpReservedData[controllerId -
							     (uint8_t)kUSB_ControllerLpcIp3511Fs0]
			[CONTROL_TRANSFER_DATA_OFFSET];
		...
		lpc3511IpState->zeroTransactionData =
			...
			[ZERO_TRANSFER_DATA_OFFSET];
		/* set the endpoint list */
		lpc3511IpState->epCommandStatusList = endpointListArray[controllerId -
									(uint8_t)
									kUSB_ControllerLpcIp3511Fs0];
		/* get the ip base address */
		lpc3511IpState->registerBase =
			(USB_LPC3511IP_Type *)ip3511FsBases[controllerId -
							    (uint8_t)kUSB_ControllerLpcIp3511Fs0];
	}
#else
	{
...
#if (defined(FSL_FEATURE_USBHSD_INTERRUPT_DATAX_ISSUE_VERSION_CHECK) && \
		(FSL_FEATURE_USBHSD_INTERRUPT_DATAX_ISSUE_VERSION_CHECK))
		lpc3511IpState->hsInterruptIssue = ((Chip_GetVersion() == FSL_ROM_VERSION_1B) ? 0U :
						    1U);
#endif
	} else {
	...
	for (controllerId = 0; controllerId < ((USB_DEVICE_IP3511_BITS_FOR_RESERVED_BUFFER + 7U) /
					       8U); ++controllerId) {
...
{
	usb_device_lpc3511ip_state_struct_t *lpc3511IpState =
		(usb_device_lpc3511ip_state_struct_t *)controllerHandle;
	uint32_t usbAddress;
	...
	lpc3511IpState->registerBase->INTSTAT = (USB_LPC3511IP_INTSTAT_DEV_INT_MASK |
...
#define USB_DEVICE_IP3511_NEED_CHECK_BUFFER (1u)
		...
		{
#if ((defined(USB_DEVICE_CONFIG_LPCIP3511HS)) && (USB_DEVICE_CONFIG_LPCIP3511HS > 0U))
#if ((defined(FSL_FEATURE_USBHSD_USB_RAM_BASE_ADDRESS)) && \
			...
			if ((((uint32_t)actualBuffer & 0x0000003FU) != 0U) ||
			    ...
			    || ((0U != lpc3511IpState->controllerSpeed) &&
				((bufferValue <
				  (uint32_t)FSL_FEATURE_USBHSD_USB_RAM_BASE_ADDRESS) ||
				 (bufferValue >
				  ((uint32_t)FSL_FEATURE_USBHSD_USB_RAM_BASE_ADDRESS +
				   (uint32_t)FSL_FEATURE_USBHSD_USB_RAM))))
#endif
#endif
#if ((defined(USB_DEVICE_CONFIG_LPCIP3511HS)) && (USB_DEVICE_CONFIG_LPCIP3511HS > 0U))
#if ((defined(FSL_FEATURE_USBHSD_USB_RAM_BASE_ADDRESS)) && \
...
#endif
			{
				epState->epBufferStatusUnion[odd].epBufferStatusField.epPacketCopyed
					= 1U;
/* usb copy buffer for this packet */
#if (defined USB_DEVICE_IP3511_DOUBLE_BUFFER_ENABLE) && (USB_DEVICE_IP3511_DOUBLE_BUFFER_ENABLE)
				destBuffer =
					(uint8_t *)(epState->epPacketBuffer + (odd *
									       USB_LPC3511IP_GET_MULTIPLE_OF_64(
										       epState->
										       stateUnion.
										       stateBitField
...
#if ((defined(FSL_FEATURE_USB_VERSION) && (FSL_FEATURE_USB_VERSION >= 200U)) || \
					(defined(FSL_FEATURE_USBHSD_VERSION) && \
					(FSL_FEATURE_USBHSD_VERSION >= 300U)))
#else
					length = epState->stateUnion.stateBitField.maxPacketSize;
#endif
				}
	...
	if (0U == lpc3511IpState->isResetting) {
		return USB_DeviceLpc3511IpEndpointPrime(lpc3511IpState, epState, endpointIndex,
							actualBuffer, length);
	} else {
		return kStatus_USB_Error;
	}
	...
	endpointIndex)
{
...
#if (defined USB_DEVICE_IP3511_DOUBLE_BUFFER_ENABLE) && (USB_DEVICE_IP3511_DOUBLE_BUFFER_ENABLE)
		USB_LPC3511IP_ENDPOINT_SET_ENDPOINT(
			lpc3511IpState, endpointIndex,
			epState->stateUnion.stateBitField.producerOdd,
			(epState->stateUnion.stateBitField.epControlDefault <<
				USB_LPC3511IP_ENDPOINT_CONFIGURE_BITS_SHIFT) |
			(epState->stateUnion.stateBitField.stalled <<
				USB_LPC3511IP_ENDPOINT_STALL_SHIFT),
	...
	if ((epState->transferLength <= epState->transferPrimedLength) &&
	    (epState->transferLength != 0U)) {
		return kStatus_USB_Success;
	}
#if (defined USB_DEVICE_IP3511_DOUBLE_BUFFER_ENABLE) && (USB_DEVICE_IP3511_DOUBLE_BUFFER_ENABLE)
	if ((endpointIndex >> 1U) != USB_CONTROL_ENDPOINT) {
		...
		if (0U == (endpointIndex & 0x1U)) {
			status = USB_DeviceLpc3511IpGetActualBufferAndPrime(lpc3511IpState, epState,
									    endpointIndex, 1U);
		} else
#endif
		{
			do{
				status = USB_DeviceLpc3511IpGetActualBufferAndPrime(lpc3511IpState,
										    epState,
										    endpointIndex,
										    1U);
 ...
 */
#if 0
	if ((0U == length) && (USB_CONTROL_ENDPOINT == (endpointAddress &
...
usb_status_t USB_DeviceLpc3511IpCancel(usb_device_controller_handle controllerHandle, uint8_t ep)
{
	...
	if ((0U != epState->stateUnion.stateBitField.transferring)
	    ...
	    || (
		    ...
		    (0U !=
		     (lpc3511IpState->epCommandStatusList[epState->stateUnion.stateBitField.
							  ...
							  (((uint32_t)endpointIndex) * 2U)] &
		      USB_LPC3511IP_ENDPOINT_TOGGLE_RESET_MASK)) &&
		    (0U == (lpc3511IpState->registerBase->EPTOGGLE & ((uint32_t)(0x01UL <<
										 endpointIndex)))))
#endif
#endif
	    ) {
...
#endif
#endif
		if ((lpc3511IpState->registerBase->DEVCMDSTAT &
		     USB_LPC3511IP_DEVCMDSTAT_DCON_MASK) != 0U) {
			while (1U == 1U) {
				if ((lpc3511IpState->epCommandStatusList[(uint32_t)endpointIndex *
									 2U +
									 ((lpc3511IpState->
									   registerBase->EPINUSE &
									   (((
										     uint32_t)
									     0x00000001U <<
									     endpointIndex))) >>
					...
					lpc3511IpState->registerBase->EPSKIP |=
						((uint32_t)0x00000001U << endpointIndex);
					while (((lpc3511IpState->registerBase->EPSKIP &
						 ((uint32_t)0x00000001U << endpointIndex)) != 0U) &&
					       ((lpc3511IpState
						 ->epCommandStatusList[(uint32_t)endpointIndex *
								       2U +
								       ((lpc3511IpState->
									 registerBase->EPINUSE &
									 (((
					...
					if ((lpc3511IpState->registerBase->EPSKIP &
					     ((uint32_t)0x00000001U << endpointIndex)) != 0U) {
						lpc3511IpState->registerBase->EPSKIP &=
							(~((uint32_t)0x00000001U << endpointIndex));
					}
				...
				if (((lpc3511IpState->epCommandStatusList[endpointIndex * 2U] &
				      USB_LPC3511IP_ENDPOINT_ACTIVE_MASK) != 0U) ||
				    ((lpc3511IpState->epCommandStatusList[endpointIndex * 2U + 1U] &
				      USB_LPC3511IP_ENDPOINT_ACTIVE_MASK) != 0U)) {
					if ((lpc3511IpState->registerBase->EPINUSE &
					     (((uint32_t)0x00000001U << endpointIndex))) != 0U) {
			 ...
			 */
			lpc3511IpState->epCommandStatusList[endpointIndex * 2U] =
		...
		epState->stateUnion.stateBitField.producerOdd =
			((lpc3511IpState->registerBase->EPINUSE &
			  ((uint32_t)((uint32_t)0x00000001U << endpointIndex))) >> endpointIndex);
					...
					void *				param)
{
	...
	case kUSB_DeviceControlEndpointInit:
		if (NULL != param) {
			error = USB_DeviceLpc3511IpEndpointInit(lpc3511IpState,
								(usb_device_endpoint_init_struct_t *)
								param);
		}
		break;
		...
		if (NULL != param) {
			...
			if ((((endpointStatus->endpointAddress) & USB_ENDPOINT_NUMBER_MASK)) <
				...
				epState = USB_DeviceLpc3511IpGetEndpointStateStruct(
					lpc3511IpState, USB_LPC3511IP_ENDPOINT_DES_INDEX(
						endpointStatus->endpointAddress));
				endpointStatus->endpointStatus =
					(uint16_t)((epState->stateUnion.stateBitField.stalled ==
						    1U) ? kUSB_DeviceEndpointStateStalled :
	...
	case kUSB_DeviceControlResume:
		...
		while (0U != (lpc3511IpState->registerBase->DEVCMDSTAT &
			      USB_LPC3511IP_DEVCMDSTAT_DSUS_MASK)) {
		}
		/* the W1C bits */
		lpc3511IpState->registerBase->DEVCMDSTAT &=
			~(USB_LPC3511IP_DEVCMDSTAT_FORCE_NEEDCLK_MASK |
	...
	case kUSB_DeviceControlSleepResume:
		...
		while (0U != (lpc3511IpState->registerBase->DEVCMDSTAT &
			      USB_LPC3511IP_DEVCMDSTAT_LPM_SUS_MASK)) {
			__NOP();
		}
		/* the W1C bits */
		lpc3511IpState->registerBase->DEVCMDSTAT &=
	...
	case kUSB_DeviceControlSetDefaultStatus:
		for (tmp32Value = 0U; tmp32Value < (uint32_t)USB_DEVICE_IP3511_ENDPOINTS_NUM;
		     tmp32Value++) {
			(void)USB_DeviceLpc3511IpEndpointDeinit(lpc3511IpState,
								(uint8_t)(tmp32Value | (USB_IN <<
											0x07U)));
			(void)USB_DeviceLpc3511IpEndpointDeinit(lpc3511IpState,
								(uint8_t)(tmp32Value | (USB_OUT <<
											0x07U)));
		}
		USB_DeviceLpc3511IpSetDefaultState(lpc3511IpState);
		error = kStatus_USB_Success;
	...
	case kUSB_DeviceControlDcdEnable:
		if (kStatus_hsdcd_Success == USB_HSDCD_Control(lpc3511IpState->dcdHandle,
							       kUSB_DeviceHSDcdEnable, NULL)) {
			error = kStatus_USB_Success;
		}
		break;
	case kUSB_DeviceControlDcdDisable:
		if (kStatus_hsdcd_Success == USB_HSDCD_Control(lpc3511IpState->dcdHandle,
							       kUSB_DeviceHSDcdDisable, NULL)) {
			error = kStatus_USB_Success;
		}
		break;
	...
	if (0U != (interruptStatus & USB_LPC3511IP_INTSTAT_DEV_INT_MASK)) {
		...
		if ((0U == lpc3511IpState->deviceState) &&
...
#if ((defined(USB_DEVICE_CONFIG_LPCIP3511HS)) && (USB_DEVICE_CONFIG_LPCIP3511HS > 0U))
		     || ((0U != (lpc3511IpState->registerBase->LPM &
				 USB_LPC3511IP_USB_LPM_ADPPROBE_MASK)) &&
			...
			uint32_t debounceCount =
				USB_DEVICE_IP3511HS_LPM_ADPPROBE_ATTACH_DEBOUNCE_COUNT;
			if (1U == lpc3511IpState->controllerSpeed) {
				while ((0U != debounceCount) && (0U == (devState &
									USB_LPC3511IP_DEVCMDSTAT_VBUS_DEBOUNCED_MASK)))
				{
					if (0U == (lpc3511IpState->registerBase->LPM &
						   USB_LPC3511IP_USB_LPM_ADPPROBE_MASK)) {
						break;
					}
					debounceCount--;
			...
			if ((debounceCount == 0U) || (0U != (devState &
							     USB_LPC3511IP_DEVCMDSTAT_VBUS_DEBOUNCED_MASK)))
#endif
#endif
			{
				...
				(defined(USB_DEVICE_CONFIG_CHARGER_DETECT) && \
				(USB_DEVICE_CONFIG_CHARGER_DETECT > 0U))
				(void)USB_HSDCD_Control(lpc3511IpState->dcdHandle,
							kUSB_DeviceHSDcdRun, NULL);
#endif
			}
		}
		 ...
		 */
			 ...
			 (((0U != (lpc3511IpState->registerBase->DEVCMDSTAT &
...
#if ((defined FSL_FEATURE_SOC_USBPHY_COUNT) && (FSL_FEATURE_SOC_USBPHY_COUNT > 0U))
#if ((defined FSL_FEATURE_USBHSD_HAS_EXIT_HS_ISSUE) && (FSL_FEATURE_USBHSD_HAS_EXIT_HS_ISSUE > 0U))
#if ((defined USB_DEVICE_IP3511HS_FORCE_EXIT_HS_MODE_ENABLE) && \
			(USB_DEVICE_IP3511HS_FORCE_EXIT_HS_MODE_ENABLE > 0U))
...
/* Suspend/Resume */
		...
		if (0U != (devState & USB_LPC3511IP_DEVCMDSTAT_DSUS_C_MASK)) {
			if (0U != (lpc3511IpState->registerBase->DEVCMDSTAT &
				   USB_LPC3511IP_DEVCMDSTAT_DSUS_MASK)) {
				(void)USB_DeviceLpc3511IpInterruptSuspend(lpc3511IpState);
			}
#if (defined(USB_DEVICE_CONFIG_LPM_L1) && (USB_DEVICE_CONFIG_LPM_L1 > 0U))
			else if (0U != (lpc3511IpState->registerBase->DEVCMDSTAT &
					USB_LPC3511IP_DEVCMDSTAT_LPM_SUS_MASK)) {
		...
		/* OTG Status change */
		if (lpc3511IpState->registerBase->DEVCMDSTAT &
		    USB_LPC3511IP_DEVCMDSTAT_OTG_C_MASK) {
		}
#endif
	}
	...
	if (0U != (interruptStatus & USB_LPC3511IP_MAX_PHY_ENDPOINT_MASK)) {
		...
		if (0U != (interruptStatus & 0x01U)) {  /* control OUT */
			if (0U != (lpc3511IpState->registerBase->DEVCMDSTAT &
				...
				devState = 2U;
				if ((lpc3511IpState->endpointState[0].stateUnion.stateBitField.
				     stalled == 1U) ||
				    (lpc3511IpState->endpointState[1].stateUnion.stateBitField.
				     stalled == 1U)) {
					USB_LPC3511IP_ENDPOINT_SET_ENDPOINT_AND(
						lpc3511IpState, 0, 0,
						(~(USB_LPC3511IP_ENDPOINT_STALL_MASK |
					...
					USB_LPC3511IP_ENDPOINT_SET_ENDPOINT_AND(
						...
						(~(USB_LPC3511IP_ENDPOINT_STALL_MASK |
						   USB_LPC3511IP_ENDPOINT_ACTIVE_MASK)));
					lpc3511IpState->endpointState[0].stateUnion.stateBitField.
					stalled = 0U;
					lpc3511IpState->endpointState[1].stateUnion.stateBitField.
					stalled = 0U;
				}
				...
				USB_DeviceLpc3511IpInterruptToken(lpc3511IpState, 0U, 1,
								  usbErrorCode);
			}
		}
		...
		for (; devState < ((uint32_t)USB_DEVICE_IP3511_ENDPOINTS_NUM * 2U); ++devState) {
			...
			if (0U != (interruptStatus & (0x01UL << devState))) {
				USB_DeviceLpc3511IpInterruptToken(lpc3511IpState, (uint8_t)devState,
								  0U, usbErrorCode);
			}
		}
	}
