...
static usb_status_t USB_DeviceGetControllerInterface(uint8_t controllerId, const
						     usb_device_controller_interface_struct_t **
						     controllerInterface);
static usb_status_t USB_DeviceTransfer(usb_device_handle handle, uint8_t endpointAddress,
				       uint8_t *buffer, uint32_t length);
static usb_status_t USB_DeviceControl(usb_device_handle handle, usb_device_control_type_t type,
				      void *param);
...
static usb_status_t USB_DeviceAttachNotification(usb_device_struct_t *			handle,
						 usb_device_callback_message_struct_t * message);
#endif
static usb_status_t USB_DeviceNotification(usb_device_struct_t *			handle,
					   usb_device_callback_message_struct_t *	message);
	...
	uint8_t controllerId, const usb_device_controller_interface_struct_t **controllerInterface)
{
	...
	((defined(USB_DEVICE_CONFIG_LPCIP3511HS)) && (USB_DEVICE_CONFIG_LPCIP3511HS > 0U)))
	/* Get the EHCI controller driver interface */
	if ((kUSB_ControllerLpcIp3511Fs0 == controlerIndex) || (kUSB_ControllerLpcIp3511Fs1 ==
								controlerIndex) ||
	    (kUSB_ControllerLpcIp3511Hs0 == controlerIndex) || (kUSB_ControllerLpcIp3511Hs1 ==
								controlerIndex)) {
		*controllerInterface = (const
					usb_device_controller_interface_struct_t *)&
				       s_UsbDeviceLpc3511IpInterface;
		error = kStatus_USB_Success;
	}
#endif
#if ((defined(USB_DEVICE_CONFIG_DWC3)) && (USB_DEVICE_CONFIG_DWC3 > 0U))
				       ...
				       uint32_t			length)
{
	...
	if (NULL != deviceHandle->controllerInterface) {
		if (0U != deviceHandle->epCallback[(uint8_t)((uint32_t)endpoint << 1U) |
						   direction].isBusy) {
			return kStatus_USB_Busy;
		}
		OSA_ENTER_CRITICAL();
		deviceHandle->epCallback[(uint8_t)((uint32_t)endpoint << 1U) | direction].isBusy =
			1U;
		OSA_EXIT_CRITICAL();
		if (0U != (endpointAddress & USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK)) {
			...
			status = deviceHandle->controllerInterface->deviceSend(
			 ...
			 * USB_DeviceGetControllerInterface */
			status = deviceHandle->controllerInterface->deviceRecv(
				deviceHandle->controllerHandle, endpointAddress,
				buffer, length);
		}
		if (kStatus_USB_Success != status) {
			OSA_ENTER_CRITICAL();
			deviceHandle->epCallback[(uint8_t)((uint32_t)endpoint << 1U) |
						 direction].isBusy = 0U;
			OSA_EXIT_CRITICAL();
		}
	} else {
				      ...
				      void *param)
{
	...
	if (NULL != deviceHandle->controllerInterface) {
		 ...
		 * USB_DeviceGetControllerInterface */
		status = deviceHandle->controllerInterface->deviceControl(
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
static usb_status_t USB_DeviceDcdDetectFinihsedNotification(
	usb_device_struct_t *handle,
	usb_device_callback_message_struct_t *
	message)
{
	/* Call device callback to notify the application that the DCP facility is detected.
	 * the deviceCallback is the second parameter of USB_DeviceInit */
	return handle->deviceCallback(handle, kUSB_DeviceEventDcdDetectionfinished,
				      message->buffer);
}
#endif
					   ...
					   usb_device_callback_message_struct_t *	message)
{
	...
	default:
		if (endpoint < USB_DEVICE_CONFIG_ENDPOINTS) {
			if (NULL != handle->epCallback[(uint8_t)((uint32_t)endpoint << 1U) |
						       direction].callbackFn) {
				...
				if (0U != message->isSetup) {
					...
					handle->epCallback[(uint8_t)((uint32_t)endpoint << 1U) |
							   direction].isBusy = 0U;
				}
				/* Call endpoint callback, callbackFn is in the third parameter of USB_DeviceInitEndpoint */
				status = handle->epCallback[(uint8_t)((uint32_t)endpoint << 1U) |
							    direction].callbackFn(
					handle, &endpointCallbackMessage,
					handle->epCallback[(uint8_t)((uint32_t)endpoint << 1U) |
							   direction].callbackParam);
			}
		}
		break;
	}
...
usb_status_t USB_DeviceNotificationTrigger(void *handle, void *msg)
{
	...
	if (0U != deviceHandle->epCallbackDirectly) {
		if ((0U != (message->code & USB_ENDPOINT_NUMBER_MASK)) && (0U == (message->code &
										  0x70U))) {
	...
	if (KOSA_StatusSuccess != OSA_MsgQPut(deviceHandle->notificationQueue,
					      (osa_msg_handle_t)message)) {
		return kStatus_USB_Busy;
	}
	return kStatus_USB_Success;
#else
 ...
 */
usb_status_t USB_DeviceInit(uint8_t controllerId, usb_device_callback_t deviceCallback,
			    usb_device_handle *handle)
{
	...
	if (((usb_device_controller_init_t)NULL == deviceHandle->controllerInterface->deviceInit) ||
	    ((usb_device_controller_deinit_t)NULL ==
	    ...
	    ((usb_device_controller_cancel_t)NULL ==
	     deviceHandle->controllerInterface->deviceCancel) ||
	    ((usb_device_controller_control_t)NULL ==
	...
	deviceHandle->notificationQueue =
		(osa_msgq_handle_t)&deviceHandle->notificationQueueBuffer[0];
	if (KOSA_StatusSuccess !=
	    OSA_MsgQCreate(deviceHandle->notificationQueue, USB_DEVICE_CONFIG_MAX_MESSAGES,
	...
	*handle = deviceHandle;
	...
	error = deviceHandle->controllerInterface->deviceInit(controllerId, deviceHandle,
...
usb_status_t USB_DeviceDeinit(usb_device_handle handle)
{
	...
	if (NULL != deviceHandle->controllerInterface) {
		...
		(void)deviceHandle->controllerInterface->deviceDeinit(
			deviceHandle->controllerHandle);
		deviceHandle->controllerInterface =
			(usb_device_controller_interface_struct_t *)NULL;
	}
				   ...
				   uint8_t *buffer, uint32_t length)
{
	return USB_DeviceTransfer(
		handle,
		(endpointAddress & USB_ENDPOINT_NUMBER_MASK) | (USB_IN <<
								USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT),
		buffer, length);
}
				   ...
				   uint8_t *buffer, uint32_t length)
{
	return USB_DeviceTransfer(
		handle,
		(endpointAddress & USB_ENDPOINT_NUMBER_MASK) | (USB_OUT <<
								USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT),
		buffer, length);
}
...
usb_status_t USB_DeviceCancel(usb_device_handle handle, uint8_t endpointAddress)
{
	...
	if (NULL != deviceHandle->controllerInterface) {
		/* the callbackFn is initialized in USB_DeviceGetControllerInterface */
		status = deviceHandle->controllerInterface->deviceCancel(
			deviceHandle->controllerHandle, endpointAddress);
	} else {
		status = kStatus_USB_ControllerNotFound;
	}
				    ...
				    usb_device_endpoint_callback_struct_t *	epCallback)
{
	...
	if (endpoint < USB_DEVICE_CONFIG_ENDPOINTS) {
		deviceHandle->epCallback[(uint8_t)((uint32_t)endpoint << 1U) |
					 direction].callbackFn = epCallback->callbackFn;
		deviceHandle->epCallback[(uint8_t)((uint32_t)endpoint << 1U) |
					 direction].callbackParam =
...
{
	if ((endpointAddress & USB_ENDPOINT_NUMBER_MASK) < USB_DEVICE_CONFIG_ENDPOINTS) {
		return USB_DeviceControl(handle, kUSB_DeviceControlEndpointUnstall,
					 &endpointAddress);
	} else {
		return kStatus_USB_InvalidParameter;
	}
		...
		if ((uint8_t)kUSB_DeviceStateAddressing != ((usb_device_struct_t *)handle)->state) {
			if (NULL != param) {
				...
				((usb_device_struct_t *)handle)->state =
					(uint8_t)kUSB_DeviceStateAddressing;
				status = USB_DeviceControl(handle,
...
void USB_DeviceTaskFunction(void *deviceHandle)
{
	...
	if (NULL != deviceHandle) {
		...
		if (KOSA_StatusSuccess ==
		    OSA_MsgQGet(handle->notificationQueue, (osa_msg_handle_t)&message,
				USB_OSA_WAIT_TIMEOUT)) {
			/* Handle the message */
...
usb_status_t USB_DeviceUpdateHwTick(usb_device_handle handle, uint64_t tick)
{
	...
	status = USB_DeviceControl(handle, kUSB_DeviceControlUpdateHwTick,
				   (void *)(&deviceHandle->hwTick));
#endif
#elif (defined(USB_DEVICE_CONFIG_CHARGER_DETECT) && (USB_DEVICE_CONFIG_CHARGER_DETECT > 0U)) && \
	(defined(FSL_FEATURE_SOC_USB_ANALOG_COUNT) && (FSL_FEATURE_SOC_USB_ANALOG_COUNT > 0U))
