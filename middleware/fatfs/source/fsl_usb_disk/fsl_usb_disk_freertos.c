 ...
 */
static void USB_HostMsdUfiCallback(void *param, uint8_t *data, uint32_t dataLength, usb_status_t
				   status);
...
DSTATUS USB_HostMsdInitializeDisk(BYTE pdrv)
{
	...
	if (USB_HostMsdTestUnitReady(g_UsbFatfsClassHandle, 0, USB_HostMsdUfiCallback, NULL) !=
	    kStatus_USB_Success) {
		return STA_NOINIT;
	}
	...
	if (USB_HostMsdRequestSense(g_UsbFatfsClassHandle, 0, s_UsbTransferBuffer,
				    sizeof(usb_host_ufi_sense_data_t),
	...
	if (USB_HostMsdReadCapacity(g_UsbFatfsClassHandle, 0, s_UsbTransferBuffer,
				    sizeof(usb_host_ufi_read_capacity_t),
				    USB_HostMsdUfiCallback, NULL) != kStatus_USB_Success) {
		return STA_NOINIT;
	} else {
		if (pdTRUE != xSemaphoreTake(s_CommandSemaphore, portMAX_DELAY)) { /* wait the command */
		...
		if (ufiStatus == kStatus_USB_Success) {
			...
			address =
				(uint32_t)((usb_host_ufi_read_capacity_t *)(address))->
				blockLengthInBytes;
...
DRESULT USB_HostMsdReadDisk(BYTE pdrv, BYTE *buff, LBA_t sector, UINT count)
{
	...
	sectorIndex = sector;
		...
		while (retry--) {
			...
			status = USB_HostMsdRead10(g_UsbFatfsClassHandle, 0, sectorIndex,
						   (uint8_t *)transferBuf,
						   (uint32_t)(s_FatfsSectorSize * sectorCount),
						   sectorCount, USB_HostMsdUfiCallback, NULL);
	...
	DRESULT USB_HostMsdWriteDisk(BYTE pdrv, const BYTE *buff, LBA_t sector, UINT count)
	{
		...
		for (index = 0; index < count; ++index) {
			sectorIndex = sector + index;
			memcpy(s_UsbTransferBuffer, buff + index * s_FatfsSectorSize,
			       s_FatfsSectorSize);
#else
		transferBuf = buff;
		sectorCount = count;
		sectorIndex = sector;
			...
			while (retry--) {
				...
				status = USB_HostMsdWrite10(g_UsbFatfsClassHandle, 0, sectorIndex,
							    (uint8_t *)transferBuf,
							    (uint32_t)(s_FatfsSectorSize *
								       sectorCount), sectorCount,
							    USB_HostMsdUfiCallback, NULL);
				if (status != kStatus_USB_Success) {
					fatfs_code = RES_ERROR;
				} else {
					if (pdTRUE != xSemaphoreTake(s_CommandSemaphore,
								     portMAX_DELAY)) { /* wait the command */
						return RES_ERROR;
					}
					if (ufiStatus == kStatus_USB_Success) {
						fatfs_code = RES_OK;
		...
		DRESULT USB_HostMsdIoctlDisk(BYTE pdrv, BYTE cmd, void *buff)
		{
			...
			case GET_SECTOR_SIZE:
				...
				status = USB_HostMsdReadCapacity(g_UsbFatfsClassHandle, 0,
								 s_UsbTransferBuffer,
								 sizeof(usb_host_ufi_read_capacity_t),
								 USB_HostMsdUfiCallback, NULL);
				if (status != kStatus_USB_Success) {
				...
				if (fatfs_code == RES_OK) {
					if (GET_SECTOR_COUNT == cmd) { /* Get number of sectors on the disk (DWORD) */
						address = (uint32_t)&s_UsbTransferBuffer[0];
						address =
							(uint32_t)((usb_host_ufi_read_capacity_t *)(
									   address))
							->lastLogicalBlockAddress;
						value = USB_LONG_FROM_BIG_ENDIAN_ADDRESS(
							((uint8_t *)address));
