...
static void HAL_TimerInterruptHandle(uint8_t instance)
{
	...
	MRT_ClearStatusFlags(s_MrtBase[halTimerState->instance], kMRT_Channel_0,
			     (uint32_t)kMRT_TimerInterruptFlag);
				 ...
				 hal_timer_config_t *	halTimerConfig)
{
	...
	MRT_EnableInterrupts(s_MrtBase[halTimerState->instance], kMRT_Channel_0,
			     (uint32_t)kMRT_TimerInterruptEnable);
...
void HAL_TimerEnable(hal_timer_handle_t halTimerHandle)
{
	...
	MRT_StartTimer(s_MrtBase[halTimerState->instance], kMRT_Channel_0,
		       (uint32_t)USEC_TO_COUNT(halTimerState->timeout,
...
uint32_t HAL_TimerGetCurrentTimerCount(hal_timer_handle_t halTimerHandle)
{
	assert(halTimerHandle);
	hal_timer_handle_struct_t *halTimerState = halTimerHandle;
	return (uint32_t)COUNT_TO_USEC(MRT_GetCurrentTimerCount(s_MrtBase[halTimerState->instance],
								kMRT_Channel_0),
				       halTimerState->timerClock_Hz);
}
