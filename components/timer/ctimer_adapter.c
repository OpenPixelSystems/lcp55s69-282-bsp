...
static void HAL_TimerInterruptHandle(uint8_t instance)
{
	hal_timer_handle_struct_t *halTimerState =
		(hal_timer_handle_struct_t *)s_timerHandle[instance];
...
static ctimer_callback_t ctimer_callback_table[] = { ctimer0_match0_callback,
						     ctimer1_match0_callback };

static hal_timer_status_t HAL_CTimerConfigTimeout(hal_timer_handle_t halTimerHandle, uint32_t
						  timeout)
{
	...
	mCtimerMatchConfig.matchValue = (uint32_t)USEC_TO_COUNT(halTimerState->timeout,
	...
	CTIMER_RegisterCallBack(s_CtimerBase[halTimerState->instance],
				&ctimer_callback_table[halTimerState->instance],
				kCTIMER_SingleCallback);
	CTIMER_SetupMatch(s_CtimerBase[halTimerState->instance],
			  (ctimer_match_t)gStackTimerChannel_c, &mCtimerMatchConfig);
				 ...
				 hal_timer_config_t *	halTimerConfig)
{
	...
	halTimerState->timerClock_Hz = (uint32_t)halTimerConfig->srcClock_Hz /
				       (uint32_t)(config.prescale + 1U);
...
uint32_t HAL_TimerGetCurrentTimerCount(hal_timer_handle_t halTimerHandle)
{
	assert(halTimerHandle);
	hal_timer_handle_struct_t *halTimerState = halTimerHandle;
	return (uint32_t)COUNT_TO_USEC(s_CtimerBase[halTimerState->instance]->TC,
				       halTimerState->timerClock_Hz);
}
