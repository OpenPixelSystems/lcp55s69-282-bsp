				 ...
				 hal_pwm_setup_config_t *setupConfig)
{
	...
	if (kStatus_Success != CTIMER_SetupPwm(s_cTimerBase[halPwmState->instance],
					       (ctimer_match_t)channel,
					       setupConfig->dutyCyclePercent,
					       setupConfig->pwmFreq_Hz,
					...
					uint8_t			dutyCyclePercent)
{
	...
	CTIMER_UpdatePwmDutycycle(s_cTimerBase[halPwmState->instance], (ctimer_match_t)channel,
				  duty);
	return kStatus_HAL_PwmSuccess;
}
