...
#if (defined(HAL_UART_TRANSFER_MODE) && (HAL_UART_TRANSFER_MODE > 0U))
static void HAL_UartCallback(USART_Type *base, usart_handle_t *handle, status_t status,
			     void *callbackParam)
{
...
static void HAL_UartInterruptHandle(USART_Type *base, void *handle)
{
	...
	if ((USART_FIFOSTAT_RXNOTEMPTY_MASK & status) &&
	    (USART_GetEnabledInterrupts(s_UsartAdapterBase[instance]) &
	     USART_FIFOINTENSET_RXLVL_MASK)) {
		if (uartHandle->rx.buffer) {
			uartHandle->rx.buffer[uartHandle->rx.bufferSofar++] = USART_ReadByte(
				s_UsartAdapterBase[instance]);
	...
	if ((USART_FIFOSTAT_TXNOTFULL_MASK & status) &&
		...
		if (uartHandle->tx.buffer) {
			USART_WriteByte(s_UsartAdapterBase[instance],
...
#if (defined(HAL_UART_TRANSFER_MODE) && (HAL_UART_TRANSFER_MODE > 0U))
	USART_TransferCreateHandle(s_UsartAdapterBase[config->instance],
				   &uartHandle->hardwareHandle,
				   (usart_transfer_callback_t)HAL_UartCallback, handle);
#else
	/* Enable interrupt in NVIC. */
	FLEXCOMM_SetIRQHandler(s_UsartAdapterBase[config->instance],
			       (flexcomm_irq_handler_t)HAL_UartInterruptHandle,
			       handle);
						     ...
						     hal_uart_transfer_t *	transfer)
{
	...
	status = USART_TransferReceiveNonBlocking(s_UsartAdapterBase[uartHandle->instance],
						  &uartHandle->hardwareHandle,
						  ...
						  hal_uart_transfer_t * transfer)
{
	...
	status = USART_TransferSendNonBlocking(s_UsartAdapterBase[uartHandle->instance],
					       &uartHandle->hardwareHandle,
...
hal_uart_status_t HAL_UartTransferGetReceiveCount(hal_uart_handle_t handle, uint32_t *count)
{
	...
	status =
		USART_TransferGetReceiveCount(s_UsartAdapterBase[uartHandle->instance],
...
hal_uart_status_t HAL_UartTransferGetSendCount(hal_uart_handle_t handle, uint32_t *count)
{
	...
	status = USART_TransferGetSendCount(s_UsartAdapterBase[uartHandle->instance],
					    &uartHandle->hardwareHandle, count);
...
hal_uart_status_t HAL_UartTransferAbortReceive(hal_uart_handle_t handle)
{
	...
	USART_TransferAbortReceive(s_UsartAdapterBase[uartHandle->instance],
				   &uartHandle->hardwareHandle);
...
hal_uart_status_t HAL_UartTransferAbortSend(hal_uart_handle_t handle)
{
	...
	USART_TransferAbortSend(s_UsartAdapterBase[uartHandle->instance],
				&uartHandle->hardwareHandle);
...
#endif
	USART_TransferHandleIRQ(s_UsartAdapterBase[uartHandle->instance],
				&uartHandle->hardwareHandle);
#if 0
