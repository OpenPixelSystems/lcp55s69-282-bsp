...
osa_status_t OSA_TaskSetPriority(osa_task_handle_t taskHandle, osa_task_priority_t taskPriority)
{
	...
	if (ptaskStruct->priority > tcb->priority) {
		...
		(void)LIST_AddTail(&s_osaState.taskList,
				   (list_element_handle_t)(void *)&(ptaskStruct->link));
			    ...
			    osa_task_param_t task_param)
{
	...
	if ((NULL == tcb) || (ptaskStruct->priority > tcb->priority)) {
		OSA_EnterCritical(&regPrimask);
		listStatus = LIST_AddTail(&s_osaState.taskList,
					  (list_element_handle_t)(void *)&(ptaskStruct->link));
...
osa_status_t OSA_SemaphoreWait(osa_semaphore_handle_t semaphoreHandle, uint32_t millisec)
{
	...
	if (0U != pSemStruct->semCount) {
		...
		if (0U == millisec) {
			...
			if (pSemStruct->timeout < OSA_TimeDiff(pSemStruct->time_start,
							       currentTime)) {
				OSA_EnterCritical(&regPrimask);
				pSemStruct->isWaiting = 0U;
				OSA_ExitCritical(regPrimask);
...
osa_status_t OSA_MutexLock(osa_mutex_handle_t mutexHandle, uint32_t millisec)
{
	...
	if (0U == pMutexStruct->isLocked) {
		...
		if (0U == millisec) {
			...
			/* Check for timeout */
			currentTime = OSA_TimeGetMsec();
			if (pMutexStruct->timeout < OSA_TimeDiff(pMutexStruct->time_start,
								 currentTime)) {
				OSA_EnterCritical(&regPrimask);
				pMutexStruct->isWaiting = 0U;
				OSA_ExitCritical(regPrimask);
	...
	*pSetFlags = pEventStruct->flags & flagsToWait;
	...
	if (((0U == waitAll) && (0U != *pSetFlags)) || (*pSetFlags == flagsToWait)) {
		...
		if (0U == millisec) {
			...
			/* Check for timeout */
			currentTime = OSA_TimeGetMsec();
			if (pEventStruct->timeout < OSA_TimeDiff(pEventStruct->time_start,
								 currentTime)) {
				pEventStruct->isWaiting = 0U;
				retVal = KOSA_StatusTimeout;
			}
...
int main(void)
{
	...
	BOARD_InitHardware();
	OSA_TimeInit();
	(void)OSA_TaskCreate((osa_task_handle_t)s_osaState.mainTaskHandle, OSA_TASK(main_task),
			     NULL);
	OSA_Start();
