...
#define MSEC_TO_TICK(msec) \
	(((uint32_t)(msec) + 500uL / (uint32_t)configTICK_RATE_HZ) * (uint32_t)configTICK_RATE_HZ / \
	 1000uL)
...
osa_status_t OSA_EventSet(osa_event_handle_t eventHandle, osa_event_flags_t flagsToSet)
{
	...
	if (0U != __get_IPSR()) {
		result = xEventGroupSetBitsFromISR(pEventStruct->handle, (event_flags_t)flagsToSet,
						   &taskToWake);
...
osa_status_t OSA_EventClear(osa_event_handle_t eventHandle, osa_event_flags_t flagsToClear)
{
	...
	if (0U != __get_IPSR()) {
		(void)xEventGroupClearBitsFromISR(pEventStruct->handle,
			   ...
			   osa_event_flags_t *	pSetFlags)
{
	...
	flagsSave = xEventGroupWaitBits(pEventStruct->handle, (event_flags_t)flagsToWait, clearMode,
					(BaseType_t)waitAll,
					timeoutTicks);
...
osa_status_t OSA_MsgQPut(osa_msgq_handle_t msgqHandle, osa_msg_handle_t pMessage)
{
	...
	if (0U != __get_IPSR()) {
		...
		osaStatus = (xQueueSendToBack(handler, pMessage, 0) == pdPASS) ?
			    (KOSA_StatusSuccess) : (KOSA_StatusError);
	}
...
int main(void)
{
	...
	(void)OSA_TaskCreate((osa_task_handle_t)s_osaState.mainTaskHandle, OSA_TASK(startup_task),
			     NULL);
