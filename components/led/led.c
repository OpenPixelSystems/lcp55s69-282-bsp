...
static led_status_t LED_SetStatus(led_state_t *ledState, led_color_t color, uint32_t threshold)
{
	...
	for (uint8_t i = 0; i < count; i++) {
		...
		if (ledRgbPin[i].config.dimmingEnable) {
			HAL_PwmUpdateDutycycle(ledState->pwmHandle[i], ledRgbPin[i].dimming.channel,
					       kHAL_EdgeAlignedPwm,
					       colorSet * 100 / 255);
		} else
#endif
	...
	while (NULL != ledState) {
		if (s_ledList.periodCount >= ledState->expiryPeriodCount) {
			...
			case (uint16_t)kLED_CycleFlash:         /*!< Cycle Flash */
				...
				if (LED_FLASH_CYCLE_FOREVER != ledState->flashCycle) {
					if ((0U != ledState->flashCycle)) {
						if (((led_color_t)kLED_Black ==
						     ledState->currentColor) || (100U ==
										 ledState->flashDuty))
						{
							ledState->flashCycle--;
						}
				...
				if (0U != ledState->flashCycle) {
					...
					if (((led_color_t)kLED_Black == ledState->currentColor)) {
						threshold = (uint32_t)ledState->flashPeriod *
							    (100U - (uint32_t)ledState->flashDuty) /
							    100U;
					} else {
#if (defined(LED_COLOR_WHEEL_ENABLEMENT) && (LED_COLOR_WHEEL_ENABLEMENT > 0U))
						ledState->flashCount++;
						if ((uint16_t)kLED_TricolorCycleFlash ==
						    ledState->controlType) {
							ledState->currentColor =
								(led_color_t)(0xFFUL <<
									      ((ledState->flashCount
										%
										3U) * 8U));
						} else if ((uint16_t)kLED_CycleFlash ==
							   ledState->controlType) {
							color = 0;
							if (0U == ((ledState->flashCount) &
								   0x07U)) {
								(ledState->flashCount)++;
							}
							if (0U != ((ledState->flashCount) &
...
#endif /* (defined(LED_COLOR_WHEEL_ENABLEMENT) && (LED_COLOR_WHEEL_ENABLEMENT > 0U)) */
						threshold = (uint32_t)ledState->flashPeriod *
							    (uint32_t)(ledState->flashDuty) / 100U;
					}
			...
			case kLED_Dimming: /*!< Dimming */
				...
				ledState->currentColor = LED_MAKE_COLOR(power[0], power[1],
									power[2]);
				LED_SetStatus(ledState, ledState->currentColor,
					      ledState->flashPeriod);
				break;
#endif
			default:
				/* MISRA Rule 16.4*/
...
led_status_t LED_Init(led_handle_t ledHandle, led_config_t *ledConfig)
{
	...
	if (NULL == s_ledList.ledState) {
		do{
			if (kStatus_TimerSuccess != TM_Open(
				    (timer_handle_t)s_ledList.timerHandle)) {
				break;
			}
			if (kStatus_TimerSuccess != TM_InstallCallback(s_ledList.timerHandle,
								       LED_TimerEvent,
								       &s_ledList)) {
				break;
			}
			if (kStatus_TimerSuccess != TM_Start(s_ledList.timerHandle,
	...
	for (i = 0; i < (int)count; i++) {
		...
		if (ledRgbConfigPin[i].dimmingEnable) {
			...
			HAL_PwmInit((hal_pwm_handle_t)ledState->pwmHandle[i],
				    ledRgbConfigPin[i].dimming.instance,
				    ledRgbConfigPin[i].dimming.sourceClock);
			setupConfig.dutyCyclePercent = 0;
			setupConfig.level = ledRgbConfigPin[i].dimming.pinStateDefault ?
					    kHAL_PwmLowTrue : kHAL_PwmHighTrue;
			setupConfig.mode = kHAL_EdgeAlignedPwm;
			setupConfig.pwmFreq_Hz = 1000;
			HAL_PwmSetupPwm(ledState->pwmHandle[i], ledRgbConfigPin[i].dimming.channel,
					&setupConfig);
		} else
#endif
		{
			...
			ledState->pins[i].gpio.pinStateDefault =
				ledRgbConfigPin[i].gpio.pinStateDefault;
			...
			if (kStatus_HAL_GpioSuccess != HAL_GpioInit(
				    (hal_gpio_handle_t)ledState->gpioHandle[i], &controlPin)) {
				return kStatus_LED_Error;
			}
		}
	}
...
led_status_t LED_TurnOnOff(led_handle_t ledHandle, uint8_t turnOnOff)
{
	...
	ledState->currentColor = (1U == turnOnOff) ? ledState->settingColor :
				 (led_color_t)kLED_Black;
...
led_status_t LED_Flash(led_handle_t ledHandle, led_flash_config_t *ledFlash)
{
	...
	status = LED_SetStatus(ledState, ledState->currentColor,
			       ((uint32_t)ledState->flashPeriod * (uint32_t)ledState->flashDuty) /
			       100U);
	return status;
}
...
led_status_t LED_Dimming(led_handle_t ledHandle, uint16_t dimmingPeriod, uint8_t increasement)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (ledState->dimming.increasement) {
			ledState->dimming.powerDelta[i] = (0xFF - value) *
							  LED_DIMMING_UPDATE_INTERVAL /
							  (dimmingPeriod);
			...
			ledState->dimming.powerDelta[i] = (value) * LED_DIMMING_UPDATE_INTERVAL /
							  (dimmingPeriod);
			if (value > ledState->dimming.powerDelta[i]) {
				power[i] = value - ledState->dimming.powerDelta[i];
			} else {
...
led_status_t LED_EnterLowpower(led_handle_t ledHandle)
{
	...
	for (int i = 0; i < count; i++) {
		...
		if (ledRgbPin[i].dimmingEnable) {
			HAL_PwmEnterLowpower(ledRgbPin[i].dimming.instance,
					     ledRgbPin[i].dimming.channel);
		} else
#endif
		{
...
#if (defined(LED_DIMMING_ENABLEMENT) && (LED_DIMMING_ENABLEMENT > 0U))
		if (ledRgbPin[i].dimmingEnable) {
			HAL_PwmExitLowpower(ledRgbPin[i].dimming.instance,
					    ledRgbPin[i].dimming.channel);
		} else
#endif
		{
