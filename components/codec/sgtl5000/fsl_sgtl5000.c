...
status_t SGTL_Init(sgtl_handle_t *handle, sgtl_config_t *config)
{
	...
	SGTL_ConfigDataFormat(handle, config->format.mclk_HZ, config->format.sampleRate,
			      config->format.bitWidth);

	/* sclk valid edge */
	if (config->format.sclkEdge == kSGTL_SclkValidEdgeRising) {
		SGTL_ModifyReg(handle, CHIP_I2S_CTRL, SGTL5000_I2S_SCLK_INV_CLR_MASK,
...
{
	if (master == 1) {
		SGTL_ModifyReg(handle, CHIP_I2S_CTRL, SGTL5000_I2S_MS_CLR_MASK,
			       SGTL5000_I2S_MASTER);
	} else {
...
status_t SGTL_SetDataRoute(sgtl_handle_t *handle, sgtl_route_t route)
{
	...
	case kSGTL_RouteBypass:
		...
		SGTL_ModifyReg(handle, CHIP_ANA_CTRL, SGTL5000_SEL_HP_CLR_MASK,
	...
	case kSGTL_RoutePlayback:
		...
		SGTL_ModifyReg(handle, CHIP_SSS_CTRL, SGTL5000_DAC_SEL_CLR_MASK,
	...
	case kSGTL_RoutePlaybackandRecord:
		...
		SGTL_ModifyReg(handle, CHIP_ANA_CTRL, SGTL5000_SEL_HP_CLR_MASK,
			       SGTL5000_SEL_HP_DAC);
		SGTL_ModifyReg(handle, CHIP_ANA_CTRL, SGTL5000_SEL_ADC_CLR_MASK,
			       SGTL5000_SEL_ADC_LINEIN);
		SGTL_ModifyReg(handle, CHIP_SSS_CTRL, SGTL5000_I2S_OUT_SEL_CLR_MASK,
	...
	case kSGTL_RoutePlaybackwithDAP:
		...
		SGTL_ModifyReg(handle, CHIP_SSS_CTRL, SGTL5000_DAP_SEL_CLR_MASK,
...
status_t SGTL_SetProtocol(sgtl_handle_t *handle, sgtl_protocol_t protocol)
{
	...
	switch (protocol) {
	case kSGTL_BusI2S:
		SGTL_ModifyReg(handle, CHIP_I2S_CTRL, SGTL5000_I2S_MODE_CLR_MASK,
			       SGTL5000_I2S_MODE_I2S_LJ);
		SGTL_ModifyReg(handle, CHIP_I2S_CTRL, SGTL5000_I2S_LRALIGN_CLR_MASK,
...
uint32_t SGTL_GetVolume(sgtl_handle_t *handle, sgtl_module_t module)
{
	...
	case kSGTL_ModuleADC:
		...
		vol = (vol & (uint16_t)SGTL5000_ADC_VOL_LEFT_GET_MASK) >>
		      SGTL5000_ADC_VOL_LEFT_SHIFT;
		break;
	case kSGTL_ModuleDAC:
		SGTL_ReadReg(handle, CHIP_DAC_VOL, &vol);
		vol = (vol & (uint16_t)SGTL5000_DAC_VOL_LEFT_GET_MASK) >>
	...
	case kSGTL_ModuleLineOut:
		...
		vol = (vol & (uint16_t)SGTL5000_LINE_OUT_VOL_LEFT_GET_MASK) >>
			       ...
			       bits)
{
	...
	switch (bits) {
	case 16:
		SGTL_ModifyReg(handle, CHIP_I2S_CTRL, SGTL5000_I2S_DLEN_CLR_MASK,
...
status_t SGTL_WriteReg(sgtl_handle_t *handle, uint16_t reg, uint16_t val)
{
	...
	return CODEC_I2C_Send(handle->i2cHandle, handle->config->slaveAddress, reg, 2U,
...
status_t SGTL_ReadReg(sgtl_handle_t *handle, uint16_t reg, uint16_t *val)
{
	...
	retval = CODEC_I2C_Receive(handle->i2cHandle, handle->config->slaveAddress, reg, 2U,
