...
static const da7212_register_value_t kOutputRegisterSequence[kDA7212_Output_MAX][4] = {
	/* DA7212_Output_HP */
	{
		...
		{ DIALOG7212_HP_L_CTRL, (DIALOG7212_HP_L_CTRL_AMP_EN_MASK |
					 DIALOG7212_HP_L_CTRL_AMP_RAMP_EN_MASK |
					 DIALOG7212_HP_L_CTRL_AMP_ZC_EN_MASK |
					 DIALOG7212_HP_L_CTRL_AMP_OE_MASK) },
		{ DIALOG7212_HP_R_CTRL, (DIALOG7212_HP_R_CTRL_AMP_EN_MASK |
...
status_t DA7212_WriteRegister(da7212_handle_t *handle, uint8_t u8Register, uint8_t u8RegisterData)
{
	...
	return CODEC_I2C_Send(handle->i2cHandle, handle->config->slaveAddress, u8Register, 1U,
			      (uint8_t *)&writeValue, 1U);
}
...
status_t DA7212_Init(da7212_handle_t *handle, da7212_config_t *codecConfig)
{
	...
	if (CODEC_I2C_Init(handle->i2cHandle, config->i2cConfig.codecI2CInstance,
			   DA7212_I2C_BAUDRATE,
	...
	for (i = 0; i < DA7212_INIT_SIZE; i++) {
		DA7212_WriteRegister(handle, kInitRegisterSequence[i].addr,
				     kInitRegisterSequence[i].value);
	}
	...
	DA7212_WriteRegister(handle, DIALOG7212_DAI_CTRL, DIALOG7212_DAI_EN_MASK |
			     config->protocol);
	...
	DA7212_ConfigAudioFormat(handle, sysClock, config->format.sampleRate,
...
status_t DA7212_SetPLLConfig(da7212_handle_t *handle, da7212_pll_config_t *config)
{
	...
	pllValue =
		(uint64_t)(((uint64_t)((((uint64_t)config->outputClock_HZ * 8) * inputDiv) << 13)) /
			   (config->refClock_HZ));
...
{
	return DA7212_WriteRegister(handle, DIALOG7212_DAI_CTRL, (DIALOG7212_DAI_EN_MASK |
								  protocol));
}
...
{
	uint32_t i = 0;
	uint32_t seqSize = sizeof(kInputRegisterSequence[DA7212_Input]) /
			   sizeof(da7212_register_value_t);
...
{
	uint32_t i = 0;
	uint32_t seqSize = sizeof(kOutputRegisterSequence[DA7212_Output]) /
			   sizeof(da7212_register_value_t);
