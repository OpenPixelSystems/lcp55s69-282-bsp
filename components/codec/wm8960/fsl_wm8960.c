...
static const uint16_t wm8960_reg[WM8960_CACHEREGNUM] = {
	0x0097, 0x0097, 0x0000, 0x0000, 0x0000, 0x0008, 0x0000, 0x000a, 0x01c0, 0x0000, 0x00ff,
	0x00ff, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x007b, 0x0100, 0x0032, 0x0000, 0x00c3, 0x00c3, 0x01c0, 0x0000,
	0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0100, 0x0100, 0x0050, 0x0050, 0x0050, 0x0050, 0x0000,
	0x0000, 0x0000, 0x0000,
	0x0040, 0x0000, 0x0000, 0x0050, 0x0050, 0x0000, 0x0002, 0x0037, 0x004d, 0x0080, 0x0008,
	0x0031, 0x0026, 0x00e9,
};
...
status_t WM8960_Init(wm8960_handle_t *handle, const wm8960_config_t *wm8960Config)
{
	...
	return WM8960_ConfigDataFormat(handle, config->format.mclk_HZ, config->format.sampleRate,
...
{
	if (master == 1) {
		WM8960_ModifyReg(handle, WM8960_IFACE1, WM8960_IFACE1_MS_MASK, WM8960_IFACE1_MS(
...
{
	return WM8960_ModifyReg(handle, WM8960_IFACE1, WM8960_IFACE1_FORMAT_MASK |
				WM8960_IFACE1_LRP_MASK, protocol);
}
...
status_t WM8960_ConfigDataFormat(wm8960_handle_t *handle, uint32_t sysclk, uint32_t sample_rate,
				 uint32_t bits)
{
...
status_t WM8960_WriteReg(wm8960_handle_t *handle, uint8_t reg, uint16_t val)
{
	...
	return CODEC_I2C_Send(handle->i2cHandle, handle->config->slaveAddress, cmd, 1U,
			      (uint8_t *)&buff, 2U);
}
