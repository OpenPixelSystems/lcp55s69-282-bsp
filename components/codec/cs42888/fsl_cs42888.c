...
status_t CS42888_Init(cs42888_handle_t *handle, cs42888_config_t *config)
{
	...
	errorStatus = CODEC_I2C_Init(handle->i2cHandle, config->i2cConfig.codecI2CInstance,
				     CS42888_I2C_BITRATE,
	...
	if (!config->master) {
		/* set as slave */
		errorStatus = CS42888_SelectFunctionalMode(handle, kCS42888_ModeSlave,
							   kCS42888_ModeSlave);
		...
		/* set as master */
		errorStatus = CS42888_SelectFunctionalMode(handle, config->ADCMode,
							   config->DACMode);
		if (errorStatus != kStatus_Success) {
			return errorStatus;
		}
	...
	/* set data format */
	errorStatus =
		CS42888_ConfigDataFormat(handle, config->format.mclk_HZ, config->format.sampleRate,
				      ...
				      cs42888_func_mode dacMode)
{
	if (CS42888_ModifyReg(handle, CS42888_FUNCTIONAL_MODE, CS42888_FUNCTIONAL_MODE_DAC_FM_MASK,
		...
		return CS42888_ModifyReg(handle, CS42888_FUNCTIONAL_MODE,
				  ...
				  uint32_t bits)
{
	...
	case 128:
		if (CLOCK_RATE_IN_RANGE(sample_rate, 100000U, 200000U) && CLOCK_RATE_IN_RANGE(mclk,
											      2048000U,
											      25600000U))
		{
			val |= 4U;
	...
	case 256:
		if (CLOCK_RATE_IN_RANGE(sample_rate, 50000U, 100000U) && CLOCK_RATE_IN_RANGE(mclk,
											     2048000U,
											     25600000U))
		{
			val |= 4U;
	...
	case 512:
		if (CLOCK_RATE_IN_RANGE(sample_rate, 4000U, 50000U) && CLOCK_RATE_IN_RANGE(mclk,
											   2048000U,
											   25600000U))
		{
			val |= 4U;
