...
#define WM8904_MAP_BITWIDTH(x) \
	(x == kWM8904_BitWidth16 ? 16 : x == kWM8904_BitWidth20 ? 20 : x == kWM8904_BitWidth24 ? \
	 24 : 32)
/*******************************************************************************
 * Prototypes
 ******************************************************************************/
...
static const uint8_t allRegisters[] = {
	0x00, 0x04, 0x05, 0x06, 0x07, 0x0A, 0x0C, 0x0E, 0x0F, 0x12, 0x14, 0x15, 0x16, 0x18, 0x19,
	0x1A, 0x1B,
	0x1E, 0x1F, 0x20, 0x21, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D, 0x2E,
	0x2F, 0x39,
	0x3A, 0x3B, 0x3C, 0x3D, 0x43, 0x44, 0x45, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x5A,
	0x5E, 0x62,
	0x68, 0x6C, 0x6D, 0x6E, 0x6F, 0x70, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7A, 0x7B, 0x7C,
	0x7E, 0x7F,
	0x80, 0x81, 0x82, 0x86, 0x87, 0x88, 0x89, 0x8A, 0x8B, 0x8C, 0x8D, 0x8E, 0x8F, 0x90, 0x91,
	0x92, 0x93,
...
static status_t WM8904_UpdateFormat(wm8904_handle_t *handle, wm8904_audio_format_t *format)
{
	...
	result = WM8904_WriteRegister(handle, WM8904_CLK_RATES_1, (format->fsRatio << 10) |
				      format->sampleRate);
	if (result != kStatus_WM8904_Success) {
		return result;
	}
	...
	result = WM8904_ModifyRegister(handle, WM8904_AUDIO_IF_1, 0x000C | (1U << 6U),
				       format->bitWidth << 2U);
	if (result != kStatus_WM8904_Success) {
		return result;
	}
...
status_t WM8904_WriteRegister(wm8904_handle_t *handle, uint8_t reg, uint16_t value)
{
	...
	return CODEC_I2C_Send(handle->i2cHandle, handle->config->slaveAddress, reg, 1U,
			      (uint8_t *)&writeValue, 2U);
}
...
status_t WM8904_ReadRegister(wm8904_handle_t *handle, uint8_t reg, uint16_t *value)
{
	...
	retval = CODEC_I2C_Receive(handle->i2cHandle, handle->config->slaveAddress, reg, 1U,
...
status_t WM8904_Init(wm8904_handle_t *handle, wm8904_config_t *wm8904Config)
{
	...
	result = CODEC_I2C_Init(handle->i2cHandle, wm8904Config->i2cConfig.codecI2CInstance,
				WM8904_I2C_BITRATE,
	...
	if (config->master) {
		result = WM8904_SeMasterClock(handle, sysclk / 2U, WM8904_MAP_SAMPLERATE(
	...
	result = WM8904_SetRecordChannel(handle, config->recordChannelLeft,
					 config->recordChannelRight);
	if (result != kStatus_WM8904_Success) {
		return result;
	}
	/* set play source */
...
status_t WM8904_SetFLLConfig(wm8904_handle_t *handle, wm8904_fll_config_t *config)
{
	...
	/* configure WM8904  */
	if (WM8904_ModifyRegister(handle, WM8904_FLL_CONTROL_2, 0x3F07U, ((outputDiv - 1) << 8U) |
				  ratio) !=
	    kStatus_Success) {
		return kStatus_WM8904_Fail;
	}
	...
	if (WM8904_ModifyRegister(handle, WM8904_FLL_CONTROL_4, 0x7FE0, n << 5U) !=
	    kStatus_Success) {
		return kStatus_WM8904_Fail;
	}

	if (kStatus_WM8904_Success !=
	    WM8904_WriteRegister(handle, WM8904_FLL_CONTROL_5, (inputDivider << 3U) |
				 config->source)) {
		return kStatus_WM8904_Fail;
	}
...
{
	return WM8904_ModifyRegister(handle, WM8904_AUDIO_IF_1, (0x0003 | (1U << 4U)),
				     (uint16_t)protocol);
}
...
{
	return WM8904_ModifyRegister(handle, WM8904_AUDIO_IF_1, 3U << 12U, 1U << 13U | timeSlot <<
				     12U);
}
...
{
	return WM8904_ModifyRegister(handle, WM8904_AUDIO_IF_1, 3U << 10U, 1U << 11U | timeSlot <<
				     10U);
}
...
status_t WM8904_SetVolume(wm8904_handle_t *handle, uint16_t volumeLeft, uint16_t volumeRight)
{
	...
	result = WM8904_ModifyRegister(handle, WM8904_ANALOG_OUT1_RIGHT, 0xBF, volumeRight |
				       0x0080);
	if (result != kStatus_WM8904_Success) {
		return result;
	}
...
status_t WM8904_SetChannelVolume(wm8904_handle_t *handle, uint32_t channel, uint32_t volume)
{
	...
	if (channel & kWM8904_HeadphoneLeft) {
		ret = WM8904_ModifyRegister(handle, WM8904_ANALOG_OUT1_LEFT, volume == 0U ? 0x100U :
					    0x3FU,
					    volume == 0U ? 0x100U : (volume));
	}
	/* headphone right channel */
	if (channel & kWM8904_HeadphoneRight) {
		ret = WM8904_ModifyRegister(handle, WM8904_ANALOG_OUT1_RIGHT, volume == 0U ?
					    0x100U : 0xBFU,
					    volume == 0U ? 0x100U : (volume | 0x80U));
	}
	/* line out left channel */
	if (channel & kWM8904_LineoutLeft) {
		ret = WM8904_ModifyRegister(handle, WM8904_ANALOG_OUT2_LEFT, volume == 0U ? 0x100U :
					    0x3FU,
					    volume == 0U ? 0x100U : (volume));
	}
	/* line out right channel */
	if (channel & kWM8904_LineoutRight) {
		ret = WM8904_ModifyRegister(handle, WM8904_ANALOG_OUT2_RIGHT, volume == 0U ?
					    0x100U : 0xBFU,
					    volume == 0U ? 0x100U : (volume | 0x80U));
	}
				 ...
				 rightRecordChannel)
{
	...
	uint16_t regLeftValue = 0U, regRightValue = 0U, regBitMask;
	status_t ret = kStatus_Success;
	uint8_t leftPositiveChannel = 0U, leftNegativeChannel = 0U, rightPositiveChannel = 0U,
