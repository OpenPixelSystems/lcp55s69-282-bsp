...
#define HAL_DA7212_MODULE_CAPABILITY \
	kCODEC_SupportModuleADC | kCODEC_SupportModuleDAC | kCODEC_SupportModuleHeadphone | \
...
status_t HAL_CODEC_DA7212_Init(void *handle, void *config)
{
	...
	da7212_handle_t *da7212Handle =
		(da7212_handle_t *)((uint32_t)(((codec_handle_t *)handle)->codecDevHandle));
 ...
 */
status_t HAL_CODEC_DA7212_SetFormat(void *handle, uint32_t mclk, uint32_t sampleRate, uint32_t
				    bitWidth)
{
	assert(handle != NULL);

	return DA7212_ConfigAudioFormat(
		(da7212_handle_t *)((uint32_t)(((codec_handle_t *)handle)->codecDevHandle)), mclk,
		sampleRate, bitWidth);
}
...
status_t HAL_CODEC_DA7212_SetVolume(void *handle, uint32_t playChannel, uint32_t volume)
{
	...
	return DA7212_SetChannelVolume(
		(da7212_handle_t *)((uint32_t)(((codec_handle_t *)handle)->codecDevHandle)),
		playChannel, volume);
}
 ...
 * return kStatus_Success is success, else configure failed.
 */
status_t HAL_CODEC_DA7212_SetRecordChannel(void *handle, uint32_t leftRecordChannel, uint32_t
					   rightRecordChannel)
{
	return kStatus_CODEC_NotSupport;
}
