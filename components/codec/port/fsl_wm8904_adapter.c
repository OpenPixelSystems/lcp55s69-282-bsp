 ...
 */
status_t HAL_CODEC_WM8904_SetFormat(void *handle, uint32_t mclk, uint32_t sampleRate, uint32_t
				    bitWidth)
{
	assert(handle != NULL);
 ...
 * return kStatus_Success is success, else configure failed.
 */
status_t HAL_CODEC_WM8904_SetRecordChannel(void *handle, uint32_t leftRecordChannel, uint32_t
					   rightRecordChannel)
{
	assert(handle != NULL);
