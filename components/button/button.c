...
static void BUTTON_Event(void *param)
{
	...
	if (((uint8_t)kStatus_BUTTON_PressIdle == buttonState->state.pressed) ||
		...
		if (buttonState->config.pinStateDefault == pinState) {
			if ((BUTTON_DOUBLE_CLICK_THRESHOLD + buttonState->pushPeriodCountLast) >=
			    buttonState->pushPeriodCount) {
				if ((s_buttonList.periodCount - buttonState->pushPeriodCount) <
				    BUTTON_SHORT_PRESS_THRESHOLD) {
					BUTTON_NotificationUpdate(buttonState,
				...
				if ((s_buttonList.periodCount - buttonState->pushPeriodCount) <
					...
					buttonState->state.pressed =
						(uint8_t)kStatus_BUTTON_PressDoubleStart;
				} else if ((s_buttonList.periodCount -
					    buttonState->pushPeriodCount) <
...
static void BUTTON_Task(void *param)
{
	...
	do{
		if (KOSA_StatusSuccess == OSA_EventWait(
			    (osa_event_handle_t)s_buttonList.eventHandle, osaEventFlagsAll_c, false,
			    osaWaitForever_c, &ev)) {
#endif
...
static void BUTTON_OpenTimer(void)
{
	...
	if (0U != initTimer) {
		...
		timerStatus = TM_InstallCallback(s_buttonList.timerHandle, BUTTON_TimerEvent,
						 &s_buttonList);
		assert(kStatus_TimerSuccess == timerStatus);
		timerStatus = TM_Start(s_buttonList.timerHandle, (uint8_t)kTimerModeLowPowerTimer,
...
button_status_t BUTTON_Init(button_handle_t buttonHandle, button_config_t *buttonConfig)
{
	...
	if (NULL == s_buttonList.button) {
		...
		osaStatus = OSA_TaskCreate((osa_task_handle_t)s_buttonList.taskHandle, OSA_TASK(
						   BUTTON_Task), &s_buttonList);
		assert(KOSA_StatusSuccess == osaStatus);
#endif
