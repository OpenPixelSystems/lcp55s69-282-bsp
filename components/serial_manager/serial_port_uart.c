...
static void Serial_UartCallback(hal_uart_handle_t handle, hal_uart_status_t status, void *userData)
{
	...
	if ((hal_uart_status_t)kStatus_HAL_UartRxIdle == status) {
		if ((NULL != serialUartHandle->rx.callback)) {
			...
			serialUartHandle->rx.callback(serialUartHandle->rx.callbackParam, &msg,
		...
		if (kStatus_HAL_UartSuccess ==
		    HAL_UartTransferReceiveNonBlocking(
		...
		if ((hal_uart_status_t)kStatus_HAL_UartSuccess ==
		    HAL_UartReceiveNonBlocking(
			    ((hal_uart_handle_t)&serialUartHandle->usartHandleBuffer[0]),
			    &serialUartHandle->rx.readBuffer[0],
		...
		if (0U != serialUartHandle->tx.busy) {
			...
			if ((NULL != serialUartHandle->tx.callback)) {
				...
				serialUartHandle->tx.callback(serialUartHandle->tx.callbackParam,
							      &msg, kStatus_SerialManager_Success);
			}
		}
	} else {
	}
...
serial_manager_status_t Serial_UartInit(serial_handle_t serialHandle, void *serialConfig)
{
	...
	if (kStatus_HAL_UartSuccess != HAL_UartInit(
		    ((hal_uart_handle_t)&serialUartHandle->usartHandleBuffer[0]), &config)) {
...
#if (defined(HAL_UART_TRANSFER_MODE) && (HAL_UART_TRANSFER_MODE > 0U))
	if (kStatus_HAL_UartSuccess !=
	    HAL_UartTransferInstallCallback(
		    ((hal_uart_handle_t)&serialUartHandle->usartHandleBuffer[0]),
		    Serial_UartCallback, serialUartHandle))
#else
	if (kStatus_HAL_UartSuccess != HAL_UartInstallCallback(
		    ((hal_uart_handle_t)&serialUartHandle->usartHandleBuffer[0]),
		    Serial_UartCallback,
		    serialUartHandle))
#endif
...
#if (defined(SERIAL_MANAGER_NON_BLOCKING_MODE) && (SERIAL_MANAGER_NON_BLOCKING_MODE > 0U))
#if (defined(HAL_UART_TRANSFER_MODE) && (HAL_UART_TRANSFER_MODE > 0U))
	(void)HAL_UartTransferAbortReceive(
		((hal_uart_handle_t)&serialUartHandle->usartHandleBuffer[0]));
#else
					 ...
					 length)
{
	...
	if (kStatus_HAL_UartSuccess !=
	    HAL_UartTransferSendNonBlocking(
		    ((hal_uart_handle_t)&serialUartHandle->usartHandleBuffer[0]), &transfer))
#else
	if (kStatus_HAL_UartSuccess !=
	    HAL_UartSendNonBlocking(((hal_uart_handle_t)&serialUartHandle->usartHandleBuffer[0]),
				    buffer, length))
#endif
	{
		serialUartHandle->tx.busy = 0U;
...
serial_manager_status_t Serial_UartCancelWrite(serial_handle_t serialHandle)
{
...
#if (defined(HAL_UART_TRANSFER_MODE) && (HAL_UART_TRANSFER_MODE > 0U))
	(void)HAL_UartTransferAbortSend(
		((hal_uart_handle_t)&serialUartHandle->usartHandleBuffer[0]));
#else
	...
	if (0U != isBusy) {
		if ((NULL != serialUartHandle->tx.callback)) {
			msg.buffer = serialUartHandle->tx.buffer;
			msg.length = serialUartHandle->tx.length;
			serialUartHandle->tx.callback(serialUartHandle->tx.callbackParam, &msg,
...
serial_manager_status_t Serial_UartEnterLowpower(serial_handle_t serialHandle)
{
	...
	if (kStatus_HAL_UartSuccess != HAL_UartEnterLowpower(
		    ((hal_uart_handle_t)&serialUartHandle->usartHandleBuffer[0]))) {
		return kStatus_SerialManager_Error;
	}
...
serial_manager_status_t Serial_UartExitLowpower(serial_handle_t serialHandle)
{
	...
	if (kStatus_HAL_UartSuccess != HAL_UartExitLowpower(
		    ((hal_uart_handle_t)&serialUartHandle->usartHandleBuffer[0]))) {
		return kStatus_SerialManager_Error;
	}
