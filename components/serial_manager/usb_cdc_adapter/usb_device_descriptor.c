					   ...
					   stringDescriptor)
{
	if (stringDescriptor->stringIndex == 0U) {
		...
		stringDescriptor->buffer =
			(uint8_t *)g_UsbDeviceLanguageList.languageList[languageId].string[
				languageIndex];
...
usb_status_t USB_DeviceSetSpeed(usb_device_handle handle, uint8_t speed)
{
	...
	ptr2 =
		(usb_descriptor_union_t *)(&g_UsbDeviceConfigurationDescriptor[
	...
	for (int i = 0; i < USB_CDC_VCOM_ENDPOINT_CIC_COUNT; i++) {
		if (USB_SPEED_HIGH == speed) {
			g_UsbDeviceCdcVcomCicEndpoints[i].maxPacketSize =
				HS_CDC_VCOM_INTERRUPT_IN_PACKET_SIZE;
		} else {
			g_UsbDeviceCdcVcomCicEndpoints[i].maxPacketSize =
				FS_CDC_VCOM_INTERRUPT_IN_PACKET_SIZE;
	...
	for (int i = 0; i < USB_CDC_VCOM_ENDPOINT_DIC_COUNT; i++) {
		if (USB_SPEED_HIGH == speed) {
			if (g_UsbDeviceCdcVcomDicEndpoints[i].endpointAddress &
			    USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) {
				g_UsbDeviceCdcVcomDicEndpoints[i].maxPacketSize =
					HS_CDC_VCOM_BULK_IN_PACKET_SIZE;
			} else {
				g_UsbDeviceCdcVcomDicEndpoints[i].maxPacketSize =
					HS_CDC_VCOM_BULK_OUT_PACKET_SIZE;
