...
typedef struct _serial_usb_cdc_state {
	...
	uint8_t				currentInterfaceAlternateSetting[
		USB_CDC_VCOM_INTERFACE_COUNT];                  /* Current alternate setting value for each
	                                                         * interface. */
...
serial_manager_status_t Serial_UsbCdcRead(serial_handle_t serialHandle, uint8_t *buffer, uint32_t
					  length);
 ...
 * Code
 ******************************************************************************/
static serial_usb_cdc_state_t *USB_DeviceGetInstanceFromDeviceHandle(
	serial_usb_cdc_state_t *head,
	usb_device_handle
	deviceHandle)
{
	while (head) {
...
static serial_usb_cdc_state_t *USB_DeviceGetInstanceFromClassHandle(
	serial_usb_cdc_state_t *head,
	class_handle_t
	ClassHandle)
{
...
static usb_status_t USB_DeviceRemoveItem(serial_usb_cdc_state_t **	head,
					 serial_usb_cdc_state_t *	node)
{
	serial_usb_cdc_state_t *p = *head;
...
static usb_status_t USB_DeviceCdcVcomCallback(class_handle_t handle, uint32_t event, void *param)
{
	...
	case kUSB_DeviceCdcEventSendResponse:
	{
		if ((1 == serialUsbCdc->attach)) {
			if ((epCbParam->length != 0) &&
				...
				*/
				USB_DeviceCdcAcmSend(handle, USB_CDC_VCOM_BULK_IN_ENDPOINT, NULL,
						     0);
			} else {
				...
				if ((NULL != serialUsbCdc->tx.callback)) {
					...
					serialUsbCdc->tx.callback(serialUsbCdc->tx.callbackParam,
								  &msg, serialManagerStatus);
				}
			}
		}
	}
	...
	case kUSB_DeviceCdcEventRecvResponse:
	{
		...
		if ((1 == serialUsbCdc->startTransactions)) {
			...
			if ((NULL != serialUsbCdc->rx.callback)) {
				...
				serialUsbCdc->rx.callback(serialUsbCdc->rx.callbackParam, &msg,
							  callbackStatus);
			}
		}
	}
	...
	case kUSB_DeviceCdcEventSetControlLineState:
	{
		...
		acmInfo->dtePresent = (acmInfo->dteStatus &
				       USB_DEVICE_CDC_CONTROL_SIG_BITMAP_DTE_PRESENCE) ? true :
				      false;
...
static usb_status_t USB_DeviceCallback(usb_device_handle handle, uint32_t event, void *param)
{
	...
	case kUSB_DeviceEventBusReset:
	{
		...
		if (kStatus_USB_Success == USB_DeviceClassGetSpeed(serialUsbCdc->instance,
								   &serialUsbCdc->speed)) {
	...
	case kUSB_DeviceEventSetConfiguration:
		if (param) {
			...
			if (USB_CDC_VCOM_CONFIGURE_INDEX == (*temp8)) {
				...
				Serial_UsbCdcRead(serialUsbCdc, s_currRecvBuf,
						  g_UsbDeviceCdcVcomDicEndpoints[0].maxPacketSize);
			}
		}
		break;
	case kUSB_DeviceEventSetInterface:
		if (serialUsbCdc->attach) {
			...
			if (interface < USB_CDC_VCOM_INTERFACE_COUNT) {
				serialUsbCdc->currentInterfaceAlternateSetting[interface] =
					alternateSetting;
			}
		}
		break;
...
static void USB_DeviceIsrEnable(serial_usb_cdc_state_t *serialUsbCdc)
{
	...
	serialUsbCdc->irqNumber = usbDeviceEhciIrq[serialUsbCdc->instance -
						   kSerialManager_UsbControllerEhci0];
#endif

#if defined(USB_DEVICE_CONFIG_KHCI) && (USB_DEVICE_CONFIG_KHCI > 0U)
	uint8_t usbDeviceKhciIrq[] = USB_IRQS;
	serialUsbCdc->irqNumber = usbDeviceKhciIrq[serialUsbCdc->instance -
						   kSerialManager_UsbControllerKhci0];
#endif

#if defined(USB_DEVICE_CONFIG_LPCIP3511FS) && (USB_DEVICE_CONFIG_LPCIP3511FS > 0U)
	uint8_t usbDeviceIP3511Irq[] = USB_IRQS;
	serialUsbCdc->irqNumber = usbDeviceIP3511Irq[serialUsbCdc->instance -
...
serial_manager_status_t Serial_UsbCdcInit(serial_handle_t serialHandle, void *serialConfig)
{
	...
	if (kStatus_USB_Success !=
	    USB_DeviceClassInit(serialUsbCdc->instance, &s_cdcAcmConfigList,
				&serialUsbCdc->deviceHandle)) {
		return kStatus_SerialManager_Error;
	} else {
...
serial_manager_status_t Serial_UsbCdcWrite(serial_handle_t serialHandle, uint8_t *buffer, uint32_t
					   length)
{
	serial_usb_cdc_state_t *serialUsbCdc;
					  ...
					  length)
{
	...
	if (kStatus_USB_Success !=
	    USB_DeviceCdcAcmRecv(serialUsbCdc->cdcAcmHandle, USB_CDC_VCOM_BULK_OUT_ENDPOINT,
				 primeBuffer, primeLength)) {
		serialUsbCdc->rx.busy = 0U;
		return kStatus_SerialManager_Error;
	}
...
serial_manager_status_t Serial_UsbCdcInstallTxCallback(serial_handle_t
						       serialHandle,
						       serial_manager_callback_t callback,
						       void *
						       callbackParam)
...
serial_manager_status_t Serial_UsbCdcInstallRxCallback(serial_handle_t
						       serialHandle,
						       serial_manager_callback_t callback,
						       void *
						       callbackParam)
