...
#define IncrementActiveTimerNumber(type)                                                                     \
	((((type) & (uint8_t)kTimerModeLowPowerTimer) != 0U) ? \
...
#else
	(void)OSA_EventSet((osa_event_handle_t)s_timermanager.halTimerTaskEventHandle,
			   mTmrDummyEvent_c);
#endif
#else
...
static void TimersUpdate(bool updateRemainingUs, bool updateOnlyPowerTimer, uint32_t remainingUs)
{
	...
	if ((s_timermanager.numberOfLowPowerActiveTimers != 0U) ||
	    (s_timermanager.numberOfActiveTimers != 0U)) {
		...
		while (list_element != NULL) {
			...
			if (updateRemainingUs) {
				if ((timer_state_t)TimerGetTimerStatus(th) == kTimerStateActive_c) {
					if ((updateOnlyPowerTimer && (0U != IsLowPowerTimer(
									      TimerGetTimerType(
										      th)))) ||
					    (!updateOnlyPowerTimer)) {
						if (th->remainingUs > remainingUs) {
							th->remainingUs = th->remainingUs -
									  remainingUs;
						} else {
							th->remainingUs = 0;
						}
...
static void TimersUpdateSyncTask(uint32_t remainingUs)
{
	...
	s_timermanager.previousTimeInUs = HAL_TimerGetCurrentTimerCount(
		(hal_timer_handle_t)s_timermanager.halTimerHandle);
	NotifyTimersTask();
}
...
static void TimerManagerTask(void *param)
{
	...
	do{
		if (KOSA_StatusSuccess == OSA_EventWait(
			    (osa_event_handle_t)s_timermanager.halTimerTaskEventHandle,
			    osaEventFlagsAll_c, false, osaWaitForever_c,
			    &ev)) {
#endif
#endif
	...
	s_timermanager.mUsInTimerInterval = HAL_TimerGetMaxTimeout(
		(hal_timer_handle_t)s_timermanager.halTimerHandle);
	list_element = LIST_GetHead(timerHandle);
	while (NULL != list_element) {
		...
		if (kTimerStateActive_c == state) {
			...
			if ((kTimerStateActive_c == state) && (s_timermanager.mUsInTimerInterval >
							       th->remainingUs)) {
...
static void TimerEnable(timer_handle_t timerHandle)
{
	...
	if ((uint8_t)kTimerStateInactive_c == TimerGetTimerStatus(timerHandle)) {
		...
		TimersUpdateSyncTask(HAL_TimerGetCurrentTimerCount(
					     (hal_timer_handle_t)s_timermanager.halTimerHandle));
	}
	EnableGlobalIRQ(regPrimask);
}
...
timer_status_t TM_Init(timer_config_t *timerConfig)
{
	...
	if (0U == initialized) {
		...
		COMMON_TASK_init();
#else
		status = OSA_EventCreate((osa_event_handle_t)s_timermanager.halTimerTaskEventHandle,
					 true);
		assert(KOSA_StatusSuccess == (osa_status_t)status);

		status = OSA_TaskCreate((osa_task_handle_t)s_timermanager.timerTaskHandle, OSA_TASK(
						TimerManagerTask), NULL);
		assert(KOSA_StatusSuccess == (osa_status_t)status);
#endif
#endif
...
timer_status_t TM_Open(timer_handle_t timerHandle)
{
	...
	(void)LIST_AddTail((void *)&s_timermanager.timerHead,
			   (list_element_handle_t)&(timerState->link));
	return kStatus_TimerSuccess;
}
...
uint32_t TM_GetRemainingTime(timer_handle_t timerHandle)
{
	...
	return (uint32_t)(timerState->remainingUs) -
	       (uint32_t)(HAL_TimerGetCurrentTimerCount(
				  (hal_timer_handle_t)s_timermanager.halTimerHandle) -
...
uint32_t TM_GetFirstExpireTime(uint8_t timerType)
{
	...
	while (NULL != list_element) {
		timer_handle_struct_t *th = (timer_handle_struct_t *)(void *)list_element;
		if ((bool)TM_IsTimerActive(th) && ((timerType & TimerGetTimerType(timerHandle)) >
						   0U)) {
...
uint32_t TM_NotCountedTimeBeforeSleep(void)
{
	...
	if (0U != s_timermanager.numberOfLowPowerActiveTimers) {
		currentTimeInUs = HAL_TimerGetCurrentTimerCount(
			(hal_timer_handle_t)s_timermanager.halTimerHandle);
		HAL_TimerDisable((hal_timer_handle_t)s_timermanager.halTimerHandle);
		s_timermanager.timerHardwareIsRunning = 0U;
