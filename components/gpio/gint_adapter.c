...
hal_gpio_status_t HAL_GpioDeinit(hal_gpio_handle_t gpioHandle)
{
	...
	/* Disbale mask */
	GINT_ConfigPins(gintList[gpioStateHandle->gintInstance],
			(gint_port_t)gpioStateHandle->pin.port, 0, 0);
	/* Relase the instance */
					 ...
					 gpioTrigger)
{
	...
	GINT_ConfigPins(gintList[gpioStateHandle->gintInstance],
			(gint_port_t)gpioStateHandle->pin.port, polarityMask,
			enableMask);
...
static void HAL_GpioInterruptCallback(void)
{
	...
	while (head) {
		...
		if (0 != (enableMask & pinMask)) {
			...
			pinState = (GPIO_PinRead(gpioList[0], head->pin.port, head->pin.pin) ? 1 :
				    0);
...
hal_gpio_status_t HAL_GpioGetInput(hal_gpio_handle_t gpioHandle, uint8_t *pinState)
{
	...
	*pinState = ((GPIO_PinRead(gpioList[0], gpioStateHandle->pin.port,
				   gpioStateHandle->pin.pin)) ? 1 : 0);
	return kStatus_HAL_GpioSuccess;
}
...
hal_gpio_status_t HAL_GpioSetOutput(hal_gpio_handle_t gpioHandle, uint8_t pinState)
{
	...
	GPIO_PinWrite(gpioList[0], gpioStateHandle->pin.port, gpioStateHandle->pin.pin, (pinState) ?
		      1 : 0);
	return kStatus_HAL_GpioSuccess;
}
