...
#include "fsl_gpio.h"
#if (defined(FSL_FEATURE_SYSCON_HAS_PINT_SEL_REGISTER) && \
	(FSL_FEATURE_SYSCON_HAS_PINT_SEL_REGISTER > 0U))
...
#if (defined(FSL_FEATURE_SYSCON_HAS_PINT_SEL_REGISTER) && \
	(FSL_FEATURE_SYSCON_HAS_PINT_SEL_REGISTER > 0U))
#define HAL_INPUTMUX_GpioPortPinToPintsel(port, pin) ((pin) + ((port) << 5) + (PINTSEL_ID << \
									       SYSCON_SHIFT))
#else
#define HAL_INPUTMUX_GpioPortPinToPintsel(port, pin) ((pin) + ((port) << 5) + (PINTSEL_PMUX_ID << \
									       PMUX_SHIFT))
#endif
...
hal_gpio_status_t HAL_GpioInit(hal_gpio_handle_t gpioHandle, hal_gpio_pin_config_t *pinConfig)
{
	...
	if ((NULL != s_GpioHead) &&
	    (kStatus_HAL_GpioSuccess != HAL_GpioConflictSearch(s_GpioHead, pinConfig->port,
							       pinConfig->pin))) {
		return kStatus_HAL_GpioPinConflict;
	}
...
hal_gpio_status_t HAL_GpioDeinit(hal_gpio_handle_t gpioHandle)
{
	...
	if (0U != gpioStateHandle->pin.pint.pintInitialized) {
		...
		PINT_DisableCallbackByIndex(PINT,
					    (pint_pin_int_t)gpioStateHandle->pin.pint.pintIndex);
	}

	for (k = 0U; k < FSL_FEATURE_PINT_NUMBER_OF_CONNECTED_OUTPUTS; k++) {
		if (HAL_GPIO_INPUTMUX_PIN_FLAG(gpioStateHandle->pin.port,
					       gpioStateHandle->pin.pin) == s_GpioInputMuxPin[k]) {
...
hal_gpio_status_t HAL_GpioGetInput(hal_gpio_handle_t gpioHandle, uint8_t *pinState)
{
	...
	*pinState = ((0U != GPIO_PinRead(s_GpioList[0], gpioStateHandle->pin.port,
					 gpioStateHandle->pin.pin)) ? 1U : 0U);
	return kStatus_HAL_GpioSuccess;
}
...
hal_gpio_status_t HAL_GpioSetOutput(hal_gpio_handle_t gpioHandle, uint8_t pinState)
{
	...
	GPIO_PinWrite(s_GpioList[0], gpioStateHandle->pin.port, gpioStateHandle->pin.pin, (0U !=
											   pinState)
	? 1U : 0U);
	return kStatus_HAL_GpioSuccess;
}
					 ...
					 gpioTrigger)
{
	...
	if (0U == gpioStateHandle->pin.pint.pintInitialized) {
		for (uint32_t inputMuxIndex = 0; inputMuxIndex <
		     FSL_FEATURE_PINT_NUMBER_OF_CONNECTED_OUTPUTS; inputMuxIndex++) {
			if (0U == (s_GpioInputMux[inputMuxIndex >> 3] & (1U << (inputMuxIndex &
										0x07U)))) {
	...
	if (0U != gpioStateHandle->pin.pint.pintInitialized) {
		NVIC_SetPriority(s_PintIRQ[gpioStateHandle->pin.pint.pintIndex],
				 HAL_GPIO_ISR_PRIORITY);
		s_GpioInputMuxPin[gpioStateHandle->pin.pint.pintIndex] =
			HAL_GPIO_INPUTMUX_PIN_FLAG(gpioStateHandle->pin.port,
						   gpioStateHandle->pin.pin);
		/* Connect trigger sources to PINT */
#if (defined(FSL_FEATURE_SYSCON_HAS_PINT_SEL_REGISTER) && \
		(FSL_FEATURE_SYSCON_HAS_PINT_SEL_REGISTER > 0U))
		SYSCON_AttachSignal(SYSCON, (uint32_t)gpioStateHandle->pin.pint.pintIndex,
				    (syscon_connection_t)HAL_INPUTMUX_GpioPortPinToPintsel(
					    (uint32_t)gpioStateHandle->pin.port,
					    (uint32_t)
					    gpioStateHandle->pin.pin));
#else
		INPUTMUX_Init(INPUTMUX);
		INPUTMUX_AttachSignal(INPUTMUX, (uint32_t)gpioStateHandle->pin.pint.pintIndex,
				      (inputmux_connection_t)HAL_INPUTMUX_GpioPortPinToPintsel(
					      (uint32_t)gpioStateHandle->pin.port,
					      (
		...
		PINT_PinInterruptConfig(PINT, (pint_pin_int_t)gpioStateHandle->pin.pint.pintIndex,
					triggerType,
					HAL_GpioInterruptCallback);
		/* Enable the pin's PINT call back */
		PINT_EnableCallbackByIndex(PINT,
					   (pint_pin_int_t)gpioStateHandle->pin.pint.pintIndex);
	} else {
...
hal_gpio_status_t HAL_GpioWakeUpSetting(hal_gpio_handle_t gpioHandle, uint8_t enable)
{
	...
	(!(defined(FSL_FEATURE_SYSCON_STARTER_DISCONTINUOUS) && \
	FSL_FEATURE_SYSCON_STARTER_DISCONTINUOUS))
