...
status_t FT6X06_Init(ft6x06_handle_t *handle, ARM_DRIVER_I2C *i2c_driver)
{
	...
	if (handle->i2c_driver->MasterTransmit(FT6X06_I2C_ADDRESS, i2c_buf, 2, false) !=
	    ARM_DRIVER_OK) {
		status = kStatus_Fail;
...
status_t FT6X06_ReadTouchData(ft6x06_handle_t *handle)
{
	...
	if (handle->i2c_driver->MasterTransmit(FT6X06_I2C_ADDRESS, i2c_buf, 1, true) !=
	    ARM_DRIVER_OK) {
		status = kStatus_Fail;
	} else if (FT_6X06_WaitEvent(handle) != ARM_I2C_EVENT_TRANSFER_DONE) {
		status = kStatus_Fail;
	} else if (handle->i2c_driver->MasterReceive(FT6X06_I2C_ADDRESS, handle->touch_buf,
						     FT6X06_TOUCH_DATA_LEN, false) !=
		   ARM_DRIVER_OK) {
		status = kStatus_Fail;
	} else if (FT_6X06_WaitEvent(handle) != ARM_I2C_EVENT_TRANSFER_DONE) {
		status = kStatus_Fail;
...
status_t FT6X06_GetSingleTouch(ft6x06_handle_t *handle, touch_event_t *touch_event, int *touch_x,
			       int *touch_y)
{
	status_t status;
	...
	if (status == kStatus_Success) {
		ft6x06_touch_data_t *touch_data =
			(ft6x06_touch_data_t *)(void *)(handle->touch_buf);
...
status_t FT6X06_GetMultiTouch(ft6x06_handle_t *handle, int *touch_count, touch_point_t
			      touch_array[FT6X06_MAX_TOUCHES])
{
	status_t status;
