...
static void FreeRtosMessageBuffersEventHandler(uint16_t eventData, void *context)
{
	...
	if (APP_MESSAGE_BUFFER_EVENT_DATA == eventData) {
		 ...
		 * arrive. */
		(void)xMessageBufferSendCompletedFromISR(xPrimaryToSecondaryMessageBuffer,
...
static void app_task(void *param)
{
	...
	while (msg.DATA <= 100U) {
		...
		(void)xMessageBufferReceive(xPrimaryToSecondaryMessageBuffer, (void *)&msg,
					    sizeof(THE_MESSAGE), portMAX_DELAY);

		msg.DATA++;
		(void)PRINTF("Sending pong...\r\n");

		(void)xMessageBufferSend(xSecondaryToPrimaryMessageBuffer, (void *)&msg,
